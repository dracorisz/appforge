...
export function AuthEmailFlowPage({ mode }: { mode: Mode }) {
  ...
  const description = mode === 'forgot'
    ? 'Enter your email and AppForge will send a secure recovery link.'
    : mode === 'reset'
      ? 'Use the recovery session from your email to set a new login password.'
      : 'Finish email verification or request a fresh confirmation link.'
  ...
  return <div className="flex min-h-dvh flex-col bg-background text-foreground">
    ...
    <main className="mx-auto flex w-full max-w-xl flex-1 items-center px-4 py-8">
      <Card className="w-full p-8 shadow-xl">
        ...
        {mode === 'reset' && state === 'ready' && <form onSubmit={updatePassword} className="mt-4 grid gap-4"><Input label="New password" type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} required /><Input label="Confirm new password" type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} required /><Button type="submit">Reset password</Button></form>}

        {mode === 'confirm' && state !== 'loading' && <form onSubmit={resendConfirmation} className="mt-4 grid gap-4"><Input label="Email for confirmation resend" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required /><Button type="submit" variant="secondary" disabled={state === 'loading' || !email.trim()}>{state === 'loading' ? <Loader2 className="animate-spin" /> : <MailCheck />} Resend confirmation</Button></form>}

        <div className="mt-8 flex flex-wrap items-center gap-4 text-sm"><Link to="/login" className="font-medium text-foreground hover:underline">Back to sign in</Link>{mode !== 'forgot' && <Link to="/auth/forgot-password" className="text-muted-foreground hover:text-foreground hover:underline">Request a new recovery link</Link>}{state === 'success' && <Link to="/" className="inline-flex items-center gap-2 font-medium text-foreground hover:underline"><CheckCircle2 className="h-4 w-4" /> Continue to AppForge</Link>}</div>
      </Card>
    </main>
    <PublicFooter />
  </div>
