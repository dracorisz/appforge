...
import { ArrowLeftRight, ArrowRight, Cloud, Github, Heart, Search, ShieldCheck, Sparkles } from 'lucide-react'
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden bg-background text-foreground">
      <div className="relative isolate flex min-h-dvh flex-col overflow-hidden">
        ...
        <header className="mx-auto flex w-full max-w-7xl shrink-0 items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
          <Link to="/" className="group inline-flex items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background">
            <img src="/appforge-mark.svg" alt="AppForge" className="h-9 w-9 transition-transform duration-200 group-hover:scale-[1.04]" />
            <div>
        ...
        <main className="mx-auto grid w-full max-w-7xl flex-1 items-center gap-10 px-4 pb-10 pt-6 sm:px-6 sm:pt-10 lg:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)] lg:gap-16 lg:px-8 lg:pb-12 lg:pt-4">
          <section className="max-w-3xl">
            ...
            <h1 className="mt-6 max-w-3xl text-balance text-5xl font-semibold tracking-[-0.05em] sm:text-6xl lg:text-7xl xl:text-[5.25rem] xl:leading-[0.96]">
              Build useful things.
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              <Button className="h-11 px-5" onClick={login} disabled={busy || loading}>
                {loading ? 'Checking session…' : user ? 'Open workspace' : busy ? 'Opening Google…' : 'Continue with Google'}
                {!loading && !busy && <ArrowRight className="h-4 w-4" />}
              </Button>
          ...
          <section className="relative mx-auto w-full max-w-xl lg:max-w-none" aria-label="AppForge public tools and workspace access">
            ...
            <div className="rounded-[2rem] border border-border/70 bg-background/75 p-4 shadow-2xl shadow-foreground/5 backdrop-blur-xl sm:p-5">
              <div className="flex items-center justify-between gap-4 border-b border-border/60 pb-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-border/70 bg-background">
                    <img src="/appforge-mark.svg" alt="" className="h-8 w-8" />
                  </div>
                  <div>
                    <div className="text-sm font-semibold">One workshop, many tools</div>
                    <div className="text-xs text-muted-foreground">Public first · workspace when you need it</div>
                  </div>
                </div>
              ...
              <div className="grid gap-2 py-4">
                {publicTools.map(({ label, description, path, icon: Icon }) => (
                  <Link key={path} to={path} className="group flex items-center gap-3 rounded-xl border border-transparent px-3 py-3 transition-colors hover:border-border/70 hover:bg-accent/55 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
              ...
              <Link to="/huggingface" className="group flex items-center gap-3 rounded-xl border border-border/70 bg-background/65 px-3 py-3 transition-colors hover:bg-accent/55 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <img src="https://huggingface.co/front/assets/huggingface_logo-noborder.svg" alt="Hugging Face" className="h-9 w-9 rounded-lg object-contain" />
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-medium">Hugging Face showcase</span>
                  <span className="block text-xs text-muted-foreground">Models, providers and creator-selected scenes</span>
                </span>
                <ArrowRight className="h-4 w-4 text-muted-foreground transition-transform group-hover:translate-x-0.5" />
              </Link>

              <div className="mt-4 rounded-xl border border-border/60 bg-accent/25 p-4">
                <div className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">Workspace</div>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">Sign in for Story Studio, synced settings, private storage and account-scoped projects.</p>
                <button type="button" onClick={login} disabled={busy || loading} className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-foreground transition-opacity hover:opacity-70 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  {user ? 'Open workspace' : 'Use Google identity'} <ArrowRight className="h-4 w-4" />
                </button>
              </div>
            </div>
          </section>
        ...
        <footer className="mx-auto flex w-full max-w-7xl shrink-0 flex-col gap-3 border-t border-border/60 px-4 py-4 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
          <span className="inline-flex items-center gap-2"><img src="/appforge-mark.svg" alt="" className="h-4 w-4" /> AppForge · public beta</span>
