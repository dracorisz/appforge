      ...
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col px-4 py-5 sm:px-6 lg:h-dvh lg:min-h-0 lg:px-8 lg:py-4">
        ...
        <main className="grid flex-1 items-center gap-10 py-12 lg:min-h-0 lg:grid-cols-[1.15fr_0.85fr] lg:gap-8 lg:py-5">
          <section className="max-w-2xl">
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg lg:mt-3 lg:text-base lg:leading-6 xl:text-lg xl:leading-7">Try Scrapper Pro without an account, then sign in to unlock the full AppForge workspace, synced profile, favorites, recent tools and protected project features.</p>

            <div className="mt-7 grid max-w-xl gap-3 sm:grid-cols-3 lg:mt-5">
              {[[String(apps.length), 'registered apps'], [String(liveCount), 'active or building'], ['MIT', 'open-source license']].map(([value, label]) => <div key={label} className="surface-card rounded-xl border p-3.5 lg:p-3"><div className="text-xl font-semibold tracking-tight">{value}</div><div className="mt-0.5 text-xs text-muted-foreground">{label}</div></div>)}
            </div>

            <div className="mt-8 flex flex-wrap gap-2.5 lg:mt-5">
            ...
            <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-sm text-muted-foreground lg:mt-4 lg:text-xs xl:text-sm"><span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5" /> Public test route</span><span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5" /> Google + Supabase identity</span><span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5" /> Installable PWA shell</span></div>
          </section>
