  ...
  return (
    <aside className={`flex h-full flex-col bg-background/92 backdrop-blur-xl transition-[width,transform,opacity] duration-200 ease-out ${onClose ? 'w-screen max-w-none border-r-0' : `border-r border-border ${isCollapsed ? 'w-16' : 'w-64'}`}>
      <div className={`flex items-center gap-2 py-4 ${isCollapsed ? 'flex-col px-2' : 'px-4'}`}>
        {isCollapsed && <NavLink to="/" aria-label="AppForge home" className="shrink-0"><img src="/favicon.svg" alt="" className="h-6 w-6" /></NavLink>}
        {!isCollapsed && <NavLink to="/" onClick={onClose} className="flex min-w-0 items-center gap-2"><img src="/favicon.svg" alt="AppForge" className="h-8 w-8 shrink-0" /><h1 className="truncate text-sm font-semibold text-foreground">AppForge</h1></NavLink>}
