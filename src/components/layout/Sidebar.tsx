...
export function Sidebar({ onClose, collapsed: collapsedProp, onToggleCollapse }: { onClose?: () => void; collapsed?: boolean; onToggleCollapse?: () => void }) {
  ...
  return <aside className={`relative z-40 flex h-full flex-col overflow-visible bg-background/92 backdrop-blur-xl transition-[width,transform,opacity] duration-200 ease-out ${asideWidth}`}>
    ...
    {!isCollapsed && <div className="px-4 pb-2"><div className="relative"><SearchInput value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onClear={() => setSearchQuery('')} placeholder="Search apps…" aria-label="Search apps" />{searchResults.length > 0 && <div className="surface-card mt-2 max-h-52 space-y-2 overflow-y-auto rounded-xl border p-2 shadow-xl">{searchResults.slice(0, 8).map((app) => { const Icon = iconMap[app.id === 'ai-dragon-arena' ? 'DragonArena' : app.icon] || Wrench; return <NavLink key={app.id} to={app.route} onClick={closeSearchAndSidebar} className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground"><Icon className="h-4 w-4 shrink-0" /><span className="truncate">{app.name}</span></NavLink> })}</div>}</div></div>}
    <nav className={`scrollbar-hide flex-1 px-4 py-2 ${isCollapsed ? 'overflow-visible' : 'overflow-y-auto overflow-x-hidden'}`}>
      {!isCollapsed && <h3 className="mb-2 px-4 text-sm font-semibold uppercase tracking-[0.14em] text-muted-foreground">System</h3>}
