  ...
  return (
    <aside className={`flex h-full flex-col border-r border-border bg-background/92 backdrop-blur-xl transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
        ...
        {!isCollapsed && (
          <NavLink to="/" onClick={onClose} className="flex min-w-0 items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-muted-foreground">Simple, powerful tools</p>
            </div>
          </NavLink>
        )}
        <button onClick={toggleCollapse} className="ml-auto rounded-lg p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground" aria-label={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
      ...
      <nav className="scrollbar-hide flex-1 space-y-5 overflow-y-auto overflow-x-hidden px-3 py-2">
        <div>
          {!isCollapsed && <h3 className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Core</h3>}
        ...
        <div>
          {!isCollapsed && <h3 className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Categories</h3>}
          <div className="space-y-0.5">
