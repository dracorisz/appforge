...
import {
  ...
  Image as ImageIcon, LayoutDashboard, Lock, LogOut, Palette, QrCode, Regex, Search,
  Settings, Sliders, Sparkles, Table2, Type, Users, Video, Wrench, X,
} from 'lucide-react'
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = { ArrowLeftRight, Binary, Braces, Calendar, CloudSun, Code, FileCode, FileText, Hash, Image: ImageIcon, Lock, Palette, QrCode, Regex, Search, Sparkles, Table2, Type, Video, Wrench, DragonArena: DragonArenaIcon }
...
export function Sidebar({ onClose, collapsed: collapsedProp, onToggleCollapse }: { onClose?: () => void; collapsed?: boolean; onToggleCollapse?: () => void }) {
  ...
  const sidebarApps = getAllApps().filter((app) =>
    app.status !== 'deprecated' && isAppVisibleInSidebar(app.id, categoryOverrides[appSidebarPreferenceKey(app.id)]),
  )
  ...
  return (
    <aside className={`flex h-full flex-col bg-background/92 backdrop-blur-xl transition-[width,transform,opacity] duration-200 ease-out ${onClose ? 'w-screen max-w-none border-r-0' : `border-r border-border ${isCollapsed ? 'w-16' : 'w-64'}`}`}>
      <div className={`flex items-center gap-2 py-4 ${isCollapsed ? 'flex-col px-2' : 'px-4'}`}>{isCollapsed && <NavLink to="/" aria-label="AppForge home" className="shrink-0"><img src="/favicon.svg" alt="" className="h-6 w-6" /></NavLink>}{!isCollapsed && <NavLink to="/" onClick={onClose} className="flex min-w-0 items-center gap-2"><img src="/favicon.svg" alt="AppForge" className="h-8 w-8 shrink-0" /><div className="min-w-0"><h1 className="truncate text-sm font-semibold text-foreground">AppForge</h1><p className="truncate text-xs text-muted-foreground">Build · use · fork</p></div></NavLink>}{onClose ? <button onClick={onClose} className="ml-auto rounded-lg p-2 text-muted-foreground hover:bg-accent hover:text-foreground" aria-label="Close navigation"><X className="h-5 w-5" /></button> : canCollapse && <button onClick={toggleCollapse} className={`${isCollapsed ? '' : 'ml-auto'} rounded-lg p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground`} aria-label={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>{isCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}</button>}</div>
      ...
      <nav className="scrollbar-hide flex-1 overflow-y-auto overflow-x-hidden px-3 py-2">
        ...
        {(sidebarCategories.length > 0 || sidebarApps.length > 0) && <div className="mt-5">
          ...
          {sidebarApps.length > 0 && <div className={`${sidebarCategories.length > 0 ? 'mt-2 ' : ''}space-y-0.5`}>{sidebarApps.map((app) => { const iconName = app.id === 'ai-dragon-arena' ? 'DragonArena' : app.icon; const Icon = iconMap[iconName] || Wrench; const active = location.pathname === app.route; return <NavLink key={app.id} to={app.route} onClick={onClose} title={isCollapsed ? app.name : undefined} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${active ? 'bg-accent text-foreground' : 'text-muted-foreground hover:bg-accent/70 hover:text-foreground'}`}><Icon className="h-4 w-4 shrink-0" />{!isCollapsed && <span className="truncate">{app.name}</span>}</NavLink> })}</div>}
        </div>}
      </nav>
