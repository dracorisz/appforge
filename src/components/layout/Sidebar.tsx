...
export function Sidebar({ onClose, collapsed: collapsedProp, onToggleCollapse }: { onClose?: () => void; collapsed?: boolean; onToggleCollapse?: () => void }) {
  ...
  return <aside className={`relative z-40 flex h-full flex-col overflow-visible bg-background/92 backdrop-blur-xl transition-[width,transform,opacity] duration-200 ease-out ${asideWidth}`}>
    <div className={`flex items-center gap-2 py-4 ${isCollapsed ? 'flex-col px-2' : 'px-4'}`}>
      ...
      {onClose ? <button onClick={onClose} className="ml-auto rounded-xl p-2 text-muted-foreground hover:bg-accent hover:text-foreground" aria-label="Close navigation"><X className="h-4 w-4" /></button> : <button onClick={toggleCollapse} className={`${isCollapsed ? '' : 'ml-auto'} rounded-xl p-2 text-muted-foreground hover:bg-accent hover:text-foreground`} aria-label={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>{isCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}</button>}
    </div>
    {!isCollapsed && <div className="px-4 pb-2"><div className="relative"><Search className="pointer-events-none absolute z-10 left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input type="search" aria-label="Search apps" placeholder="Search apps…" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="h-9 w-full rounded-xl border border-input bg-background/55 pl-8 pr-8 text-sm outline-none backdrop-blur-md placeholder:text-muted-foreground focus:ring-1 focus:ring-ring/20 [&::-webkit-search-cancel-button]:hidden" />{searchQuery && <button type="button" onClick={() => setSearchQuery('')} aria-label="Clear search" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl p-2 text-muted-foreground hover:bg-accent hover:text-foreground"><X className="h-4 w-4" /></button>}</div>{searchResults.length > 0 && <div className="surface-card mt-2 max-h-52 space-y-2 overflow-y-auto rounded-xl border p-2 shadow-xl">{searchResults.slice(0, 8).map((app) => { const Icon = iconMap[app.id === 'ai-dragon-arena' ? 'DragonArena' : app.icon] || Wrench; return <NavLink key={app.id} to={app.route} onClick={closeSearchAndSidebar} className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground"><Icon className="h-4 w-4 shrink-0" /><span className="truncate">{app.name}</span></NavLink> })}</div>}</div>}
    <nav className={`scrollbar-hide flex-1 px-4 py-2 ${isCollapsed ? 'overflow-visible' : 'overflow-y-auto overflow-x-hidden'}`}>
      {!isCollapsed && <h3 className="mb-2 px-4 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">System</h3>}
      <div className="space-y-2">{coreItems.map((item) => { const Icon = item.icon; return <NavLink key={item.id} to={item.path} onClick={onClose} title={isCollapsed ? item.label : undefined} className={collapsedLinkClass(isCoreActive(item.path))}><Icon className="h-4 w-4 shrink-0" />{isCollapsed ? <span className={collapsedLabelClass}>{item.label}</span> : <span className="truncate">{item.label}</span>}</NavLink> })}</div>
      {(sidebarCategories.length > 0 || sidebarApps.length > 0) && <div className="mt-4">{!isCollapsed && <div className="mb-2 flex items-center justify-between px-4"><h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Workspace</h3><NavLink to="/workspace" onClick={onClose} className="text-xs text-muted-foreground hover:text-foreground">Customize</NavLink></div>}<div className="space-y-2">{sidebarCategories.map((category) => { const Icon = iconMap[category.icon] || Wrench; const path = `/category/${category.id}`; return <NavLink key={category.id} to={path} onClick={onClose} title={isCollapsed ? category.name : undefined} className={collapsedLinkClass(location.pathname === path)}><Icon className="h-4 w-4 shrink-0" />{isCollapsed ? <span className={collapsedLabelClass}>{category.name}</span> : <span className="truncate">{category.name}</span>}</NavLink> })}</div>{sidebarApps.length > 0 && <div className={`${sidebarCategories.length > 0 ? 'mt-2 ' : ''}space-y-0.5`}>{sidebarApps.map((app) => { const Icon = iconMap[app.id === 'ai-dragon-arena' ? 'DragonArena' : app.icon] || Wrench; return <NavLink key={app.id} to={app.route} onClick={onClose} title={isCollapsed ? app.name : undefined} className={collapsedLinkClass(location.pathname === app.route)}><Icon className="h-4 w-4 shrink-0" />{isCollapsed ? <span className={collapsedLabelClass}>{app.name}</span> : <span className="truncate">{app.name}</span>}</NavLink> })}</div>}</div>}
    </nav>
    {!isCollapsed && <div className="px-4 pb-2"><SidebarWeather collapsed={false} /></div>}
    <div ref={accountMenuRef} className="relative border-t border-border p-4">{user && <>{accountOpen && <div className={`absolute z-[80] w-56 rounded-xl border border-border/70 bg-background p-2 shadow-xl ${isCollapsed ? 'bottom-0 left-[calc(100%+0.5rem)]' : 'bottom-[calc(100%-0.25rem)] left-3 right-3 w-auto'}`}><div className="px-2 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Appearance</div><div className="grid grid-cols-3 gap-2 px-2 pb-2">{(['light','dark','system'] as const).map((mode) => <button key={mode} type="button" onClick={() => { localStorage.setItem('appforge-theme', JSON.stringify({ mode })); const dark = mode === 'dark' || (mode === 'system' && matchMedia('(prefers-color-scheme: dark)').matches); document.documentElement.classList.toggle('dark', dark); document.documentElement.style.colorScheme = dark ? 'dark' : 'light'; setAccountOpen(false) }} className="rounded-xl px-2 py-2 text-xs capitalize text-muted-foreground hover:bg-accent hover:text-foreground">{mode}</button>)}</div><NavLink to="/settings" onClick={() => { setAccountOpen(false); onClose?.() }} className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground"><Settings className="h-4 w-4" /> Settings</NavLink><a href="https://github.com/dracorisz/appforge/issues" target="_blank" rel="noreferrer" onClick={() => setAccountOpen(false)} className="flex items-center gap-2 rounded-xl px-2 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground"><HeartHandshake className="h-4 w-4" /> Support</a><button onClick={() => void handleSignOut()} disabled={signingOut} className="flex w-full items-center gap-2 rounded-xl px-2 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground disabled:opacity-60"><LogOut className="h-4 w-4" />{signingOut ? 'Signing out…' : 'Sign out'}</button></div>}<button type="button" onClick={() => setAccountOpen((value) => !value)} className={`flex w-full items-center gap-2 rounded-xl border border-border/60 bg-background/35 p-2 text-left hover:border-foreground/15 ${isCollapsed ? 'justify-center' : ''}`} title={isCollapsed ? String(displayName) : undefined} aria-expanded={accountOpen} aria-haspopup="menu"><div className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-border/70 bg-accent text-xs font-semibold text-foreground">{avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" referrerPolicy="no-referrer" /> : initial}</div>{!isCollapsed && <><div className="min-w-0 flex-1"><div className="truncate text-xs font-medium text-foreground">{String(displayName)}</div>{user.email && <div className="truncate text-xs text-muted-foreground">{user.email}</div>}</div><ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${accountOpen ? 'rotate-180' : ''}`} /></>}</button></>}{signOutError && !isCollapsed && <div role="alert" className="mt-2 rounded-xl border border-destructive/25 bg-destructive/5 px-2 py-2 text-xs leading-4 text-destructive">{signOutError}</div>}</div>
  </aside>
}

export function MobileHeader({ onOpen, triggerRef }: { onOpen: () => void; triggerRef?: React.Ref<HTMLButtonElement> }) {
  return <header className="flex h-14 items-center justify-between border-b border-border bg-background/90 px-4 backdrop-blur-xl lg:hidden"><button ref={triggerRef} onClick={onOpen} className="rounded-xl p-2 text-muted-foreground hover:bg-accent hover:text-foreground" aria-label="Open navigation"><Sliders className="h-4 w-4" /></button><NavLink to="/" className="flex items-center gap-2"><img src="/favicon.svg" alt="AppForge" className="h-8 w-8" /><span className="text-sm font-semibold text-foreground">AppForge</span></NavLink></header>
}
