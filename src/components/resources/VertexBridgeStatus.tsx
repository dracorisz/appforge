    ...
    <Card className="p-4">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-xl text-xs leading-5 text-muted-foreground">Server status only. Google project, Workload Identity, service-account and bucket identifiers remain deployment configuration and are never profile fields.</p>
        </div>
      ...
      <div className="mt-4 rounded-xl border border-border/60 bg-background/35 p-4 text-xs leading-5 text-muted-foreground">
        <div className="flex items-center gap-2 font-medium text-foreground">
          <ShieldCheck className="h-4 w-4" /> Keyless server identity
        </div>
        <p className="mt-2">Vercel OIDC → Google Workload Identity Federation → short-lived bridge identity → private Cloud Run. No downloadable Google service-account key is required.</p>
        {status?.model && (
          <p className="mt-2">
            Configured image policy: <span className="font-medium text-foreground">{status.model}</span>
          </p>
        )}
