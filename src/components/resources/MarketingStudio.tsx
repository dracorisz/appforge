    ...
    <div className="w-full">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
        ...
        {selectedDemo ? <main className="grid gap-5">
          <section className="surface-card rounded-xl border p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-semibold">{selectedDemo.title}</h2><p className="mt-1 text-xs text-muted-foreground">{selectedDemo.buildFingerprint}</p></div><label className="grid gap-1 text-xs font-medium text-muted-foreground">Recording status<select value={selectedDemo.status} onChange={(event) => updateDemo(selectedDemo.id, { status: event.target.value as DemoStatus })} className="h-10 rounded-xl border bg-background px-3 text-sm text-foreground"><option value="planned">Planned</option><option value="recorded">Recorded</option><option value="edited">Edited</option><option value="published">Published</option></select></label></div>
          ...
          <section className="grid gap-4">
            {selectedPublications.map((record) => <article key={record.id} className="surface-card rounded-xl border p-5 sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">{record.channel} · {record.format}</div><h3 className="mt-2 font-semibold">{record.title}</h3></div><select value={record.status} onChange={(event) => updatePublication(record.id, { status: event.target.value as PublicationStatus })} className="h-10 rounded-xl border bg-background px-3 text-sm"><option value="draft">Draft</option><option value="ready">Ready</option><option value="publishing">Publishing</option><option value="published">Published</option><option value="failed">Failed</option></select></div>
