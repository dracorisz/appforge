        ...
        {selectedDemo ? <main className="grid gap-4">
          ...
          <section className="grid gap-4">
            {selectedPublications.map((record) => <article key={record.id} className="surface-card rounded-xl border p-4 sm:p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void copy(`${record.title}\n\n${record.description}\n\nTags: ${record.tags.join(', ')}`)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border px-4 text-xs font-semibold hover:bg-accent"><Clipboard className="h-3.5 w-3.5" /> Copy publication package</button><button type="button" disabled title="Enable only after Google approves the narrow delegated YouTube upload scope" className="inline-flex min-h-10 cursor-not-allowed items-center gap-2 rounded-xl border px-4 text-xs font-semibold opacity-50"><Send className="h-3.5 w-3.5" /> Upload via YouTube OAuth · approval required</button></div>
            </article>)}
