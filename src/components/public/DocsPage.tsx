...
const InlineMarkdown = ({ text }: { text: string }) => (
  <>
    {inlineParts(text).map((part, index) => {
      if (part.type === "code")
        return (
          <code key={index} className="rounded-xl bg-muted px-2 py-2 font-mono text-sm">
            {part.value}
          </code>
        );
      if (part.type === "strong")
      ...
      if (part.type === "link") {
        ...
        return external ? (
          <a key={index} href={href} target="_blank" rel="noopener noreferrer" className="font-medium text-foreground underline underline-offset-4">
            {part.value}
          </a>
        ) : (
          <Link key={index} to={href} className="font-medium text-foreground underline underline-offset-4">
            {part.value}
          </Link>
        );
      }
...
function MarkdownDocument({ body }: { body: string }) {
  ...
  const flushParagraph = () => {
    ...
    if (text)
      nodes.push(
        <p key={`p-${nodes.length}`} className="text-sm text-muted-foreground">
  ...
  const flushList = () => {
    ...
    nodes.push(
      <Tag key={`l-${nodes.length}`} className={`${list.ordered ? "list-decimal" : "list-disc"} space-y-2 pl-8 text-sm text-muted-foreground`}>
  ...
  const flushCode = () => {
    if (!code) return;
    nodes.push(
      <pre key={`c-${nodes.length}`} className="overflow-x-auto rounded-xl border border-border bg-background/70 p-4 text-sm">
        <code className="font-mono" data-language={codeLanguage || undefined}>
          {code.join("\n")}
        </code>
      </pre>,
    );
          ...
          <thead className="bg-muted/40 text-foreground">
            <tr>
              {table.headers.map((cell, index) => (
                <th key={index} className="border-b border-border p-4 font-semibold">
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (heading) {
      ...
      if (level === 1)
        nodes.push(
          <h1 key={`h-${nodes.length}`} className="text-lg font-semibold tracking-[-0.04em]">
            <InlineMarkdown text={text} />
          </h1>,
        );
      else if (level === 2)
        nodes.push(
          <h2 key={`h-${nodes.length}`} className="pt-4 text-lg font-semibold tracking-[-0.02em]">
            <InlineMarkdown text={text} />
          </h2>,
        );
      else
        nodes.push(
          <h3 key={`h-${nodes.length}`} className="pt-2 text-sm font-semibold">
            <InlineMarkdown text={text} />
          </h3>,
        );
      continue;
    ...
    if (line.startsWith("> ")) {
      ...
      flushList();
      nodes.push(
        <blockquote key={`q-${nodes.length}`} className="rounded-xl border border-border bg-muted/30 p-4 text-sm text-muted-foreground">
      ...
      <main className="mx-auto grid w-full max-w-7xl flex-1 gap-8 px-4 py-8 lg:grid-cols-[240px_minmax(0,1fr)] lg:px-8">
        <aside className="lg:sticky lg:top-24 lg:self-start" aria-label="Documentation navigation">
          <div className="surface-panel p-4">
            <Link to={docsHref("index")} className="flex items-center gap-2 text-sm font-semibold">
              <FileText className="h-4 w-4" /> AppForge Docs
            </Link>
            <nav className="mt-4 space-y-2">
              {navigation.map((item) => {
                const active = normalizeDocsSlug(item.slug) === requestedSlug;
                return (
                  <Link key={item.id} to={docsHref(normalizeDocsSlug(item.slug))} className={`flex items-center justify-between gap-2 rounded-xl px-2 py-2 text-sm ${active ? "bg-accent text-foreground" : "text-muted-foreground hover:bg-accent/40 hover:text-foreground"}`}>
        ...
        <section className="min-w-0">
          ...
          {!loading && !error && !page && (
            <div className="surface-panel p-8 text-center">
              <h1 className="text-lg font-semibold">Documentation page not found</h1>
              <Link to={docsHref("index")} className="mt-4 inline-flex text-sm font-semibold underline">
                Back to docs
              </Link>
            </div>
          )}
          {!loading && page && (
            <article className="surface-card p-4 sm:p-8">
              <MarkdownDocument body={page.body || page.summary || ""} />
            </article>
          )}
        </section>
      </main>
