  ...
  return (
    <div className="mx-auto grid w-full max-w-[1500px] gap-4 p-4 sm:p-4 xl:grid-cols-[420px_minmax(0,1fr)]">
      <aside className="surface-card self-start rounded-xl border p-4 xl:sticky xl:top-4 xl:max-h-[calc(100dvh-2rem)] xl:overflow-auto">
        ...
        <div className="mt-4"><div className="mb-2 text-sm font-semibold">Add section</div><div className="grid grid-cols-3 gap-2">{(['hero', 'features', 'gallery', 'cta', 'faq', 'footer'] as SectionKind[]).map((kind) => <button key={kind} type="button" onClick={() => add(kind)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border px-2 text-sm font-medium capitalize hover:bg-accent"><Plus className="h-3.5 w-3.5" /> {kind}</button>)}</div></div>
        ...
        <div className="mt-4 grid gap-4">
          {project.sections.map((section, index) => (
            <article key={section.id} className="rounded-xl border bg-background/70 p-4">
              ...
              <div className="mt-4 grid gap-2"><input value={section.title} onChange={(event) => updateSection(section.id, { title: event.target.value })} className="h-10 rounded-xl border bg-background px-4 text-sm font-medium" aria-label={`${section.kind} title`} /><textarea value={section.body} onChange={(event) => updateSection(section.id, { body: event.target.value })} rows={3} className="rounded-xl border bg-background p-4 text-sm" aria-label={`${section.kind} body`} />{(section.kind === 'hero' || section.kind === 'cta' || section.kind === 'footer') && <div className="grid grid-cols-2 gap-2"><input value={section.linkLabel || ''} onChange={(event) => updateSection(section.id, { linkLabel: event.target.value })} placeholder="Button label" className="h-10 rounded-xl border bg-background px-4 text-sm" /><input value={section.linkUrl || ''} onChange={(event) => updateSection(section.id, { linkUrl: event.target.value })} placeholder="https://… or #section" className="h-10 rounded-xl border bg-background px-4 text-sm" /></div>}{section.kind === 'gallery' && <label className="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-xl border text-sm font-medium hover:bg-accent"><ImageIcon className="h-3.5 w-3.5" /> Local image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/avif" className="sr-only" onChange={(event) => { uploadImage(section.id, event.target.files?.[0]); event.currentTarget.value = '' }} /></label>}</div>
            </article>
          ))}
        </div>
