...
const safeImageData = (value: unknown) => (typeof value === "string" && value.length <= MAX_IMAGE_DATA_CHARS && /^data:image\/(?:png|jpe?g|webp|gif|avif);base64,[a-z0-9+/=\r\n]+$/i.test(value) ? value : undefined);
...
const buildHtml = (project: LandingProject) => {
  ...
  const sectionHtml = project.sections
    .map((section) => {
...
export default function LandingBuilder() {
  const [project, setProject] = React.useState<LandingProject>(() => {
    try {
      ...
      return safeProject(parsed) || defaultProject();
    } catch {
  ...
  React.useEffect(() => {
    try {
      ...
      setMessage("This project is too large for browser storage. Export project JSON to keep a portable copy.");
    }
  }, [project]);
  React.useEffect(() => {
    try {
      ...
      setMessage("Saved-project storage is full. Export project JSON before clearing browser data.");
    }
  }, [savedProjects]);
  ...
  const uploadImage = (sectionId: string, file?: File) => {
    ...
    if (!["image/png", "image/jpeg", "image/webp", "image/gif", "image/avif"].includes(file.type)) {
      setMessage("Choose a PNG, JPEG, WebP, GIF, or AVIF image.");
      return;
    }
    if (file.size > MAX_IMAGE_BYTES) {
      setMessage("Choose an image under 5 MB so the project remains portable.");
      return;
    }
  ...
  const importProject = (file?: File) => {
    ...
    if (file.size > MAX_PROJECT_BYTES) {
      setMessage("Project JSON must be under 2 MB.");
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
  ...
  return (
    <div className="mx-auto grid w-full max-w-[1500px] gap-4 p-4 sm:p-4 xl:grid-cols-[420px_minmax(0,1fr)]">
        ...
        {Object.keys(savedProjects).length > 0 && (
          <label className="mt-4 grid gap-2 text-sm font-medium text-muted-foreground">
            ...
            <Select
              ...
              className="h-9 rounded-xl border bg-background px-4 text-sm text-foreground"
            >
      ...
      <section className="min-w-0">
        ...
        <div className="overflow-auto rounded-xl border bg-muted/20 p-4 sm:p-4">
          <div className="mx-auto min-h-[720px] overflow-hidden rounded-xl border transition-[width] duration-200" style={{ width: previewWidth, maxWidth: "100%", background: palette.bg, color: palette.text }}>
            <main className="grid gap-4 p-4 sm:p-4">
              {project.sections.map((section) => {
                const href = safeHref(section.linkUrl);
                return (
                  <section key={section.id} id={`preview-${sectionDomId(section)}`} className="rounded-xl border p-4 sm:p-8" style={{ background: palette.card, borderColor: palette.border }}>
                    <div className="text-sm font-semibold uppercase tracking-[0.16em]" style={{ color: palette.muted }}>
                      {section.kind}
                    </div>
                    <h2 className={`${section.kind === "hero" ? "text-lg sm:text-5xl" : "text-lg sm:text-lg"} mt-2 font-semibold tracking-tight`}>{section.title}</h2>
                    <p className="mt-4 whitespace-pre-wrap text-sm sm:text-sm" style={{ color: palette.muted }}>
                      {section.body}
                    </p>
                    {safeImageData(section.image) && <img src={section.image} alt="Local project visual" className="mt-4 max-h-96 w-full rounded-xl border object-contain" style={{ borderColor: palette.border }} />}
                    {section.linkLabel && href && (
                      <a href={href} onClick={(event) => event.preventDefault()} className="mt-4 inline-flex rounded-xl px-4 py-2 text-sm font-semibold text-inverse" style={{ background: project.accent }}>
                        {section.linkLabel}
                      </a>
                    )}
                  </section>
