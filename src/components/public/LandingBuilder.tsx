...
const safeProject = (value: unknown): LandingProject | null => {
  ...
  const sections = project.sections.filter((section): section is LandingSection => Boolean(section && typeof section.id === 'string' && typeof section.kind === 'string' && typeof section.title === 'string' && typeof section.body === 'string'))
  if (!sections.length) return null
  return {
    version: 1,
    name: typeof project.name === 'string' ? project.name : 'Imported landing page',
    preset: project.preset === 'paper' || project.preset === 'midnight' ? project.preset : 'slate',
    accent: typeof project.accent === 'string' ? project.accent : '#2563eb',
    sections,
...
const buildHtml = (project: LandingProject) => {
  ...
  const sectionHtml = project.sections.map((section) => {
    const image = section.image ? `<img src="${section.image}" alt="" style="max-width:100%;border-radius:18px;border:1px solid ${palette.border};margin-top:20px">` : ''
    const link = section.linkLabel && section.linkUrl ? `<a href="${escapeHtml(section.linkUrl)}" class="button">${escapeHtml(section.linkLabel)}</a>` : ''
    return `<section id="${escapeHtml(section.kind)}"><div class="card"><p class="eyebrow">${escapeHtml(section.kind)}</p><h2>${escapeHtml(section.title)}</h2><p>${escapeHtml(section.body).replaceAll('\n', '<br>')}</p>${image}${link}</div></section>`
  }).join('\n')
...
export default function LandingBuilder() {
  ...
  const [savedProjects, setSavedProjects] = React.useState<Record<string, LandingProject>>(() => {
    try { const parsed = JSON.parse(localStorage.getItem(PROJECTS_KEY) || '{}'); return parsed && typeof parsed === 'object' ? parsed : {} } catch { return {} }
  })
  const [message, setMessage] = React.useState('')

  React.useEffect(() => { localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...project, updatedAt: new Date().toISOString() })) }, [project])
  React.useEffect(() => { localStorage.setItem(PROJECTS_KEY, JSON.stringify(savedProjects)) }, [savedProjects])
  ...
  const uploadImage = (sectionId: string, file?: File) => {
    ...
    reader.onload = () => { if (typeof reader.result === 'string') updateSection(sectionId, { image: reader.result }) }
      ...
      <section className="min-w-0">
        ...
        <div className="overflow-auto rounded-2xl border bg-muted/20 p-3 sm:p-5"><div className="mx-auto min-h-[720px] overflow-hidden rounded-2xl border shadow-2xl transition-[width] duration-200" style={{ width: previewWidth, maxWidth: '100%', background: palette.bg, color: palette.text }}>
          <main className="grid gap-4 p-4 sm:p-6">
            {project.sections.map((section) => <section key={section.id} id={`preview-${section.kind}`} className="rounded-2xl border p-6 sm:p-8" style={{ background: palette.card, borderColor: palette.border }}><div className="text-[10px] font-semibold uppercase tracking-[0.16em]" style={{ color: palette.muted }}>{section.kind}</div><h2 className={`${section.kind === 'hero' ? 'text-3xl sm:text-5xl' : 'text-2xl sm:text-3xl'} mt-2 font-semibold tracking-tight`}>{section.title}</h2><p className="mt-3 whitespace-pre-wrap text-sm leading-6 sm:text-base" style={{ color: palette.muted }}>{section.body}</p>{section.image && <img src={section.image} alt="Local project visual" className="mt-5 max-h-96 w-full rounded-xl border object-contain" style={{ borderColor: palette.border }} />}{section.linkLabel && section.linkUrl && <a href={section.linkUrl} onClick={(event) => event.preventDefault()} className="mt-5 inline-flex rounded-xl px-4 py-2.5 text-sm font-semibold text-white" style={{ background: project.accent }}>{section.linkLabel}</a>}</section>)}
          </main>
        </div></div>
      </section>
    </div>
