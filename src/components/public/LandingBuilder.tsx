...
export default function LandingBuilder() {
  ...
  return (
    <div className="mx-auto grid w-full max-w-[1500px] gap-4 p-4 sm:p-4 xl:grid-cols-[420px_minmax(0,1fr)]">
      <aside className="surface-card self-start rounded-xl border p-4 xl:sticky xl:top-4 xl:max-h-[calc(100dvh-2rem)] xl:overflow-auto">
        ...
        <div className="mt-4 grid gap-4">
          <label className="grid gap-2 text-sm font-medium">Project name<input value={project.name} onChange={(event) => setProject((current) => ({ ...current, name: event.target.value }))} className="h-11 rounded-xl border bg-background px-4" /></label>
          <div className="grid grid-cols-2 gap-4"><label className="grid gap-2 text-sm font-medium">Appearance<select value={project.preset} onChange={(event) => setProject((current) => ({ ...current, preset: event.target.value as LandingProject['preset'] }))} className="h-11 rounded-xl border bg-background px-4"><option value="slate">Slate</option><option value="paper">Paper</option><option value="midnight">Midnight</option></select></label><label className="grid gap-2 text-sm font-medium">Accent<input type="color" value={project.accent} onChange={(event) => setProject((current) => ({ ...current, accent: event.target.value }))} className="h-11 w-full rounded-xl border bg-background p-2" /></label></div>
        </div>

        <div className="mt-4"><div className="mb-2 text-sm font-semibold">Add section</div><div className="grid grid-cols-3 gap-2">{(['hero', 'features', 'gallery', 'cta', 'faq', 'footer'] as SectionKind[]).map((kind) => <button key={kind} type="button" onClick={() => add(kind)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border px-2 text-xs font-medium capitalize hover:bg-accent"><Plus className="h-3.5 w-3.5" /> {kind}</button>)}</div></div>

        <div className="mt-4 grid gap-4">
          {project.sections.map((section, index) => (
            <article key={section.id} className="rounded-xl border bg-background/70 p-4">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{index + 1}. {section.kind}</span><div className="flex gap-2"><button type="button" onClick={() => move(index, -1)} disabled={index === 0} className="grid h-8 w-8 place-items-center rounded-xl border disabled:opacity-40" aria-label={`Move ${section.kind} up`}><ArrowUp className="h-3.5 w-3.5" /></button><button type="button" onClick={() => move(index, 1)} disabled={index === project.sections.length - 1} className="grid h-8 w-8 place-items-center rounded-xl border disabled:opacity-40" aria-label={`Move ${section.kind} down`}><ArrowDown className="h-3.5 w-3.5" /></button><button type="button" onClick={() => remove(section.id)} className="grid h-8 w-8 place-items-center rounded-xl border hover:bg-destructive/10" aria-label={`Remove ${section.kind}`}><Trash2 className="h-3.5 w-3.5" /></button></div></div>
              <div className="mt-4 grid gap-2"><input value={section.title} onChange={(event) => updateSection(section.id, { title: event.target.value })} className="h-10 rounded-xl border bg-background px-4 text-sm font-medium" aria-label={`${section.kind} title`} /><textarea value={section.body} onChange={(event) => updateSection(section.id, { body: event.target.value })} rows={3} className="rounded-xl border bg-background p-4 text-sm" aria-label={`${section.kind} body`} />{(section.kind === 'hero' || section.kind === 'cta' || section.kind === 'footer') && <div className="grid grid-cols-2 gap-2"><input value={section.linkLabel || ''} onChange={(event) => updateSection(section.id, { linkLabel: event.target.value })} placeholder="Button label" className="h-10 rounded-xl border bg-background px-4 text-sm" /><input value={section.linkUrl || ''} onChange={(event) => updateSection(section.id, { linkUrl: event.target.value })} placeholder="https://… or #section" className="h-10 rounded-xl border bg-background px-4 text-sm" /></div>}{section.kind === 'gallery' && <label className="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-xl border text-xs font-medium hover:bg-accent"><ImageIcon className="h-3.5 w-3.5" /> Local image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/avif" className="sr-only" onChange={(event) => { uploadImage(section.id, event.target.files?.[0]); event.currentTarget.value = '' }} /></label>}</div>
            </article>
          ))}
        </div>

        <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={saveNamed} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border text-sm font-semibold hover:bg-accent"><Save className="h-4 w-4" /> Save local</button><button type="button" onClick={() => downloadText(JSON.stringify({ ...project, updatedAt: new Date().toISOString() }, null, 2), `${project.name.replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'landing'}.appforge.json`, 'application/json')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border text-sm font-semibold hover:bg-accent"><Download className="h-4 w-4" /> Project JSON</button><label className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl border text-sm font-semibold hover:bg-accent"><Upload className="h-4 w-4" /> Import JSON<input type="file" accept="application/json,.json" className="sr-only" onChange={(event) => { importProject(event.target.files?.[0]); event.currentTarget.value = '' }} /></label><button type="button" onClick={() => downloadText(buildHtml(project), `${project.name.replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'landing'}.html`, 'text/html;charset=utf-8')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground"><Download className="h-4 w-4" /> Static HTML</button></div>

        {Object.keys(savedProjects).length > 0 && <label className="mt-4 grid gap-2 text-xs font-medium text-muted-foreground">Saved browser projects<select defaultValue="" onChange={(event) => { const saved = savedProjects[event.target.value]; if (saved) setProject(saved); event.currentTarget.value = '' }} className="h-10 rounded-xl border bg-background px-4 text-sm text-foreground"><option value="" disabled>Load a saved project…</option>{Object.keys(savedProjects).sort().map((name) => <option key={name} value={name}>{name}</option>)}</select></label>}
        <div aria-live="polite" className="mt-4 min-h-5 text-xs text-muted-foreground">{message}</div>
      </aside>

      <section className="min-w-0">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-4"><div className="text-sm font-semibold">Responsive preview</div><div className="flex rounded-xl border bg-background p-2">{(['phone', 'tablet', 'desktop'] as const).map((item) => <button key={item} type="button" onClick={() => setViewport(item)} className={`rounded-xl px-4 py-2 text-xs font-medium capitalize ${viewport === item ? 'bg-accent text-foreground' : 'text-muted-foreground'}`}>{item}</button>)}</div></div>
        <div className="overflow-auto rounded-xl border bg-muted/20 p-4 sm:p-4"><div className="mx-auto min-h-[720px] overflow-hidden rounded-xl border transition-[width] duration-200" style={{ width: previewWidth, maxWidth: '100%', background: palette.bg, color: palette.text }}>
          <main className="grid gap-4 p-4 sm:p-4">
            {project.sections.map((section) => {
              const href = safeHref(section.linkUrl)
              return <section key={section.id} id={`preview-${sectionDomId(section)}`} className="rounded-xl border p-4 sm:p-8" style={{ background: palette.card, borderColor: palette.border }}><div className="text-xs font-semibold uppercase tracking-[0.16em]" style={{ color: palette.muted }}>{section.kind}</div><h2 className={`${section.kind === 'hero' ? 'text-lg sm:text-5xl' : 'text-lg sm:text-lg'} mt-2 font-semibold tracking-tight`}>{section.title}</h2><p className="mt-4 whitespace-pre-wrap text-sm leading-6 sm:text-sm" style={{ color: palette.muted }}>{section.body}</p>{safeImageData(section.image) && <img src={section.image} alt="Local project visual" className="mt-4 max-h-96 w-full rounded-xl border object-contain" style={{ borderColor: palette.border }} />}{section.linkLabel && href && <a href={href} onClick={(event) => event.preventDefault()} className="mt-4 inline-flex rounded-xl px-4 py-2 text-sm font-semibold text-white" style={{ background: project.accent }}>{section.linkLabel}</a>}</section>
            })}
          </main>
        </div></div>
      </section>
