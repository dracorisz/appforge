...
export default function SvgIconsBrowser() {
  ...
  React.useEffect(() => { localStorage.setItem(RECENTS_KEY, JSON.stringify(recents)) }, [recents])
  ...
  const copyImport = async (icon: IconRecord) => {
    const snippet = `import { ${icon.name} } from 'react-icons/${icon.pack}'`
    await navigator.clipboard.writeText(snippet)
    remember(icon)
    setMessage('React import copied.')
  }

  const copyJsx = async (icon: IconRecord) => {
    await navigator.clipboard.writeText(`<${icon.name} size={24} />`)
    remember(icon)
    setMessage('JSX usage copied.')
  }
  ...
  const copySvg = async (icon: IconRecord) => {
    await navigator.clipboard.writeText(await getSvgMarkup(icon))
    remember(icon)
    setMessage('Rendered SVG copied. Keep the upstream icon-pack license/attribution requirements.')
  }
  ...
  const remember = (icon: IconRecord) => {
    ...
    <div className="mx-auto w-full max-w-7xl p-4 sm:p-6">
      <section className="surface-card rounded-2xl border p-4 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground">Lazy-loaded catalog</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight">SVG Icons</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Browse every icon pack exposed by react-icons without loading every pack up front. Packs are loaded only when selected; favorites and recent icons stay in this browser.</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[minmax(220px,0.45fr)_minmax(260px,1fr)_180px]">
          ...
          <label className="grid gap-1.5 text-sm font-medium">Search component name<span className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => { setQuery(event.target.value); setVisible(120) }} placeholder="Search e.g. arrow, github, cloud…" className="h-11 w-full rounded-xl border bg-background pl-10 pr-3" /></span></label>
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6" aria-busy={loading}>
        {filtered.slice(0, visible).map((icon) => {
          ...
          return (
            <article key={id} className="surface-card group flex min-h-44 flex-col rounded-2xl border p-3">
              <div className="flex min-h-20 flex-1 items-center justify-center rounded-xl bg-background/60"><icon.Component size={size} aria-hidden="true" /></div>
              <div className="mt-3 min-w-0"><div className="truncate text-sm font-semibold" title={icon.name}>{icon.name}</div><div className="text-xs text-muted-foreground">react-icons/{icon.pack}</div></div>
              <div className="mt-3 grid grid-cols-4 gap-1">
                <button type="button" onClick={() => void copyImport(icon)} className="grid min-h-9 place-items-center rounded-lg border hover:bg-accent" title="Copy import"><Copy className="h-3.5 w-3.5" /></button>
                <button type="button" onClick={() => void copyJsx(icon)} className="grid min-h-9 place-items-center rounded-lg border text-xs font-semibold hover:bg-accent" title="Copy JSX">JSX</button>
                <button type="button" onClick={() => void copySvg(icon)} className="grid min-h-9 place-items-center rounded-lg border text-xs font-semibold hover:bg-accent" title="Copy rendered SVG">SVG</button>
                <button type="button" onClick={() => toggleFavorite(icon)} className="grid min-h-9 place-items-center rounded-lg border hover:bg-accent" title={favorite ? 'Remove favorite' : 'Favorite'}>{favorite ? <Heart className="h-3.5 w-3.5 fill-current" /> : <Star className="h-3.5 w-3.5" />}</button>
              </div>
              <button type="button" onClick={() => void downloadSvg(icon)} className="mt-1 inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg border text-xs font-medium hover:bg-accent"><Download className="h-3.5 w-3.5" /> Download SVG</button>
            </article>
          )
        })}
      </section>
