    ...
    <div className="mx-auto w-full max-w-7xl p-4 sm:p-4">
      <section className="surface-card rounded-xl border p-4 sm:p-4">
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(220px,0.45fr)_minmax(260px,1fr)_180px]">
          <label className="grid gap-2 text-sm font-medium">Icon pack<select value={pack} onChange={(event) => setPack(event.target.value as PackKey)} className="h-11 rounded-xl border bg-background px-4">{Object.entries(PACKS).map(([key, item]) => <option key={key} value={key}>{item.label}</option>)}</select></label>
          <label className="grid gap-2 text-sm font-medium">Search component name<span className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={query} maxLength={100} onChange={(event) => { setQuery(event.target.value); setVisible(120) }} placeholder="Search e.g. arrow, github, cloud…" className="h-11 w-full rounded-xl border bg-background pl-8 pr-4 hover:cursor-pointer  " /></span></label>
          <label className="grid gap-2 text-sm font-medium grid-cols-2">Preview size <span className="font-normal text-muted-foreground">{size}px</span><input type="range" min="18" max="64" className="col-span-2 -mt-8" value={size} onChange={(event) => setSize(Number(event.target.value))} /></label>
        </div>
        <div className="flex justify-between">
        <div className="flex mt-4 flex-wrap gap-2">{(['all', 'favorites', 'recent'] as const).map((value) => <button key={value} type="button" onClick={() => { setScope(value); setVisible(120) }} className={`rounded-xl border px-4 py-2 text-xs font-medium hover:cursor-pointer capitalize ${scope === value ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>{value === 'favorites' ? `Favorites (${favorites.length})` : value === 'recent' ? `Recent (${recents.length})` : 'All icons'}</button>)}</div>
        <div aria-live="polite" className="min-h-8 px-2 pt-4 text-xs text-muted-foreground">{loading ? `Loading ${PACKS[pack].label}…` : message}</div>
        </div>
      </section>
      ...
      <section className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6" aria-busy={loading}>
        {filtered.slice(0, visible).map((icon) => {
          ...
          return <article key={id} className="surface-card group flex min-h-44 flex-col rounded-xl border p-4"><div className="flex min-h-20 flex-1 items-center justify-center rounded-xl bg-background/60"><icon.Component size={size} aria-hidden="true" /></div><div className="mt-4 min-w-0"><div className="truncate text-sm font-semibold" title={icon.name}>{icon.name}</div><div className="text-xs text-muted-foreground">react-icons/{icon.pack}</div></div><div className="mt-4 grid grid-cols-4 gap-2"><button type="button" onClick={() => void copyImport(icon)} className="grid min-h-9 place-items-center rounded-xl border hover:bg-accent" title="Copy import"><Copy className="h-3.5 w-3.5" /></button><button type="button" onClick={() => void copyJsx(icon)} className="grid min-h-9 place-items-center rounded-xl border text-xs font-semibold hover:bg-accent" title="Copy JSX">JSX</button><button type="button" onClick={() => void copySvg(icon)} className="grid min-h-9 place-items-center rounded-xl border text-xs font-semibold hover:bg-accent" title="Copy rendered SVG">SVG</button><button type="button" onClick={() => toggleFavorite(icon)} className="grid min-h-9 place-items-center rounded-xl border hover:bg-accent" title={favorite ? 'Remove favorite' : 'Favorite'}>{favorite ? <Heart className="h-3.5 w-3.5 fill-current" /> : <Star className="h-3.5 w-3.5" />}</button></div><button type="button" onClick={() => void downloadSvg(icon)} className="mt-2 inline-flex min-h-9 items-center justify-center gap-2 rounded-xl border text-xs font-medium hover:bg-accent"><Download className="h-3.5 w-3.5" /> Download SVG</button></article>
        })}
      </section>
