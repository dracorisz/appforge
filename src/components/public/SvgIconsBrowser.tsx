    ...
    <div className="mx-auto w-full max-w-7xl p-4 sm:p-6">
      ...
      <section className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6" aria-busy={loading}>
        {filtered.slice(0, visible).map((icon) => {
          ...
          return <article key={id} className="surface-card group flex min-h-44 flex-col rounded-xl border p-3"><div className="flex min-h-20 flex-1 items-center justify-center rounded-xl bg-background/60"><icon.Component size={size} aria-hidden="true" /></div><div className="mt-3 min-w-0"><div className="truncate text-sm font-semibold" title={icon.name}>{icon.name}</div><div className="text-xs text-muted-foreground">react-icons/{icon.pack}</div></div><div className="mt-3 grid grid-cols-4 gap-1"><button type="button" onClick={() => void copyImport(icon)} className="grid min-h-9 place-items-center rounded-xl border hover:bg-accent" title="Copy import"><Copy className="h-3.5 w-3.5" /></button><button type="button" onClick={() => void copyJsx(icon)} className="grid min-h-9 place-items-center rounded-xl border text-xs font-semibold hover:bg-accent" title="Copy JSX">JSX</button><button type="button" onClick={() => void copySvg(icon)} className="grid min-h-9 place-items-center rounded-xl border text-xs font-semibold hover:bg-accent" title="Copy rendered SVG">SVG</button><button type="button" onClick={() => toggleFavorite(icon)} className="grid min-h-9 place-items-center rounded-xl border hover:bg-accent" title={favorite ? 'Remove favorite' : 'Favorite'}>{favorite ? <Heart className="h-3.5 w-3.5 fill-current" /> : <Star className="h-3.5 w-3.5" />}</button></div><button type="button" onClick={() => void downloadSvg(icon)} className="mt-1 inline-flex min-h-9 items-center justify-center gap-1.5 rounded-xl border text-xs font-medium hover:bg-accent"><Download className="h-3.5 w-3.5" /> Download SVG</button></article>
        })}
      </section>

      {!loading && filtered.length === 0 && <div className="surface-card rounded-xl border p-10 text-center text-sm text-muted-foreground">No icons in this pack match “{query}”.</div>}
