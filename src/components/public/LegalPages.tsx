...
export function PrivacyPolicyPage() {
  return (
    <LegalShell title="Privacy Policy" intro="This policy explains how AppForge handles account information, workspace data, tool inputs, and Google sign-in data while the service is in public beta.">
      ...
      <section><h2>6. Local storage and technical data</h2><p>The application may use browser storage to keep preferences, cached application state, PWA data, and session-related information. Hosting and infrastructure providers may also process routine technical information such as IP addresses, request metadata, timestamps, error details, and device or browser information for security and service operation.</p></section>
...
export function TermsOfServicePage() {
  return (
    <LegalShell title="Terms of Service" intro="These terms set the baseline rules for using the AppForge public beta, including the open-source tools, authenticated workspace, and AI-powered features.">
      ...
      <section><h2>7. Open-source code and third-party services</h2><p>Source code published in the AppForge repository is licensed under the repository's stated open-source license. Hosted service functionality may also depend on third-party platforms that have their own terms, policies, quotas, and availability.</p></section>
      <section><h2>8. Availability and changes</h2><p>Features may be added, changed, restricted, or removed. AppForge may suspend access when necessary for security, abuse prevention, maintenance, legal compliance, or service integrity.</p></section>
      <section><h2>9. Disclaimer and limitation</h2><p>To the extent permitted by applicable law, AppForge is provided on an "as is" and "as available" basis without guarantees that every feature will be uninterrupted, error-free, or suitable for a particular purpose. Nothing in these terms excludes rights or liabilities that cannot legally be excluded.</p></section>
      <section><h2>10. Changes and contact</h2><p>These terms may be updated as the public beta evolves. The date at the top identifies the latest version. Questions can be directed through the contact methods listed in the AppForge GitHub repository.</p></section>
    </LegalShell>
  )
}
