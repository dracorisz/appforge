  ...
  return (
    <LegalShell title="Privacy Policy" intro="This policy explains how AppForge handles account information, workspace data, tool inputs, Google sign-in data, cookies, and browser storage while the service is in public beta.">
      <section>
        ...
        <p>
          When you sign in with Google through Supabase Auth, AppForge receives basic identity information needed to create and maintain your account, such as your Google account identifier, name, email address, and profile image when provided by Google. AppForge may also store workspace information
          you choose to create, including profile fields, favorites, recent apps, settings, category overrides, and other account-scoped preferences.
        </p>
      </section>
      <section>
        <h2>2. Google API user data</h2>
        <p>
          AppForge uses Google user data only to authenticate you, identify your account, display basic profile information, and associate your workspace data with your authenticated user ID. AppForge does not request access to Gmail, Drive, Calendar, Contacts, or other Google services as part of
      ...
      <section>
        ...
        <ul>
          <li>Provide sign-in, account, profile, preference, and workspace features.</li>
          <li>Operate requested tools and return their results.</li>
          <li>Maintain security, prevent abuse, diagnose failures, and improve reliability.</li>
          <li>Preserve user-selected settings and cross-device continuity where enabled.</li>
        </ul>
      </section>
      <section>
      ...
      <section>
        <h2>9. Changes</h2>
        <p>This policy may be updated when features, providers, or data practices change. The date at the top of this page identifies the latest published version.</p>
      </section>
      <section>
        <h2>10. Contact</h2>
        <p>For privacy questions, use the contact methods listed in the AppForge GitHub repository. For private security matters, follow the repository's SECURITY.md process rather than opening a public issue.</p>
      </section>
    </LegalShell>
  );
}
