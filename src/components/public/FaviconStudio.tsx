    ...
    <div className="mx-auto grid w-full max-w-6xl gap-4 p-4 sm:p-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <section className="surface-card rounded-xl border p-4 sm:p-4">
        ...
        <div className="grid gap-4">
          ...
          <label className="grid gap-2 text-sm font-medium">Corner radius <span className="text-sm font-normal text-muted-foreground">{radius}%</span><input type="range" min="0" max="50" value={radius} onChange={(event) => setRadius(Number(event.target.value))} /></label>
          <label className="grid gap-2 text-sm font-medium">Glyph size <span className="text-sm font-normal text-muted-foreground">{Math.round(fontScale * 100)}%</span><input type="range" min="0.28" max="0.8" step="0.01" value={fontScale} onChange={(event) => setFontScale(Number(event.target.value))} disabled={Boolean(imageData)} /></label>
      ...
      <section className="grid gap-4">
        <div className="surface-card rounded-xl border p-4 sm:p-4">
          <div className="mb-4 flex items-center justify-between gap-4"><div><h2 className="font-semibold">Live previews</h2><p className="text-sm text-muted-foreground">Actual target sizes, enlarged where needed for inspection.</p></div><ImageIcon className="h-5 w-5 text-muted-foreground" /></div>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
            {PREVIEW_SIZES.map((size) => (
              <button key={size} type="button" onClick={() => void exportPreview(size)} className="group grid min-h-28 place-items-center gap-2 rounded-xl border bg-background p-4 text-sm text-muted-foreground hover:bg-accent" title={`Download ${size}×${size} PNG`}>
