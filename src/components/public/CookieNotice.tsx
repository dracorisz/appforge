...
export function CookieNotice() {
  ...
  return (
    <div className="fixed inset-x-3 bottom-3 z-[100] mx-auto max-w-3xl rounded-xl border border-border/80 bg-background/95 px-4 py-3 backdrop-blur-xl sm:bottom-4 sm:flex sm:items-center sm:gap-4">
      ...
      <button type="button" onClick={dismiss} className="absolute right-2 top-2 inline-flex h-7 w-7 items-center justify-center rounded-xl text-muted-foreground hover:bg-accent hover:text-foreground sm:static sm:shrink-0" aria-label="Dismiss storage notice"><X className="h-4 w-4" /></button>
    </div>
  )
}
