    ...
    <div className="dark flex min-h-dvh flex-col bg-black text-foreground" style={{ colorScheme: 'dark', '--background': '0 0% 0%' } as React.CSSProperties}>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="py-6" aria-label="Filter apps">
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_240px]">
            <label className="relative block">
              ...
              {query && <button type="button" onClick={() => setQuery('')} aria-label="Clear search" className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-xl p-1 text-muted-foreground hover:bg-white/10 hover:text-white"><X className="h-3.5 w-3.5" /></button>}
            </label>
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" aria-live="polite">
          {visible.map((app) => (
            <Link key={app.id} to={app.route} className="group flex min-h-44 flex-col rounded-xl border border-border/70 bg-background/55 p-4 transition-colors hover:border-foreground/20 hover:bg-accent/35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
              {app.coverImage && <img src={app.coverImage} alt="" className="-mx-4 -mt-4 mb-4 h-28 w-[calc(100%+2rem)] rounded-xl object-cover" loading="lazy" />}
        ...
        {!visible.length && <div className="rounded-xl border border-border/70 p-8 text-center text-sm text-muted-foreground">No apps match.</div>}
      </main><PublicFooter />
    </div>
  )
}
