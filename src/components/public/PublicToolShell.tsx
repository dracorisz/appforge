...
export function PublicToolShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold">AppForge</div>
              <div className="truncate text-[11px] text-muted-foreground">Public beta tool</div>
            </div>
          </Link>
      ...
      <footer className="border-t border-border/65 bg-background/75">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-4 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
          <span>AppForge public beta · Scrapper Pro is available without an account.</span>
          <span>Sign in to access the full workspace and synced profile features.</span>
        </div>
      </footer>
    </div>
  )
