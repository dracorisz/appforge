  ...
  return (
    <div className="dark min-h-screen bg-black text-foreground" style={{ colorScheme: 'dark', '--background': '0 0% 0%' } as React.CSSProperties}>
      ...
      <main className="mx-auto w-full max-w-7xl space-y-7 px-4 py-8 sm:px-6 lg:px-8">
        <section className="rounded-xl border border-border bg-background/55 p-5"><div className="flex flex-wrap items-center gap-2 text-xs font-medium text-muted-foreground"><Sparkles className="h-4 w-4" /> Hugging Face powered generation</div><h1 className="mt-2 text-3xl font-semibold tracking-tight">Story Studio × Hugging Face</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Story Studio uses Hugging Face for narrative generation and scene artwork. Generated scenes stay private by default; creators explicitly choose what appears here.</p><div className="mt-4 flex flex-wrap gap-2"><Link to="/" className="inline-flex items-center gap-2 rounded-xl border border-border/70 bg-background/65 px-3 py-2 text-sm font-medium text-foreground hover:bg-accent/55">Open Story Studio <ArrowRight className="h-4 w-4" /></Link><a href="https://huggingface.co/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-border/70 bg-background/65 px-3 py-2 text-sm font-medium text-foreground hover:bg-accent/55"><ExternalLink className="h-4 w-4" /> Hugging Face</a></div></section>
