...
function CarouselCard({ asset, active, onSelect }: { asset: PublicDragonAsset; active: boolean; onSelect: () => void }) {
  ...
  return (
    <button type="button" onClick={onSelect} className={`group relative shrink-0 cursor-pointer overflow-hidden rounded-xl border border-inverse/10 bg-[#080d16] text-left transition-all duration-500 ${active ? 'z-10 w-[78vw] max-w-3xl scale-100 opacity-100 md:w-[58vw]' : 'w-[52vw] max-w-xl scale-[.82] opacity-45 md:w-[34vw]'}`} aria-current={active ? 'true' : undefined}>
      <div className="relative aspect-[16/10] overflow-hidden bg-muted">
        {src ? <img src={src} alt={asset.title || 'Story Studio generated scene'} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.02]" loading="lazy" /> : <div className="flex h-full items-center justify-center text-inverse/35"><ImageIcon className="h-10 w-10" /></div>}
      ...
      {active && <div className="flex items-center justify-between gap-4 border-t border-inverse/10 px-4 py-2">
        <div className="min-w-0 truncate text-sm text-inverse/55">Model: <span className="text-inverse/80">{modelLabel}</span></div>
...
function InfiniteShowcase({ assets }: { assets: PublicDragonAsset[] }) {
  ...
  return (
    <div className="relative overflow-hidden py-8" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={() => setPaused(false)}>
      ...
      {assets.length > 1 && <div className="mt-4 flex items-center justify-center gap-4"><button type="button" onClick={() => setIndex((value) => wrap(value - 1, assets.length))} className="cursor-pointer rounded-xl border border-inverse/10 bg-inverse/5 p-2 text-inverse/75 hover:bg-inverse/10" aria-label="Previous scene"><ArrowLeft className="h-4 w-4" /></button><span className="min-w-16 text-center text-sm text-inverse/45">{index + 1} / {assets.length}</span><button type="button" onClick={() => setIndex((value) => wrap(value + 1, assets.length))} className="cursor-pointer rounded-xl border border-inverse/10 bg-inverse/5 p-2 text-inverse/75 hover:bg-inverse/10" aria-label="Next scene"><ArrowRight className="h-4 w-4" /></button></div>}
    </div>
  )
}
  ...
  return (
    <div className="dark min-h-screen bg-overlay text-foreground" style={{ colorScheme: 'dark', '--background': '0 0% 0%' } as React.CSSProperties}>
      ...
      <main className="mx-auto w-full max-w-7xl space-y-8 px-4 py-8">
        ...
        <section className="grid gap-4 lg:grid-cols-2"><Card className="border-border/70 bg-background/55 p-4"><div className="flex items-center gap-2 text-sm font-semibold"><MessageSquareText className="h-4 w-4" /> Story-model rotation</div><p className="mt-2 text-sm text-muted-foreground">Shared requests rotate through available Hugging Face credentials and these text models until one succeeds.</p><div className="mt-4 space-y-2">{GAME_MASTER_MODELS.map((model, i) => <div key={model.id} className="rounded-xl border border-border/70 bg-overlay/20 p-4"><div className="font-mono text-sm">{i + 1}. {model.id}</div><div className="mt-2 text-sm font-medium">{model.role}</div><div className="mt-2 text-sm text-muted-foreground">{model.note}</div></div>)}</div></Card><Card className="border-border/70 bg-background/55 p-4"><div className="flex items-center gap-2 text-sm font-semibold"><ImageIcon className="h-4 w-4" /> Scene-model + provider rotation</div><p className="mt-2 text-sm text-muted-foreground">AppForge resolves each model’s current provider mapping and tries compatible providers within a bounded request.</p><div className="mt-4 space-y-2">{IMAGE_MODELS.map((model, i) => <div key={model.id} className="rounded-xl border border-border/70 bg-overlay/20 p-4"><div className="font-mono text-sm">{i + 1}. {model.id}</div><div className="mt-2 text-sm font-medium">{model.role}</div><div className="mt-2 text-sm text-muted-foreground">{model.note}</div></div>)}</div><div className="mt-4 flex flex-wrap gap-2">{IMAGE_PROVIDERS.map((provider) => <Badge key={provider} color="slate">{provider}</Badge>)}</div></Card></section>
      </main>
      <PublicFooter />
    </div>
  )
