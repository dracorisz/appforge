    ...
    <button type="button" onClick={onSelect} className={`group relative shrink-0 cursor-pointer overflow-hidden rounded-xl border border-white/10 bg-[#080d16] text-left transition-all duration-500 ${active ? 'z-10 w-[78vw] max-w-3xl scale-100 opacity-100 md:w-[58vw]' : 'w-[52vw] max-w-xl scale-[.82] opacity-45 md:w-[34vw]'}`} aria-current={active ? 'true' : undefined}>
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-950">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 p-4">
          ...
          {active && <p className="mt-2 text-[11px] text-white/55">{metaText(asset) || 'Generation metadata unavailable'}</p>}
        </div>
      </div>
      {active && <div className="flex items-center justify-between gap-4 border-t border-white/10 px-4 py-2">
        <div className="min-w-0 truncate text-[11px] text-white/55">Model: <span className="text-white/80">{modelLabel}</span></div>
