...
function CarouselCard({ asset, active, onSelect }: { asset: PublicDragonAsset; active: boolean; onSelect: () => void }) {
  ...
  return (
    <button type="button" onClick={onSelect} className={`group relative shrink-0 cursor-pointer overflow-hidden rounded-xl border border-white/10 bg-[#080d16] text-left transition-all duration-500 ${active ? 'z-10 w-[78vw] max-w-3xl scale-100 opacity-100 md:w-[58vw]' : 'w-[52vw] max-w-xl scale-[.82] opacity-45 md:w-[34vw]'}`} aria-current={active ? 'true' : undefined}>
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-950">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 p-4">
          ...
          <h3 className={`${active ? 'text-lg sm:text-lg' : 'text-sm sm:text-sm'} line-clamp-2 font-semibold tracking-tight text-white`}>{asset.title || 'Story Studio scene'}</h3>
          {active && <p className="mt-2 text-sm text-white/55">{metaText(asset) || 'Generation metadata unavailable'}</p>}
        </div>
      </div>
      {active && <div className="flex items-center justify-between gap-4 border-t border-white/10 px-4 py-2">
        <div className="min-w-0 truncate text-sm text-white/55">Model: <span className="text-white/80">{modelLabel}</span></div>
        <span className="shrink-0 text-sm text-white/40">{new Date(asset.generated_at).toLocaleDateString()}</span>
      </div>}
    </button>
  )
}
...
function InfiniteShowcase({ assets }: { assets: PublicDragonAsset[] }) {
  ...
  if (!assets.length) return <Card className="border-white/10 bg-white/[0.035] p-8 text-center text-sm text-white/45">No public creator-selected scenes yet.</Card>
  ...
  return (
    <div className="relative overflow-hidden py-8" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={() => setPaused(false)}>
      ...
      {assets.length > 1 && <div className="mt-4 flex items-center justify-center gap-4"><button type="button" onClick={() => setIndex((value) => wrap(value - 1, assets.length))} className="cursor-pointer rounded-xl border border-white/10 bg-white/5 p-2 text-white/75 hover:bg-white/10" aria-label="Previous scene"><ArrowLeft className="h-4 w-4" /></button><span className="min-w-16 text-center text-sm text-white/45">{index + 1} / {assets.length}</span><button type="button" onClick={() => setIndex((value) => wrap(value + 1, assets.length))} className="cursor-pointer rounded-xl border border-white/10 bg-white/5 p-2 text-white/75 hover:bg-white/10" aria-label="Next scene"><ArrowRight className="h-4 w-4" /></button></div>}
    </div>
  )
}
...
export function HuggingFaceGalleryPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    setError('')
    try {
      const [{ data, error: rpcError }, managed] = await Promise.all([
        supabase.rpc('dragon_arena_public_gallery', { limit_count: 60 }),
  ...
  return (
    <div className="dark min-h-screen bg-black text-foreground" style={{ colorScheme: 'dark', '--background': '0 0% 0%' } as React.CSSProperties}>
      ...
      <main className="mx-auto w-full max-w-7xl space-y-8 px-4 py-8">
        <section className="rounded-xl border border-border bg-background/55 p-4"><div className="flex flex-wrap items-center gap-2 text-sm font-medium text-muted-foreground"><Sparkles className="h-4 w-4" /> Hugging Face powered generation</div><h1 className="mt-2 text-lg font-semibold tracking-tight">Story Studio × Hugging Face</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Story Studio uses Hugging Face for narrative generation and scene artwork. Generated scenes stay private by default; creators explicitly choose what appears here.</p><div className="mt-4 flex flex-wrap gap-2"><Link to="/" className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border/70 bg-background/65 px-4 py-2 text-sm font-medium text-foreground hover:bg-accent/55">Open Story Studio <ArrowRight className="h-4 w-4" /></Link><a href="https://huggingface.co/" target="_blank" rel="noreferrer" className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border/70 bg-background/65 px-4 py-2 text-sm font-medium text-foreground hover:bg-accent/55"><ExternalLink className="h-4 w-4" /> Hugging Face</a></div></section>

        <section><div className="mb-2 flex items-end justify-between gap-4"><div><h2 className="text-lg font-semibold tracking-tight">Public generated assets</h2><p className="mt-2 text-sm text-muted-foreground">One gallery for creator-selected Story Studio scenes and admin-managed images.</p></div><span className="text-sm text-muted-foreground">{assets.length} scenes</span></div>{error && <Card className="mb-4 border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">Could not load the public gallery: {error}</Card>}{loading ? <Card className="flex items-center justify-center gap-2 border-border/70 bg-background/55 p-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Loading public Story Studio assets…</Card> : <InfiniteShowcase assets={assets} />}</section>

        <section className="grid gap-4 lg:grid-cols-2"><Card className="border-border/70 bg-background/55 p-4"><div className="flex items-center gap-2 text-sm font-semibold"><MessageSquareText className="h-4 w-4" /> Story-model rotation</div><p className="mt-2 text-sm leading-5 text-muted-foreground">Shared requests rotate through available Hugging Face credentials and these text models until one succeeds.</p><div className="mt-4 space-y-2">{GAME_MASTER_MODELS.map((model, i) => <div key={model.id} className="rounded-xl border border-border/70 bg-black/20 p-4"><div className="font-mono text-sm">{i + 1}. {model.id}</div><div className="mt-2 text-sm font-medium">{model.role}</div><div className="mt-2 text-sm leading-5 text-muted-foreground">{model.note}</div></div>)}</div></Card><Card className="border-border/70 bg-background/55 p-4"><div className="flex items-center gap-2 text-sm font-semibold"><ImageIcon className="h-4 w-4" /> Scene-model + provider rotation</div><p className="mt-2 text-sm leading-5 text-muted-foreground">AppForge resolves each model’s current provider mapping and tries compatible providers within a bounded request.</p><div className="mt-4 space-y-2">{IMAGE_MODELS.map((model, i) => <div key={model.id} className="rounded-xl border border-border/70 bg-black/20 p-4"><div className="font-mono text-sm">{i + 1}. {model.id}</div><div className="mt-2 text-sm font-medium">{model.role}</div><div className="mt-2 text-sm leading-5 text-muted-foreground">{model.note}</div></div>)}</div><div className="mt-4 flex flex-wrap gap-2">{IMAGE_PROVIDERS.map((provider) => <Badge key={provider} color="slate">{provider}</Badge>)}</div></Card></section>
      </main>
      <PublicFooter />
    </div>
  )
