...
export function PublicBlogPage() {
  ...
  return <div className="dark flex min-h-dvh flex-col bg-overlay text-foreground" style={{ colorScheme: 'dark', '--background': '0 0% 0%' } as React.CSSProperties}><PublicHeader /><main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 sm:px-4 lg:px-8"><header className="mx-auto max-w-2xl text-center"><div className="text-sm font-semibold uppercase tracking-[0.18em] text-muted-foreground">AppForge Blog</div><h1 className="mt-4 text-lg font-semibold tracking-[-0.04em] sm:text-5xl">Practical product stories.</h1><p className="mx-auto mt-4 max-w-xl text-sm text-muted-foreground">How AppForge tools work, what they solve, and how they fit together.</p>{loading && <div className="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Updating…</div>}</header>{featured && <section className="mx-auto mt-8 max-w-3xl"><Link to={`/blog/${featured.slug}`} className="block rounded-xl border border-border/70 bg-background/45 p-4 text-center transition-[border-color,background-color,box-shadow] hover:border-foreground/20 hover:bg-accent/25 hover:shadow-xl"><div className="text-sm font-semibold uppercase tracking-[0.14em] text-muted-foreground">Featured · {featured.appName}</div><h2 className="mx-auto mt-4 max-w-2xl text-lg font-semibold tracking-[-0.03em]">{featured.title}</h2><p className="mx-auto mt-4 max-w-2xl text-sm text-muted-foreground">{featured.description}</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-semibold">Read article <ArrowRight className="h-3.5 w-3.5" /></span></Link></section>}<section className="mt-8"><div className="mb-4 flex items-center justify-between border-b border-border/60 pb-4"><h2 className="text-sm font-semibold">Latest</h2><span className="text-sm text-muted-foreground">{articles.length} published</span></div><div className="grid gap-4 md:grid-cols-2">{remaining.map((article) => <ArticleCard key={article.slug} article={article} />)}</div></section></main><PublicFooter /></div>
}
...
export function PublicBlogArticlePage({ slug }: { slug: string }) {
  ...
  const related = article ? articles.filter((item) => item.slug !== article.slug).sort((a, b) => Number(b.appName === article.appName) - Number(a.appName === article.appName)).slice(0, 3) : []
  React.useEffect(() => { window.scrollTo({ top: 0, left: 0, behavior: 'auto' }) }, [slug])
  React.useEffect(() => { document.title = article ? `${article.title} · AppForge` : 'AppForge Blog' }, [article])
  if (!article && loading) return <div className="dark flex min-h-dvh items-center justify-center bg-overlay text-foreground"><Loader2 className="h-6 w-6 animate-spin" /></div>
  if (!article) return <PublicBlogPage />
  const articleWidth = readingMode ? 'max-w-4xl' : 'max-w-7xl'
  return <div className="dark flex min-h-dvh flex-col bg-overlay text-foreground" style={{ colorScheme: 'dark', '--background': '0 0% 0%' } as React.CSSProperties}><PublicHeader /><main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 sm:px-4 lg:px-8"><Link to="/blog" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="h-3.5 w-3.5" /> All articles</Link><article className={`mx-auto mt-8 w-full transition-[max-width] ${articleWidth}`}><header className="text-center"><div className="text-sm font-semibold uppercase tracking-[0.14em] text-muted-foreground">{article.appName}</div><h1 className="mt-4 text-lg font-semibold tracking-[-0.04em] sm:text-5xl">{article.title}</h1><p className="mx-auto mt-4 max-w-7xl text-sm text-muted-foreground">{article.description}</p><div className="mt-4 flex flex-wrap items-center justify-center gap-4 text-sm text-muted-foreground"><span className="inline-flex items-center gap-2"><CalendarDays className="h-3.5 w-3.5" /> {article.publishedAt}</span><span>{article.readTime}</span><Link to={article.appRoute} className="font-semibold text-foreground hover:underline">Open {article.appName}</Link></div></header>{article.imageUrl && <div className="mt-8 overflow-hidden rounded-xl border border-border/70"><img src={article.imageUrl} alt={article.title} className="max-h-[36rem] w-full object-cover" /></div>}<VideoBlock url={article.videoUrl} /><div className={`mx-auto mt-8 space-y-8 ${readingMode ? 'max-w-4xl' : 'max-w-7xl'}`}>{article.sections.map((section) => <section key={section.heading}><h2 className="text-lg font-semibold tracking-[-0.02em]">{section.heading}</h2><p className="mt-4 whitespace-pre-line text-sm text-muted-foreground">{section.body}</p></section>)}</div></article>{related.length > 0 && <section className="mt-8 border-t border-border/60 pt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-sm font-semibold">Related articles</h2><Link to="/blog" className="text-sm font-semibold text-muted-foreground hover:text-foreground">View all</Link></div><div className="grid gap-4 md:grid-cols-3">{related.map((item) => <ArticleCard key={item.slug} article={item} compact />)}</div></section>}</main><div className="fixed bottom-4 right-4 z-40 flex items-center gap-2 rounded-xl border border-border/70 bg-background/95 p-2 shadow-xl backdrop-blur-xl" role="group" aria-label="Article width"><button type="button" onClick={() => setReadingMode(true)} aria-pressed={readingMode} className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${readingMode ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>Reading</button><button type="button" onClick={() => setReadingMode(false)} aria-pressed={!readingMode} className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${!readingMode ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>Normal</button></div><PublicFooter /></div>
}
