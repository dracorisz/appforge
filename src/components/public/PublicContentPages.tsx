...
export function PublicBlogPage() {
  ...
  return <div className="dark flex min-h-dvh flex-col bg-black text-foreground" style={{ colorScheme: 'dark', '--background': '0 0% 0%' } as React.CSSProperties}><PublicHeader /><main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 sm:px-4 lg:px-8"><header className="mx-auto max-w-2xl text-center"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">AppForge Blog</div><h1 className="mt-4 text-lg font-semibold tracking-[-0.04em] sm:text-5xl">Practical product stories.</h1><p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-muted-foreground">How AppForge tools work, what they solve, and how they fit together.</p>{loading && <div className="mt-4 inline-flex items-center gap-2 text-xs text-muted-foreground"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Updating…</div>}</header>{featured && <section className="mx-auto mt-8 max-w-3xl"><Link to={`/blog/${featured.slug}`} className="block rounded-xl border border-border/70 bg-background/45 p-4 text-center transition-[border-color,background-color,box-shadow] hover:border-foreground/20 hover:bg-accent/25 hover:shadow-xl"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Featured · {featured.appName}</div><h2 className="mx-auto mt-4 max-w-2xl text-lg font-semibold tracking-[-0.03em]">{featured.title}</h2><p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">{featured.description}</p><span className="mt-4 inline-flex items-center gap-2 text-xs font-semibold">Read article <ArrowRight className="h-3.5 w-3.5" /></span></Link></section>}<section className="mt-8"><div className="mb-4 flex items-center justify-between border-b border-border/60 pb-4"><h2 className="text-sm font-semibold">Latest</h2><span className="text-xs text-muted-foreground">{articles.length} published</span></div><div className="grid gap-4 md:grid-cols-2">{remaining.map((article) => <ArticleCard key={article.slug} article={article} />)}</div></section></main><PublicFooter /></div>
}
