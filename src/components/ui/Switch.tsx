...
export function Switch({ checked, onCheckedChange, label, className = '', disabled, ...props }: SwitchProps) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full border shadow-sm transition-[background-color,border-color,box-shadow] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/20 focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50 ${checked ? 'border-primary/60 bg-primary' : 'border-input bg-muted hover:border-foreground/20'} ${className}`}
      {...props}
    >
