    ...
    <header className={`flex min-w-0 flex-col gap-2 ${className}`}>
      <div className="flex min-w-0 items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-2">
          {icon && <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-border/70 bg-muted/50 text-muted-foreground [&>svg]:h-4 [&>svg]:w-4">{icon}</span>}
