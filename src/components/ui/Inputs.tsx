...
export function SearchInput({
  ...
  return (
    <div className={`relative w-full ${className}`}>
      <Search className="pointer-events-none absolute left-2 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
      ...
      {value && onClear && <IconButton label="Clear search" icon={<X />} onClick={onClear} className="absolute right-2 top-1/2 z-10 -translate-y-1/2" />}
    </div>
  );
}
