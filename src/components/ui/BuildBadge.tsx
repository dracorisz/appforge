...
export function BuildBadge({ compact = false, className = '' }: { compact?: boolean; className?: string }) {
  ...
  return (
    <span
      ...
      className={`inline-flex items-center gap-1.5 rounded-xl border border-border/70 bg-background/55 px-2 py-1 font-mono text-[10px] text-muted-foreground backdrop-blur-md ${className}`}
    >
