...
export function BuildBadge({ compact = false, className = '' }: { compact?: boolean; className?: string }) {
  ...
  return (
    <span
      ...
      className={`inline-flex h-8 items-center gap-2 rounded-xl border border-border/70 bg-background/55 px-2 font-mono text-sm text-muted-foreground backdrop-blur-md ${className}`}
    >
