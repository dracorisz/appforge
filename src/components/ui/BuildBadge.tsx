...
export function BuildBadge({ compact = false, className = "" }: { compact?: boolean; className?: string }) {
  ...
  return (
    <span title={`AppForge ${BUILD_INFO.version}\nCommit: ${BUILD_INFO.sha}\nBuilt: ${BUILD_INFO.buildTime || "local"}`} className={`inline-flex h-9 items-center gap-2 rounded-xl border border-border/70 bg-background/55 px-2 font-mono text-sm text-muted-foreground backdrop-blur-md ${className}`}>
