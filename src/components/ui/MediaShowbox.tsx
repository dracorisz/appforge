    ...
    <div className="fixed inset-0 z-[60] flex h-[100dvh] min-h-[100dvh] w-screen items-center justify-center overflow-y-auto bg-overlay/85 p-2 backdrop-blur-sm sm:p-4" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()} className="flex max-h-[calc(100dvh-1rem)] w-full max-w-5xl flex-col overflow-hidden rounded-xl border border-inverse/10 bg-background sm:max-h-[calc(100dvh-1.5rem)]">
        <header className="flex items-center gap-4 border-b border-border/70 px-4">
          ...
          <button onClick={onClose} className="inline-flex h-9 w-9 items-center justify-center rounded-xl text-muted-foreground hover:bg-accent hover:text-foreground" aria-label="Close preview">
            <X className="h-4 w-4" />
          </button>
        </header>
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto bg-overlay p-2 sm:p-4">
          {type === "image" ? (
            ...
            <div className="aspect-video w-full max-w-5xl overflow-hidden bg-overlay">
              <iframe src={`https://www.youtube-nocookie.com/embed/${ytId}?autoplay=1&rel=0`} title={title} className="h-full w-full" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen />
            </div>
          ) : mediaUrl ? (
