...
import { Download, ExternalLink, Image as ImageIcon, Loader2, Play, X } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/72 p-3 backdrop-blur-md sm:p-6" onMouseDown={onClose}>
      <section
        ...
        className="surface-card flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-background/88 shadow-2xl"
      >
        <header className="flex items-center gap-3 border-b border-border/70 px-4 py-3 sm:px-5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border/70 bg-muted/45 text-muted-foreground">
          ...
          <button onClick={onClose} className="rounded-lg border border-transparent p-2 text-muted-foreground hover:border-border hover:bg-accent hover:text-foreground" aria-label="Close preview">
            <X className="h-4 w-4" />
          </button>
        </header>

        <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto bg-black/85 p-2 sm:p-5">
          {type === 'image' ? (
        ...
        <footer className="flex flex-col gap-2 border-t border-border/70 bg-background/65 px-4 py-3 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div className="min-w-0 text-xs text-muted-foreground">
            {note || (type === 'video' && ytId ? 'YouTube plays securely in-page; direct video file download is not exposed by the source.' : 'Preview stays inside AppForge.')}
          </div>
          <div className="flex shrink-0 flex-wrap gap-2">
            ...
            <a href={originalUrl} target="_blank" rel="noopener noreferrer">
              <Button variant="secondary" size="sm"><ExternalLink className="h-3.5 w-3.5" /> Open source</Button>
            </a>
          </div>
        </footer>
      </section>
