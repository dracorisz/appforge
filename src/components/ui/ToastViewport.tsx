  ...
  return (
    <div className="pointer-events-none fixed right-4 top-4 z-[120] flex w-[min(24rem,calc(100vw-2rem))] flex-col gap-2" aria-live="polite" aria-atomic="false">
      {toasts.map((item) => {
        ...
        const tone = item.tone === "success" ? "border-success/30 bg-success/40 text-white" : item.tone === "error" ? "border-destructive/40 bg-destructive/60 text-white" : "border-border/80 bg-popover/80 text-popover-foreground";
        return (
          <div key={item.id} className={`pointer-events-auto flex items-center justify-between gap-4 rounded-xl border px-4 py-3 backdrop-blur ${tone}`}>
