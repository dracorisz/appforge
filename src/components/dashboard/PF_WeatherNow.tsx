...
export function PF_WeatherNow() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><AppHeading /></div></div><div className="flex items-center gap-1 rounded-lg border border-border p-1">{(['c', 'f'] as Unit[]).map((value) => <button key={value} onClick={() => setUnit(value)} className={`rounded-xl px-2.5 py-1 text-xs font-medium ${unit === value ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>°{value.toUpperCase()}</button>)}</div></div>
      ...
      <Card className="p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center"><label className="min-w-0 text-xs text-muted-foreground"><span className="mb-1 block font-medium text-foreground">Sidebar weather city</span><select value={sidebarLocation} onChange={(event) => chooseSidebarCity(event.target.value)} className="w-full rounded-xl border border-input bg-background px-2 py-2 text-xs text-foreground"><option value="">First saved city</option>{cities.map((weather) => <option key={weather.location} value={weather.location}>{weather.location}</option>)}</select></label><div className="flex items-center gap-2"><ArrowUpDown className="h-3.5 w-3.5 text-muted-foreground" /><select value={sortBy} onChange={(event) => setSortBy(event.target.value as SortBy)} className="rounded-xl border border-input bg-background px-2 py-2 text-xs text-foreground"><option value="name">Name</option><option value="temp">Temperature</option><option value="humidity">Humidity</option><option value="wind">Wind</option></select></div><div className="flex items-center gap-1 rounded-xl border border-border p-1"><button onClick={() => setViewMode('grid')} aria-label="Grid view" className={`rounded-xlp-1 ${viewMode === 'grid' ? 'bg-accent' : 'text-muted-foreground hover:text-foreground'}`}><LayoutGrid className="h-3.5 w-3.5" /></button><button onClick={() => setViewMode('list')} aria-label="List view" className={`rounded-xlp-1 ${viewMode === 'list' ? 'bg-accent' : 'text-muted-foreground hover:text-foreground'}`}><List className="h-3.5 w-3.5" /></button></div></div>
