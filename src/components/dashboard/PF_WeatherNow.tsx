...
const readSidebarLocation = () => {
  try { return localStorage.getItem(SIDEBAR_LOCATION_KEY) || '' } catch { return '' }
}

function Metric({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string }) {
  return <div className="group rounded-xl border border-border/70 bg-background/45 p-4 backdrop-blur-sm transition-colors hover:border-foreground/15 hover:bg-accent/35"><div className="flex items-center gap-2 text-muted-foreground"><Icon className="h-3.5 w-3.5" /><span className="text-sm font-semibold uppercase tracking-[0.12em]">{label}</span></div><p className="mt-2 text-sm font-semibold tabular-nums text-foreground">{value}</p></div>
}

function ListMetric({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string }) {
  return <div className="min-w-0 px-4 py-2 lg:border-l lg:border-border/55"><div className="flex items-center gap-2 text-muted-foreground"><Icon className="h-3.5 w-3.5 shrink-0" /><span className="text-sm font-semibold uppercase tracking-[0.12em]">{label}</span></div><p className="mt-2 truncate text-sm font-medium tabular-nums text-foreground">{value}</p></div>
}
...
export function PF_WeatherNow() {
  ...
  React.useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(cities)) } catch { /* saved weather is an optional device cache */ }
  }, [cities])
  ...
  const chooseSidebarCity = (location: string) => {
    ...
    } catch { /* sidebar preference remains active for this session */ }
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4 sm:p-4">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto_auto_auto] md:items-center"><label className="min-w-0 text-sm text-muted-foreground"><span className="mb-2 block font-medium text-foreground">Sidebar weather city</span><select value={sidebarLocation} onChange={(event) => chooseSidebarCity(event.target.value)} className="w-full rounded-xl border border-input bg-background px-2 py-2 text-sm text-foreground"><option value="">First saved city</option>{cities.map((weather) => <option key={weather.location} value={weather.location}>{weather.location}</option>)}</select></label><div className="flex items-center gap-2"><ArrowUpDown className="h-3.5 w-3.5 text-muted-foreground" /><select value={sortBy} onChange={(event) => setSortBy(event.target.value as SortBy)} className="rounded-xl border border-input bg-background px-2 py-2 text-sm text-foreground"><option value="name">Name</option><option value="temp">Temperature</option><option value="humidity">Humidity</option><option value="wind">Wind</option></select></div><div className="flex items-center gap-2 rounded-xl border border-border p-2">{(['c', 'f'] as Unit[]).map((value) => <button key={value} onClick={() => setUnit(value)} className={`rounded-xl px-2 py-2 text-sm font-medium ${unit === value ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>°{value.toUpperCase()}</button>)}</div><div className="flex items-center gap-2 rounded-xl border border-border p-2"><button onClick={() => setViewMode('grid')} aria-label="Grid view" className={`rounded-xl p-2 ${viewMode === 'grid' ? 'bg-accent' : 'text-muted-foreground hover:text-foreground'}`}><LayoutGrid className="h-3.5 w-3.5" /></button><button onClick={() => setViewMode('list')} aria-label="List view" className={`rounded-xl p-2 ${viewMode === 'list' ? 'bg-accent' : 'text-muted-foreground hover:text-foreground'}`}><List className="h-3.5 w-3.5" /></button></div></div>
        <div className="mt-4 flex flex-wrap gap-2"><Button variant="secondary" size="sm" onClick={() => void refreshAll()} disabled={!cities.length || loading}><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> Refresh all</Button><Button variant="secondary" size="sm" onClick={() => void copySummary()} disabled={!cities.length}>Copy weather summary</Button></div>
      ...
      {sortedCities.length > 0 ? viewMode === 'list' ? <div className="space-y-2">{sortedCities.map((weather) => <Card key={weather.location} className="relative overflow-hidden p-2"><div className="relative grid items-center gap-2 px-4 py-2 sm:grid-cols-2 lg:grid-cols-[minmax(180px,1.45fr)_minmax(145px,.9fr)_repeat(3,minmax(105px,.7fr))_108px] lg:px-4"><div className="min-w-0 py-2 lg:pr-4"><div className="flex items-center gap-2"><h3 className="truncate text-lg font-semibold text-foreground">{weather.location}</h3>{sidebarLocation === weather.location && <Star className="h-3.5 w-3.5 fill-current text-warning" />}</div>{weather.local_time && <p className="mt-2 truncate text-sm text-muted-foreground">Local: {weather.local_time.replace('T', ' ')}</p>}<p className="mt-2 truncate text-sm text-muted-foreground">{freshnessLabel(weather)}</p></div><div className="flex min-w-0 items-center gap-4 px-2 py-2 lg:border-l lg:border-border/55 lg:px-4"><span className="shrink-0">{weatherIcon(weather.condition, true)}</span><div className="min-w-0"><p className="truncate text-lg font-medium tabular-nums text-foreground">{displayTemp(weather.temp_c)}</p><p className="truncate text-sm text-muted-foreground">{weather.condition}</p></div></div><ListMetric icon={Thermometer} label="Feels like" value={displayTemp(weather.feelslike_c)} /><ListMetric icon={Droplets} label="Humidity" value={`${weather.humidity}%`} /><ListMetric icon={Wind} label="Wind" value={`${weather.wind_kph.toFixed(1)} km/h`} /><div className="flex items-center justify-end gap-2 py-2 lg:border-l lg:border-border/55 lg:pl-4"><Button variant="ghost" size="sm" onClick={() => chooseSidebarCity(weather.location)} title="Show in sidebar"><Star className={`h-3.5 w-3.5 ${sidebarLocation === weather.location ? 'fill-current text-warning' : ''}`} /></Button><Button variant="ghost" size="sm" onClick={() => void refreshCity(weather)} disabled={refreshing === weather.location} title="Refresh">{refreshing === weather.location ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}</Button><Button variant="ghost" size="sm" onClick={() => removeCity(weather.location)} title="Remove"><X className="h-3.5 w-3.5" /></Button></div></div></Card>)}</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{sortedCities.map((weather) => <Card key={weather.location} className="relative overflow-hidden p-2"><div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-primary/10 via-transparent to-accent/25" /><div className="relative p-4"><div className="flex min-w-0 items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-2"><MapPin className="h-4 w-4 shrink-0 text-muted-foreground" /><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate font-semibold text-foreground">{weather.location}</h3>{sidebarLocation === weather.location && <Star className="h-3.5 w-3.5 fill-current text-warning" />}</div>{weather.local_time && <p className="mt-2 text-sm text-muted-foreground">Local: {weather.local_time.replace('T', ' ')}</p>}<p className="mt-2 text-sm text-muted-foreground">{freshnessLabel(weather)}</p></div></div><div className="flex shrink-0 gap-2"><Button variant="ghost" size="sm" onClick={() => chooseSidebarCity(weather.location)} title="Show in sidebar"><Star className={`h-3.5 w-3.5 ${sidebarLocation === weather.location ? 'fill-current text-warning' : ''}`} /></Button><Button variant="ghost" size="sm" onClick={() => void refreshCity(weather)} disabled={refreshing === weather.location} title="Refresh">{refreshing === weather.location ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}</Button><Button variant="ghost" size="sm" onClick={() => removeCity(weather.location)} title="Remove"><X className="h-3.5 w-3.5" /></Button></div></div><div className="mt-4"><div className="flex items-center justify-between gap-4"><div><p className="text-lg font-semibold tracking-[-0.04em] text-foreground">{displayTemp(weather.temp_c)}</p><p className="mt-2 inline-flex items-center gap-2 text-sm text-muted-foreground"><Thermometer className="h-3.5 w-3.5" /> Feels like {displayTemp(weather.feelslike_c)}</p></div><div className="flex flex-col items-end gap-2"><span className="flex h-14 w-14 items-center justify-center rounded-xl border border-border/60 bg-background/55 backdrop-blur-md">{weatherIcon(weather.condition)}</span><Badge color="slate">{weather.condition}</Badge></div></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3"><Metric icon={Droplets} label="Humidity" value={`${weather.humidity}%`} /><Metric icon={Wind} label="Wind" value={`${weather.wind_kph.toFixed(1)} km/h`} /><Metric icon={Gauge} label="Pressure" value={typeof weather.pressure_hpa === 'number' ? `${Math.round(weather.pressure_hpa)} hPa` : '—'} /><Metric icon={CloudSun} label="Cloud cover" value={typeof weather.cloud_cover === 'number' ? `${Math.round(weather.cloud_cover)}%` : '—'} /><Metric icon={Eye} label="Visibility" value={typeof weather.visibility_km === 'number' ? `${weather.visibility_km.toFixed(1)} km` : '—'} /><div className="grid grid-cols-2 gap-2"><Metric icon={Sunrise} label="Sunrise" value={timeOnly(weather.sunrise)} /><Metric icon={Sunset} label="Sunset" value={timeOnly(weather.sunset)} /></div></div></div></div></Card>)}</div> : <Card><div className="py-8 text-center"><Cloud className="mx-auto h-7 w-7 text-muted-foreground" /><h2 className="mt-4 text-sm font-medium text-foreground">Add your first location</h2><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Search by city or postal code, use device location, or add one of the three-city regional sets above.</p></div></Card>}
    </div>
  )
}
