    ...
    <section className={`mx-auto w-full rounded-xl border bg-card p-4 md:p-4 ${standalone ? '' : 'mt-4'}`} aria-label={standalone ? 'Background Remover' : 'Desktop Buddy transparency repair'}>
      ...
      {result && source && (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-xl border bg-background/45 p-4">
          <div className="text-sm text-muted-foreground"><span className="font-medium text-foreground">{result.width} × {result.height}</span> · {result.transparentPercent.toFixed(1)}% transparent/feathered pixels</div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => downloadResult(result, source.name)} className="inline-flex min-h-9 items-center gap-2 rounded-xl border px-4 text-sm font-semibold hover:bg-accent"><Download className="h-3.5 w-3.5" /> Download PNG</button>
            {!standalone && <button type="button" onClick={applyToBuddy} className="inline-flex min-h-9 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground"><Check className="h-3.5 w-3.5" /> Use in Buddy</button>}
          </div>
        </div>
      )}
    </section>
