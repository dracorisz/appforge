...
export function PF_AIDragonArenaStudioV2() {
  ...
  return <div className="mx-auto max-w-5xl space-y-4 pb-8 text-foreground">
    ...
    {panel === 'sessions' && <Card className="p-4"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{sessions.length ? sessions.map((session) => <button key={session.id} type="button" onClick={() => void loadSession(session)} className={`rounded-xl border p-2 text-left text-xs hover:bg-accent ${session.id === sessionId ? 'border-primary/50 bg-accent/50' : 'border-border/70'}`}><div className="font-semibold">{session.title || 'Untitled story'}</div><div className="mt-2 text-xs text-muted-foreground">{session.turn_count} turns · {new Date(session.updated_at).toLocaleDateString()}</div></button>) : <div className="text-xs text-muted-foreground">No saved stories yet.</div>}</div></Card>}
