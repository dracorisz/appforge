...
export function PF_AIDragonArenaStudioV2() {
  ...
  return <div className="mx-auto max-w-5xl space-y-3 pb-10 text-foreground">
    ...
    <Card className="p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">What do you do?</div><div className="mt-2 flex flex-wrap gap-2">{choices.map((choice) => <Button key={choice} variant="secondary" onClick={() => void play(choice)} disabled={loading}>{choice}</Button>)}</div><div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><Input value={action} onChange={(event) => setAction(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void play() }} placeholder="Or choose your own action…" disabled={loading} /><Button onClick={() => void play()} disabled={loading || !action.trim()}><Send /> Continue</Button></div><div className="mt-3 flex flex-wrap gap-2"><Button variant="secondary" onClick={() => void generateScene()} disabled={!sessionId || !turns.length || sceneLoading}><ImagePlus /> {sceneLoading ? 'Generating…' : latestScene ? 'Regenerate scene' : 'Generate scene'}</Button></div></Card>
