  ...
  try {
    ...
    if (Array.isArray(parsed))
      return parsed
        ...
        .filter((value) => value.startsWith("hf_"))
        .slice(0, 3);
...
export function PF_AIDragonArenaStudioV2() {
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem("dragon-builder-mode", mode);
    } catch {
      /* session-only */
    }
  }, [mode]);
  const refreshSessions = React.useCallback(
    async (uid = userId) => {
      if (uid) setSessions(await listSessions(uid));
    },
    [userId],
  );
  const refreshAssets = React.useCallback(
    async (sid = sessionId, uid = userId) => {
      if (sid && uid) setAssets(await listAssets(uid, { sessionId: sid }));
    },
    [sessionId, userId],
  );

  const loadSession = React.useCallback(
    async (session: DragonSession, uid = userId) => {
      if (!uid) return;
      setLoading(true);
      setError("");
      try {
        ...
        setError(cause instanceof Error ? cause.message : "Could not restore this story.");
      } finally {
        setLoading(false);
      }
    },
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      if (!cancelled) setError(cause instanceof Error ? cause.message : "Could not restore Story Studio.");
    });
    return () => {
      cancelled = true;
    };
  ...
  const play = async (selected?: string) => {
    ...
    try {
      ...
      const history = [
        { role: "gm", text: opening.narrative },
        ...turns.flatMap((item) => [
          { role: "player", text: item.playerAction },
          { role: "gm", text: item.narrative },
        ]),
      ];
      ...
      const suggested = (Array.isArray(payload.choices) && payload.choices.length ? payload.choices : payload.actions || [])
        .map(String)
        .map((item) => item.trim())
        .filter(Boolean)
        .slice(0, 3);
      ...
      await supabase
        .from("dragon_arena_sessions")
        .update({ turn_count: turnNumber, updated_at: new Date().toISOString(), metadata: { source: "story-studio", builder_mode: mode, model: payload.model || MODEL, opening_id: opening.id, opening } })
        .eq("id", sid);
      ...
      await refreshSessions(auth.user.id);
    } catch (cause) {
  ...
  const generateScene = async () => {
    ...
    try {
      ...
      if (!data.session) throw new Error("Sign in again to generate art.");
      const hfTokens = loadTokens();
      const headers: Record<string, string> = { "Content-Type": "application/json", Authorization: `Bearer ${data.session.access_token}` };
      if (hfTokens.length) headers["x-hf-tokens"] = hfTokens.join(",");
      ...
      setError(cause instanceof Error ? cause.message : "Could not generate scene art.");
    } finally {
  ...
  const exportProduct = () => {
    ...
    const panels = turns
      .map((item) => {
        const scene = assets.find((asset) => sceneTurn(asset) === item.turnNumber);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 pb-8 text-foreground">
      <AppHeading />
      <div className="flex flex-wrap items-center justify-between gap-2">
        <Tabs
          tabs={[
            { id: "novel", label: "Novel", icon: BookOpen },
            { id: "comics", label: "Comics", icon: GalleryThumbnails },
          ]}
          active={mode}
          onChange={(id) => setMode(id as BuilderMode)}
          ariaLabel="Story format"
        />
        <div className="flex items-center gap-2">
          <Button variant={panel === "sessions" ? "secondary" : "ghost"} onClick={() => setPanel(panel === "sessions" ? null : "sessions")}>
            <Users /> Sessions
          </Button>
          <Button variant={panel === "assets" ? "secondary" : "ghost"} onClick={() => setPanel(panel === "assets" ? null : "assets")}>
            <GalleryThumbnails /> Assets
          </Button>
          <Button variant="ghost" onClick={exportProduct} disabled={!turns.length}>
            <Download /> Export
          </Button>
          <Button variant="ghost" onClick={() => startNew(randomOpening())}>
            <RefreshCcw /> New
          </Button>
        </div>
      </div>
        ...
        <Card className="p-4">
          <div className="flex gap-2 overflow-x-auto">
            {assets.length ? (
              ...
              <div className="text-sm text-muted-foreground">No generated scenes yet.</div>
            )}
          </div>
        </Card>
      )}
        ...
        <div className="border-b border-border/70 p-4">
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_220px] sm:items-end">
            ...
            <Select
              value={opening.id}
              onChange={(event) => {
                const next = DRAGON_ARENA_OPENINGS.find((item) => item.id === event.target.value);
                if (next) startNew(next);
              }}
              aria-label="Opening scenario"
            >
              {DRAGON_ARENA_OPENINGS.map((item) => (
        ...
        <div className="space-y-4 p-4 sm:p-4">
          ...
          {loading && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Loader2 className="h-3.5 w-3.5 animate-spin" /> The Game Master is writing…
            </div>
          )}
        </div>
      </Card>
      ...
      <Card className="p-4">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
          <Input
            ...
            onKeyDown={(event) => {
              if (event.key === "Enter") void play();
            }}
            placeholder="Or choose your own action…"
            disabled={loading}
          />
          <Button onClick={() => void play()} disabled={loading || !action.trim()}>
            <Send /> Continue
          </Button>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          <Button variant="secondary" onClick={() => void generateScene()} disabled={!sessionId || !turns.length || sceneLoading}>
            <ImagePlus /> {sceneLoading ? "Generating…" : latestScene ? "Regenerate scene" : "Generate scene"}
          </Button>
        </div>
      </Card>
      {lightbox && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center bg-overlay/90 p-4"
          onMouseDown={(event) => {
