...
export function PF_AIDragonArenaStudioV2() {
  ...
  return <div className="mx-auto max-w-5xl space-y-4 pb-8 text-foreground">
    <AppHeading />
    <div className="flex flex-wrap items-center justify-between gap-2"><Tabs tabs={[{ id: 'novel', label: 'Novel', icon: BookOpen }, { id: 'comics', label: 'Comics', icon: GalleryThumbnails }]} active={mode} onChange={(id) => setMode(id as BuilderMode)} ariaLabel="Story format" /><div className="flex items-center gap-2"><Button variant={panel === 'sessions' ? 'secondary' : 'ghost'} onClick={() => setPanel(panel === 'sessions' ? null : 'sessions')}><Users /> Sessions</Button><Button variant={panel === 'assets' ? 'secondary' : 'ghost'} onClick={() => setPanel(panel === 'assets' ? null : 'assets')}><GalleryThumbnails /> Assets</Button><Button variant="ghost" onClick={exportProduct} disabled={!turns.length}><Download /> Export</Button><Button variant="ghost" onClick={() => startNew(randomOpening())}><RefreshCcw /> New</Button></div></div>

    {panel === 'sessions' && <Card className="p-4"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{sessions.length ? sessions.map((session) => <button key={session.id} type="button" onClick={() => void loadSession(session)} className={`rounded-xl border p-2 text-left text-sm hover:bg-accent ${session.id === sessionId ? 'border-primary/50 bg-accent/50' : 'border-border/70'}`}><div className="font-semibold">{session.title || 'Untitled story'}</div><div className="mt-2 text-sm text-muted-foreground">{session.turn_count} turns · {new Date(session.updated_at).toLocaleDateString()}</div></button>) : <div className="text-sm text-muted-foreground">No saved stories yet.</div>}</div></Card>}
    {panel === 'assets' && <Card className="p-4"><div className="flex gap-2 overflow-x-auto">{assets.length ? assets.map((asset) => { const src = safeAssetUrl(asset); return <button key={asset.id} type="button" onClick={() => src && setLightbox(src)} className="h-20 w-28 shrink-0 overflow-hidden rounded-xl border border-border/70 bg-muted">{src && <img src={src} alt="" className="h-full w-full object-cover" />}</button> }) : <div className="text-sm text-muted-foreground">No generated scenes yet.</div>}</div></Card>}

    <Card className="overflow-hidden p-2"><div className="border-b border-border/70 p-4"><div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_220px] sm:items-end"><div><div className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">World seed</div><h2 className="mt-2 text-lg font-semibold">{opening.label}</h2><p className="mt-2 max-h-32 max-w-3xl overflow-y-auto pr-2 text-sm text-muted-foreground">{opening.narrative}</p></div><Select value={opening.id} onChange={(event) => { const next = DRAGON_ARENA_OPENINGS.find((item) => item.id === event.target.value); if (next) startNew(next) }} aria-label="Opening scenario">{DRAGON_ARENA_OPENINGS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select></div></div>
      <div className="space-y-4 p-4 sm:p-4">{turns.map((item) => { const scene = assets.find((asset) => sceneTurn(asset) === item.turnNumber); const src = safeAssetUrl(scene); return <section key={item.turnNumber} className="border-b border-border/60 pb-4 last:border-0"><div className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">You · {item.playerAction}</div><div className="mt-2 grid gap-4 md:grid-cols-[minmax(0,1fr)_180px] md:items-start"><div className="whitespace-pre-line text-sm">{item.narrative}</div>{src && <button type="button" onClick={() => setLightbox(src)} className="overflow-hidden rounded-xl border border-border/70"><img src={src} alt={`Scene ${item.turnNumber}`} className="aspect-[4/3] w-full object-cover" /></button>}</div></section> })}{loading && <div className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 className="h-3.5 w-3.5 animate-spin" /> The Game Master is writing…</div>}</div>
    </Card>

    {error && <Card className="border-destructive/30 bg-destructive/5 p-2 text-sm text-destructive">{error}</Card>}
    <Card className="p-4"><div className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">What do you do?</div><div className="mt-2 flex flex-wrap gap-2">{choices.map((choice) => <Button key={choice} variant="secondary" onClick={() => void play(choice)} disabled={loading}>{choice}</Button>)}</div><div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><Input value={action} onChange={(event) => setAction(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void play() }} placeholder="Or choose your own action…" disabled={loading} /><Button onClick={() => void play()} disabled={loading || !action.trim()}><Send /> Continue</Button></div><div className="mt-4 flex flex-wrap gap-2"><Button variant="secondary" onClick={() => void generateScene()} disabled={!sessionId || !turns.length || sceneLoading}><ImagePlus /> {sceneLoading ? 'Generating…' : latestScene ? 'Regenerate scene' : 'Generate scene'}</Button></div></Card>
    {lightbox && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-overlay/90 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setLightbox('') }}><button type="button" aria-label="Close image" onClick={() => setLightbox('')} className="absolute right-4 top-4 flex h-6 w-6 items-center justify-center rounded-xl bg-inverse/10 text-inverse"><X className="h-4 w-4" /></button><img src={lightbox} alt="Generated story scene" className="max-h-[88dvh] max-w-[92vw] rounded-xl object-contain" /></div>}
  </div>
}
