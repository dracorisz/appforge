...
import { Check, ChevronLeft, ChevronRight, Download, FolderOpen, Image as ImageIcon, Maximize2, RotateCcw, Tag, Upload, X } from "lucide-react";
  ...
  const importLabels = async (file: File) => {
    ...
    try {
      ...
      parsed.slice(0, MAX_IMAGE_COUNT).forEach((item: unknown) => {
        ...
        const rawPath = String(record.path || record.filename || "")
          .trim()
          .slice(0, 1000);
        if (!rawPath) return;
        const matching = images.find((image) => image.path === rawPath || image.name === rawPath);
        const key = matching?.id || rawPath;
        const safeTags: string[] = Array.isArray(record.tags) ? Array.from(new Set(record.tags.map((tag: unknown) => String(tag).trim().slice(0, 80)).filter((tag): tag is string => Boolean(tag)))).slice(0, 32) : [];
        imported[key] = {
      ...
      <Card>
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 min-w-[18rem] lg:grid-cols-2">
            ...
            <Button variant="secondary" onClick={exportLabels} disabled={!images.length}>
        ...
        {images.length > 0 && (
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            <div className="rounded-xl border border-border bg-muted/30 p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-foreground">{images.length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/30 p-4">
              <p className="text-sm text-muted-foreground">Labeled</p>
              <p className="mt-2 text-lg font-semibold text-foreground">{labeledCount}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/30 p-4">
              <p className="text-sm text-muted-foreground">Approved</p>
              <p className="mt-2 text-lg font-semibold text-foreground">
                {approvedCount} <span className="text-sm font-normal text-muted-foreground">({completion}%)</span>
              </p>
            </div>
          </div>
        )}
      ...
      {currentImage ? (
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="max-w-[55vw] truncate text-sm text-muted-foreground">{currentImage.path}</span>
              </div>
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={() => goTo(currentIndex - 1)} disabled={currentIndex === 0}>
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => goTo(currentIndex + 1)} disabled={currentIndex === images.length - 1}>
                  <ChevronRight className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="sm" onClick={saveCurrentImage} title="Download original">
                  <Download className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setLightbox(currentImage.url)} title="Full screen">
          ...
          <div className="space-y-4">
            <Card>
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                <Button variant={currentLabel?.approved ? "secondary" : "primary"} onClick={() => updateLabel({ approved: !currentLabel?.approved })}>
                  <Check className="h-4 w-4" /> {currentLabel?.approved ? "Approved" : "Approve image"}
                </Button>
                <Button variant="secondary" onClick={approveAndNext} disabled={currentIndex >= images.length - 1 && Boolean(currentLabel?.approved)}>
                  Approve & next
                </Button>
              </div>
            ...
            <Card>
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">{completion}% approved</span>
              </div>
              <div className="mt-4 max-h-[340px] space-y-2 overflow-y-auto pr-2">
                {images.map((image, index) => {
                  const item = labels[image.id];
                  return (
                    <Button key={image.id} onClick={() => goTo(index)} className={`flex w-full items-center gap-2 rounded-xl border px-2 py-2 text-left text-sm ${index === currentIndex ? "border-foreground/30 bg-accent" : "border-transparent hover:bg-accent/50"}`}>
                      <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-xl ${item?.approved ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}>{item?.approved ? <Check className="h-3 w-3" /> : index + 1}</span>
                      <span className="min-w-0 flex-1 truncate text-foreground">{image.path}</span>
                    </Button>
                  );
                })}
              </div>
              <Button variant="ghost" size="sm" className="mt-4 w-full" onClick={clearFolderLabels}>
                <RotateCcw className="h-4 w-4" /> Clear labels for this folder
              </Button>
            </Card>
          </div>
        </div>
        ...
        <Card>
          <div className="py-8 text-center">
            ...
            <h2 className="mt-4 text-sm font-medium text-foreground">Open a local image folder</h2>
            <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Supported formats: PNG, JPEG, WebP, GIF, and AVIF. Images are previewed through local object URLs and are not uploaded.</p>
          </div>
        </Card>
      )}
