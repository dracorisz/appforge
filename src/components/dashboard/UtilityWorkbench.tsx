      ...
      <Card className="p-4 sm:p-5">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-3">
            ...
            {generator && <div className="grid gap-3 sm:grid-cols-2"><Input label="Quantity" type="number" min={1} max={definition.mode === 'uuid' ? 100 : 50} value={quantity} onChange={(event) => setQuantity(Number(event.target.value) || 1)} />{definition.mode !== 'uuid' && <Input label={definition.mode === 'token' ? 'Bytes per token' : 'Password length'} type="number" min={definition.mode === 'token' ? 16 : 8} max={128} value={length} onChange={(event) => setLength(Number(event.target.value) || 24)} />}{definition.mode === 'token' && <div className="sm:col-span-2"><Input label="Optional prefix" value={prefix} onChange={(event) => setPrefix(event.target.value)} placeholder="project or environment" /></div>}</div>}
            {!generator && <Textarea label={definition.inputLabel || 'Input'} value={input} onChange={(event) => setInput(event.target.value)} rows={14} placeholder={definition.inputPlaceholder || 'Paste or type here…'} className="font-mono text-xs" />}
            {!generator && <div className="text-xs text-muted-foreground">{input.length.toLocaleString()} characters · {encoder.encode(input).byteLength.toLocaleString()} UTF-8 bytes</div>}
          ...
          <div className="space-y-3"><div className="flex items-center justify-between gap-3"><label className="text-sm font-medium text-foreground">{definition.outputLabel || 'Generated output'}</label><div className="flex flex-wrap justify-end gap-1">{reversible && <Button variant="ghost" size="sm" onClick={reverseOutput} disabled={!output}><ArrowDownUp className="h-3.5 w-3.5" /> Use as input</Button>}<Button variant="ghost" size="sm" onClick={() => void copy()} disabled={!output}>{copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />} {copied ? 'Copied' : 'Copy'}</Button><Button variant="ghost" size="sm" onClick={() => output && downloadText(output, `${definition.mode}-output.txt`)} disabled={!output}><Download className="h-3.5 w-3.5" /> Download</Button></div></div><pre className="surface-card min-h-[20rem] max-h-[32rem] overflow-auto whitespace-pre-wrap break-words rounded-xl border p-4 font-mono text-xs leading-5 text-foreground">{output || 'Output will appear here.'}</pre>{output && <div className="text-xs text-muted-foreground">{output.length.toLocaleString()} characters in result</div>}</div>
        </div>
      </Card>
    </div>
  )
