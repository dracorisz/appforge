...
const encodeHtml = (value: string) => value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");
...
export function UtilityWorkbench() {
  ...
  React.useEffect(() => {
    ...
    setPrefix("");
    setOperation(
      definition.mode === "hash"
        ? "SHA-256"
        : definition.mode === "hex"
          ? "text-to-hex"
          : definition.mode === "token"
            ? "hex"
            : definition.mode === "password"
              ? "mixed"
              : definition.mode === "uuid"
                ? "standard"
                : definition.mode === "jwt"
                  ? "decode"
  ...
  const run = async () => {
    ...
    try {
      ...
      switch (definition.mode) {
        ...
        case "base64":
          next = operation === "decode" ? base64ToUtf8(input) : operation === "url-safe" ? utf8ToBase64(input).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "") : utf8ToBase64(input);
          break;
        case "url":
  ...
  const operationOptions = (() => {
    switch (definition.mode) {
      case "json":
        return [
          ["format", "Format"],
          ["sort", "Format + sort keys"],
          ["minify", "Minify"],
        ];
      case "base64":
        return [
          ["encode", "Encode Base64"],
          ["url-safe", "Encode URL-safe"],
          ["decode", "Decode"],
        ];
      case "url":
        return [
      ...
      case "hex":
        return [
          ...
          ["text-to-spaced-hex", "Text → Spaced hex"],
          ...
          <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                {definition.mode === "token" && (
                  <div className="sm:col-span-2">
                    <Input label="Optional prefix" value={prefix} onChange={(event) => setPrefix(event.target.value)} placeholder="project or environment" />
                  </div>
                )}
              </div>
            )}
            ...
            {!generator && (
              <div className="text-sm text-muted-foreground">
                {input.length.toLocaleString()} characters · {encoder.encode(input).byteLength.toLocaleString()} UTF-8 bytes
              </div>
            )}
            ...
            {error && (
              <div role="alert" className="rounded-xl border border-destructive/25 bg-destructive/5 px-4 py-2 text-sm text-destructive">
                {error}
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap justify-end gap-2">
                {reversible && (
                  <Button variant="ghost" size="sm" onClick={reverseOutput} disabled={!output}>
                    <ArrowDownUp className="h-4 w-4" /> Use as input
                  </Button>
                )}
                <Button variant="ghost" size="sm" onClick={() => void copy()} disabled={!output}>
                  {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />} {copied ? "Copied" : "Copy"}
                </Button>
                <Button variant="ghost" size="sm" onClick={() => output && downloadText(output, `${definition.mode}-output.txt`)} disabled={!output}>
                  <Download className="h-4 w-4" /> Download
                </Button>
              </div>
            </div>
            <pre className="surface-card min-h-[20rem] max-h-[32rem] overflow-auto whitespace-pre-wrap break-words rounded-xl border p-4 font-mono text-sm text-foreground">{output || "Output will appear here."}</pre>
            {output && <div className="text-sm text-muted-foreground">{output.length.toLocaleString()} characters in result</div>}
          </div>
        </div>
      </Card>
    </div>
