    ...
    <div className="w-full space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-4">
          ...
          <div className="mt-2 text-lg font-semibold text-foreground">{active}</div></Card><Card className="p-4"><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Completed</div><div className="mt-2 text-lg font-semibold text-foreground">{completed}</div></Card><Card className="p-4"><div className="flex items-center justify-between gap-4"><div><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Progress</div><div className="mt-2 text-lg font-semibold text-foreground">{progress}%</div></div><div className="text-xs text-muted-foreground">{remoteReady ? 'Cloud ready' : user ? syncing ? 'Syncing' : 'Local fallback' : 'Local-only'}</div></div><div className="mt-4 h-1.5 overflow-hidden rounded-xl bg-muted"><div className="h-full rounded-xl bg-foreground/70 transition-[width]" style={{ width: `${progress}%` }} /></div></Card></div>

      <Card className="p-4 sm:p-4">
        <div className="grid gap-4 sm:grid-cols-5 grid-cols-[minmax(0,1fr)_auto] sm:items-end">
          <div className="col-span-3">
            <label className="mb-2 block text-xs font-medium text-muted-foreground">New task</label>
      ...
      <div className="space-y-2">
        ...
        {!visibleTasks.length && <Card className="flex flex-col items-center justify-center py-8 text-center"><ClipboardList className="h-8 w-8 text-muted-foreground" /><div className="mt-4 text-sm font-medium">{tasks.length ? 'No tasks match this view' : 'No tasks yet'}</div><div className="mt-2 text-xs text-muted-foreground">{tasks.length ? 'Change the filter or search text.' : 'Add your next task to get started.'}</div></Card>}
      </div>
    </div>
  )
}
