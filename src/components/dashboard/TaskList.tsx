...
export function TaskList() {
  ...
  React.useEffect(() => {
    ...
    if (storageOwnerRef.current !== userId) return;
    try {
      saveLocal(userId, tasks);
    } catch {
      setError("Browser storage is full. Remove old tasks or free browser storage before relying on local-only changes.");
    }
  }, [tasks, userId]);
  ...
  const syncFromRemote = React.useCallback(async () => {
    ...
    setError("");
    try {
      const { data, error: readError } = await supabase.from("appforge_tasks").select("id,title,completed,created_at,updated_at").eq("user_id", syncUserId).order("created_at", { ascending: false });
  ...
  const addTask = () => {
    ...
    if (tasks.length >= MAX_TASKS) {
      setError(`Task List supports up to ${MAX_TASKS} local tasks per account.`);
      return;
    }
    const timestamp = now();
  ...
  const rememberDeleted = (ids: string[]) => {
    ...
    try {
      ...
      setError("Could not record local deletions. Sync again before closing the app.");
    }
  };
  ...
  const clearCompleted = async () => {
    ...
    if (deleteError) setError(deleteError.message);
      ...
      const deleted = loadDeleted(userId);
      completedIds.forEach((id) => deleted.delete(id));
      saveDeleted(userId, deleted);
    }
  };
    ...
    <div className="w-full space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-4">
          ...
          <div className="mt-2 text-lg font-semibold text-foreground">{active}</div>
        </Card>
        <Card className="p-4">
          <div className="text-sm font-medium uppercase tracking-wide text-muted-foreground">Completed</div>
          <div className="mt-2 text-lg font-semibold text-foreground">{completed}</div>
        </Card>
        <Card className="p-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="text-sm font-medium uppercase tracking-wide text-muted-foreground">Progress</div>
              <div className="mt-2 text-lg font-semibold text-foreground">{progress}%</div>
            </div>
            <div className="text-sm text-muted-foreground">{remoteReady ? "Cloud ready" : user ? (syncing ? "Syncing" : "Local fallback") : "Local-only"}</div>
          </div>
          <div className="mt-4 h-1.5 overflow-hidden rounded-xl bg-muted">
            <div className="h-full rounded-xl bg-foreground/70 transition-[width]" style={{ width: `${progress}%` }} />
          </div>
        </Card>
      </div>
      ...
      <Card className="p-4 sm:p-4">
        <div className="grid gap-4 sm:grid-cols-5 grid-cols-[minmax(0,1fr)_auto] sm:items-end">
          ...
          <Button variant="secondary" onClick={() => void syncFromRemote()} disabled={!user || syncing} className="self-start sm:self-auto">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
          <label className="relative block">
            ...
            <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tasks…" className="pl-8" aria-label="Search tasks" />
          </label>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => void completeAll()} disabled={!active}>
        ...
        {visibleTasks.map((task) => (
          <Card key={task.id} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 p-4 sm:p-4">
            ...
            <div className="flex items-center gap-2">
              {editingId === task.id ? (
                <>
                  <Button variant="ghost" size="sm" onClick={() => saveEdit(task)} disabled={!editingTitle.trim()} title="Save task">
                ...
                <Button variant="ghost" size="sm" onClick={() => beginEdit(task)} title="Edit task">
                  <Pencil className="h-4 w-4" />
                </Button>
              )}
              <Button variant="ghost" size="sm" onClick={() => void remove(task)} title="Delete task">
