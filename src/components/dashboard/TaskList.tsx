    ...
    <div className="w-full space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-4">
          ...
          <div className="text-sm font-medium uppercase tracking-wide text-muted-foreground">Active</div>
          <div className="mt-2 text-lg font-semibold text-foreground">{active}</div></Card><Card className="p-4"><div className="text-sm font-medium uppercase tracking-wide text-muted-foreground">Completed</div><div className="mt-2 text-lg font-semibold text-foreground">{completed}</div></Card><Card className="p-4"><div className="flex items-center justify-between gap-4"><div><div className="text-sm font-medium uppercase tracking-wide text-muted-foreground">Progress</div><div className="mt-2 text-lg font-semibold text-foreground">{progress}%</div></div><div className="text-sm text-muted-foreground">{remoteReady ? 'Cloud ready' : user ? syncing ? 'Syncing' : 'Local fallback' : 'Local-only'}</div></div><div className="mt-4 h-1.5 overflow-hidden rounded-xl bg-muted"><div className="h-full rounded-xl bg-foreground/70 transition-[width]" style={{ width: `${progress}%` }} /></div></Card></div>

      <Card className="p-4 sm:p-4">
        <div className="grid gap-4 sm:grid-cols-5 grid-cols-[minmax(0,1fr)_auto] sm:items-end">
          <div className="col-span-3">
            <label className="mb-2 block text-sm font-medium text-muted-foreground">New task</label>
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center"><label className="relative block"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tasks…" className="pl-8" aria-label="Search tasks" /></label><div className="flex flex-wrap items-center gap-2">{(['all', 'active', 'completed'] as Filter[]).map((value) => <button key={value} type="button" onClick={() => setFilter(value)} className={`rounded-xl border px-4 py-2 text-sm font-medium capitalize transition-colors ${filter === value ? 'bg-accent text-foreground' : 'text-muted-foreground hover:bg-accent/60 hover:text-foreground'}`}>{value}</button>)}<Button variant="ghost" size="sm" onClick={() => void completeAll()} disabled={!active}><Check className="h-4 w-4" /> Complete all</Button><Button variant="ghost" size="sm" onClick={() => void clearCompleted()} disabled={!completed}><Trash2 className="h-4 w-4" /> Clear completed</Button><select value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)} className="min-h-9 rounded-xl border border-input bg-background px-2 text-sm"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="active-first">Active first</option></select></div></div>
        <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-muted-foreground"><span>{active} active</span><span>·</span><span>{completed} completed</span><span>·</span><span>{tasks.length} total</span><span>·</span><span>{visibleTasks.length} shown</span></div>
      </Card>

      <div className="space-y-2">
        {visibleTasks.map((task) => <Card key={task.id} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 p-4 sm:p-4"><button type="button" onClick={() => toggle(task)} className="shrink-0 rounded-xl p-2 text-muted-foreground hover:text-foreground" aria-label={task.completed ? `Mark ${task.title} active` : `Complete ${task.title}`}>{task.completed ? <CheckCircle2 className="h-5 w-5" /> : <Circle className="h-5 w-5" />}</button>{editingId === task.id ? <Input autoFocus maxLength={MAX_TITLE} value={editingTitle} onChange={(event) => setEditingTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') saveEdit(task); if (event.key === 'Escape') { setEditingId(null); setEditingTitle('') } }} aria-label={`Edit ${task.title}`} /> : <button type="button" onClick={() => toggle(task)} className={`min-w-0 break-words text-left text-sm leading-5 ${task.completed ? 'text-muted-foreground line-through' : 'text-foreground'}`}>{task.title}</button>}<div className="flex items-center gap-2">{editingId === task.id ? <><Button variant="ghost" size="sm" onClick={() => saveEdit(task)} disabled={!editingTitle.trim()} title="Save task"><Check className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => { setEditingId(null); setEditingTitle('') }} title="Cancel editing"><X className="h-4 w-4" /></Button></> : <Button variant="ghost" size="sm" onClick={() => beginEdit(task)} title="Edit task"><Pencil className="h-4 w-4" /></Button>}<Button variant="ghost" size="sm" onClick={() => void remove(task)} title="Delete task"><Trash2 className="h-4 w-4" /></Button></div></Card>)}
        {!visibleTasks.length && <Card className="flex flex-col items-center justify-center py-8 text-center"><ClipboardList className="h-8 w-8 text-muted-foreground" /><div className="mt-4 text-sm font-medium">{tasks.length ? 'No tasks match this view' : 'No tasks yet'}</div><div className="mt-2 text-sm text-muted-foreground">{tasks.length ? 'Change the filter or search text.' : 'Add your next task to get started.'}</div></Card>}
      </div>
    </div>
  )
}
