...
export function ColorPickerTool() {
  ...
  const drawImage = async (file: File) => {
    ...
    image.onload = () => {
      ...
      if (!canvas) { URL.revokeObjectURL(url); setError('Canvas is unavailable in this browser.'); return }
      const maxDimension = 1600
      const scale = Math.min(1, maxDimension / Math.max(image.naturalWidth, image.naturalHeight))
      canvas.width = Math.max(1, Math.round(image.naturalWidth * scale))
      canvas.height = Math.max(1, Math.round(image.naturalHeight * scale))
      const context = canvas.getContext('2d', { willReadFrequently: true })
      if (!context) { URL.revokeObjectURL(url); setError('Canvas is unavailable in this browser.'); return }
  ...
  const pickFromCanvas = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    } catch { setError('Could not sample that pixel.') }
  }
    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <Card className="p-4">
          ...
          <input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/avif" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void drawImage(file); event.currentTarget.value = '' }} />
        ...
        <Card className="p-4">
          ...
          <div className="mt-4 aspect-[16/8] rounded-xl border border-border/70 shadow-inner" style={{ backgroundColor: /^#[0-9A-F]{6}$/i.test(hex) ? hex : '#5B6CFF' }} />
