      ...
      {capture && (
        <div className="mt-4 overflow-hidden rounded-xl border bg-background/45">
          ...
          <div className="flex flex-col gap-4 border-t p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground"><span className="font-medium text-foreground">{capture.width} × {capture.height}</span> · {capture.source === 'screen' ? 'browser capture' : 'imported fallback'}</div>
            <div className="flex flex-wrap gap-2">
