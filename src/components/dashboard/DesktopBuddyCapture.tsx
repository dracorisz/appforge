...
export function DesktopBuddyCapture() {
  ...
  return (
    <section className="mx-auto mt-4 w-full max-w-6xl rounded-xl border bg-card p-4 md:p-4" aria-label="Desktop Buddy screenshot capture">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-xs leading-5 text-muted-foreground">Use the browser screen-share API on supported desktop browsers. If Brave or an installed PWA blocks it, import a screenshot instead and continue with the same download/Media Vault workflow.</p>
        </div>
      ...
      {capture && (
        <div className="mt-4 overflow-hidden rounded-xl border bg-background/45">
          ...
          <div className="flex flex-col gap-4 border-t p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => downloadCapture(capture)} className="inline-flex min-h-9 items-center gap-2 rounded-xl border px-4 text-xs font-semibold hover:bg-accent"><Download className="h-3.5 w-3.5" /> Download PNG</button>
              <button type="button" disabled={Boolean(busy)} onClick={() => void saveToVault()} className="inline-flex min-h-9 items-center gap-2 rounded-xl border px-4 text-xs font-semibold hover:bg-accent disabled:opacity-50">{busy === 'save' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />} Save to Media Vault</button>
              <span className="inline-flex min-h-9 items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/5 px-4 text-xs text-emerald-600 dark:text-emerald-400"><Check className="h-3.5 w-3.5" /> Local preview</span>
            </div>
          </div>
        </div>
      )}
