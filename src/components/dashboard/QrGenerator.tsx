  ...
  return (
    <div className="w-full space-y-5 pb-10">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <Card className="p-5">
          <div className="flex gap-1 rounded-xl border border-border/70 bg-background/40 p-1"><button type="button" onClick={() => changeMode('text')} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${mode === 'text' ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>Text / URL</button><button type="button" onClick={() => changeMode('wifi')} className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${mode === 'wifi' ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>Wi-Fi</button></div>

          {mode === 'text' ? <div className="mt-5"><label className="text-xs font-medium text-muted-foreground">Content<textarea value={value} onChange={(event) => { setValue(event.target.value); setError('') }} rows={6} className="mt-1.5 w-full rounded-xl border border-input bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-ring/25" placeholder="URL or text" /></label></div> : <div className="mt-5 grid gap-4 sm:grid-cols-2"><Input label="Network name" value={ssid} onChange={(event) => { setSsid(event.target.value); setError('') }} /><Input label="Password" type="password" value={password} onChange={(event) => { setPassword(event.target.value); setError('') }} disabled={security === 'nopass'} /><label className="text-xs font-medium text-muted-foreground">Security<select value={security} onChange={(event) => { setSecurity(event.target.value as typeof security); setError('') }} className="mt-1.5 h-10 w-full rounded-lg border border-input bg-background px-3 text-sm text-foreground"><option value="WPA">WPA / WPA2</option><option value="WEP">WEP</option><option value="nopass">Open network</option></select></label><label className="flex items-end gap-2 pb-2 text-sm text-muted-foreground"><input type="checkbox" checked={hidden} onChange={(event) => setHidden(event.target.checked)} /> Hidden network</label></div>}

          <div className="mt-5 grid gap-4 sm:grid-cols-3"><label className="text-xs font-medium text-muted-foreground">Size<select value={size} onChange={(event) => setSize(Number(event.target.value))} className="mt-1.5 h-10 w-full rounded-lg border border-input bg-background px-3 text-sm text-foreground">{[192,256,320,512,768].map((option) => <option key={option} value={option}>{option}px</option>)}</select></label><label className="text-xs font-medium text-muted-foreground">Margin<select value={margin} onChange={(event) => setMargin(Number(event.target.value))} className="mt-1.5 h-10 w-full rounded-lg border border-input bg-background px-3 text-sm text-foreground">{[0,1,2,4,8].map((option) => <option key={option} value={option}>{option}</option>)}</select></label><label className="text-xs font-medium text-muted-foreground">Format<select value={format} onChange={(event) => setFormat(event.target.value as Format)} className="mt-1.5 h-10 w-full rounded-lg border border-input bg-background px-3 text-sm text-foreground"><option value="png">PNG</option><option value="svg">SVG</option></select></label></div>
        ...
        <Card className="flex min-h-[420px] items-center justify-center p-5">
          {url ? <div className="w-full text-center"><div className="mx-auto flex aspect-square w-full max-w-[360px] items-center justify-center rounded-2xl bg-white p-4"><img src={url} alt="Generated QR code" className="h-full w-full object-contain" onError={() => setError('The QR preview provider is unavailable. Your content is still editable.')} /></div><p className="mt-3 text-xs text-muted-foreground">Generated by api.qrserver.com. QR content is sent to that provider.</p></div> : <div className="text-center text-sm text-muted-foreground"><QrCode className="mx-auto h-12 w-12 opacity-40" /><p className="mt-3">Add content to generate a code.</p></div>}
        </Card>
      </div>
    </div>
  )
