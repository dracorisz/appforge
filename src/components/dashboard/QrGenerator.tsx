    ...
    <div className="w-full space-y-5 pb-10">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <Card className="p-5">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2"><Button variant="secondary" onClick={() => void copyPayload()} disabled={!payload}><Copy className="h-4 w-4" /> {copied ? 'Copied' : 'Copy encoded value'}</Button>{url && <a href={url} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-border/70 bg-background px-3 py-2 text-sm font-medium shadow-sm transition-[background-color,box-shadow] hover:bg-accent hover:shadow-md"><ExternalLink className="h-4 w-4" /> Open image</a>}</div>
        ...
        <Card className="flex min-h-[420px] items-center justify-center p-5">
          {url ? <div className="w-full text-center"><div className="mx-auto flex aspect-square w-full max-w-[360px] items-center justify-center rounded-2xl bg-white p-4 shadow-sm"><img src={url} alt="Generated QR code" className="h-full w-full object-contain" onError={() => setError('The QR preview provider is unavailable. Your content is still editable.')} /></div><p className="mt-3 text-xs text-muted-foreground">Generated by api.qrserver.com. QR content is sent to that provider.</p></div> : <div className="text-center text-sm text-muted-foreground"><QrCode className="mx-auto h-12 w-12 opacity-40" /><p className="mt-3">Add content to generate a code.</p></div>}
        </Card>
      </div>
    </div>
  )
