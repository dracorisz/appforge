...
export function QrGenerator() {
  ...
  const url = React.useMemo(() => {
    if (!payload) return "";
    const params = new URLSearchParams({ data: payload, size: `${size}x${size}`, margin: String(margin), format, ecc });
    params.set("_", String(nonce));
    return `https://api.qrserver.com/v1/create-qr-code/?${params.toString()}`;
  }, [ecc, format, margin, nonce, payload, size]);
        ...
        <Card className="p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div className="flex flex-wrap gap-2 sm:ml-auto">
              ...
              <Button onClick={() => void download()} disabled={!url || busy}>
                <Download className="h-4 w-4" /> {busy ? "Downloading…" : "Download"}
              </Button>
            </div>
          </div>
          {mode === "text" ? (
            <div className="mt-4">
              <div className="mb-4 flex flex-wrap gap-2">
                <Button variant="secondary" size="sm" onClick={() => preset("https://")}>
                  <ExternalLink className="h-4 w-4" /> URL
                </Button>
                <Button variant="secondary" size="sm" onClick={() => preset("mailto:hello@example.com")}>
                  <Mail className="h-4 w-4" /> Email
                </Button>
                <Button variant="secondary" size="sm" onClick={() => preset("tel:+381")}>
              ...
              <div className="mt-2 flex justify-between text-sm text-muted-foreground">
                <span>{payload.length.toLocaleString()} characters</span>
                <span>{density} payload</span>
              </div>
            </div>
          ) : (
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-4">
            <Select label="Size" value={size} onChange={(event) => setSize(Number(event.target.value))}>
              {[192, 256, 320, 512, 768].map((option) => (
                <option key={option} value={option}>
                  {option}px
                </option>
              ))}
            </Select>
            <Select label="Margin" value={margin} onChange={(event) => setMargin(Number(event.target.value))}>
            ...
            <Select label="Error correction" value={ecc} onChange={(event) => setEcc(event.target.value as Ecc)}>
              <option value="L">L · 7%</option>
              <option value="M">M · 15%</option>
              <option value="Q">Q · 25%</option>
              <option value="H">H · 30%</option>
            </Select>
            <Select label="Format" value={format} onChange={(event) => setFormat(event.target.value as Format)}>
              <option value="png">PNG</option>
              <option value="svg">SVG</option>
            </Select>
          </div>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <Button variant="secondary" onClick={() => void copyPayload()} disabled={!payload}>
          ...
          {payload.length > 1200 && <div className="mt-4 rounded-xl border border-warning/30 bg-warning/10 px-4 py-2 text-sm text-warning dark:text-warning">Large QR payloads become harder to scan. Consider shortening the content or increasing size/error correction.</div>}
          {error && (
            <div role="alert" className="mt-4 flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-2 text-sm text-destructive">
        ...
        <Card className="flex min-h-[420px] items-center justify-center p-4">
          {url ? (
            <div className="w-full text-center">
              <div className="mx-auto flex aspect-square w-full max-w-[360px] items-center justify-center rounded-xl bg-inverse p-4">
                <img src={url} alt="Generated QR code" className="h-full w-full object-contain" onError={() => setError("The QR preview provider is unavailable. Your content is still editable.")} />
              </div>
              <p className="mt-4 text-sm text-muted-foreground">
                {size}px · ECC {ecc} · {format.toUpperCase()} · generated by api.qrserver.com. QR content is sent to that provider.
              </p>
            </div>
          ) : (
            <div className="text-center text-sm text-muted-foreground">
              <QrCode className="mx-auto h-12 w-12 opacity-40" />
              <p className="mt-4">Add content to generate a code.</p>
            </div>
          )}
        </Card>
      </div>
