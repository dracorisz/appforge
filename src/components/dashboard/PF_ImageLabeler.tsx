...
export function PF_ImageLabeler() {
  ...
  return (
    <div className="space-y-4">
      <Card>
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-xs text-muted-foreground">Images stay in your browser. Folder identity is included in local label keys so similarly named files from different datasets do not overwrite each other.</p>
          </div>
        ...
        {images.length > 0 && <div className="mt-4 grid gap-4 sm:grid-cols-3"><div className="rounded-xl border border-border bg-muted/30 p-4"><p className="text-xs text-muted-foreground">Images</p><p className="mt-2 text-lg font-semibold text-foreground">{images.length}</p></div><div className="rounded-xl border border-border bg-muted/30 p-4"><p className="text-xs text-muted-foreground">Labeled</p><p className="mt-2 text-lg font-semibold text-foreground">{labeledCount}</p></div><div className="rounded-xl border border-border bg-muted/30 p-4"><p className="text-xs text-muted-foreground">Approved</p><p className="mt-2 text-lg font-semibold text-foreground">{approvedCount} <span className="text-xs font-normal text-muted-foreground">({completion}%)</span></p></div></div>}
      </Card>

      {currentImage ? (
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><Badge color="slate">{currentIndex + 1} / {images.length}</Badge><span className="max-w-[55vw] truncate text-xs text-muted-foreground">{currentImage.path}</span></div><div className="flex gap-2"><Button variant="ghost" size="sm" onClick={() => goTo(currentIndex - 1)} disabled={currentIndex === 0}><ChevronLeft className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => goTo(currentIndex + 1)} disabled={currentIndex === images.length - 1}><ChevronRight className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={saveCurrentImage} title="Download original"><Download className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => setLightbox(currentImage.url)} title="Full screen"><Maximize2 className="h-4 w-4" /></Button></div></div>
            ...
            <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted-foreground"><span>{currentImage.name}</span><span>{formatBytes(currentImage.size)}</span><span>{currentImage.type || 'image'}</span></div>
          </Card>
          ...
          <div className="space-y-4">
            <Card>
              ...
              <div className="mt-2 flex flex-wrap gap-2">{(currentLabel?.tags || []).map((tag) => <button key={tag} onClick={() => removeTag(tag)} className="inline-flex items-center gap-2 rounded-xl bg-muted px-2 py-2 text-xs text-foreground hover:bg-accent">{tag}<X className="h-3 w-3 text-muted-foreground" /></button>)}</div>
              <div className="mt-4 grid gap-2 sm:grid-cols-2"><Button variant={currentLabel?.approved ? 'secondary' : 'primary'} onClick={() => updateLabel({ approved: !currentLabel?.approved })}><Check className="h-4 w-4" /> {currentLabel?.approved ? 'Approved' : 'Approve image'}</Button><Button variant="secondary" onClick={approveAndNext} disabled={currentIndex >= images.length - 1 && Boolean(currentLabel?.approved)}>Approve & next</Button></div><p className="mt-2 text-xs text-muted-foreground">Keyboard: ← / → navigate · A approves and advances.</p>
            </Card>
            ...
            <Card>
              ...
              <div className="mt-4 max-h-[340px] space-y-2 overflow-y-auto pr-2">{images.map((image, index) => { const item = labels[image.id]; return <button key={image.id} onClick={() => goTo(index)} className={`flex w-full items-center gap-2 rounded-xl border px-2 py-2 text-left text-xs ${index === currentIndex ? 'border-foreground/30 bg-accent' : 'border-transparent hover:bg-accent/50'}`}><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-xl ${item?.approved ? 'bg-emerald-500/15 text-emerald-500' : 'bg-muted text-muted-foreground'}`}>{item?.approved ? <Check className="h-3 w-3" /> : index + 1}</span><span className="min-w-0 flex-1 truncate text-foreground">{image.path}</span></button> })}</div>
      ...
      {lightbox && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setLightbox(null)}><button onClick={() => setLightbox(null)} className="absolute right-4 top-4 rounded-xl bg-black/40 p-2 text-white hover:bg-black/60" aria-label="Close preview"><X className="h-6 w-6" /></button><img src={lightbox} alt="Full-size preview" className="max-h-[92vh] max-w-[96vw] object-contain" /></div>}
    </div>
  )
}
