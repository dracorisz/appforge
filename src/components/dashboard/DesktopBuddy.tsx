      ...
      <section className="overflow-hidden rounded-xl border bg-card">
        <div className="grid lg:grid-cols-[1.05fr_.95fr]">
          <div className="space-y-4 p-8"><AppHeading />
            <div className="grid gap-4 sm:grid-cols-2"><label className="rounded-xl border p-4"><span className="text-xs font-medium text-muted-foreground">Name</span><input className="mt-2 w-full bg-transparent text-sm outline-none" value={config.name} maxLength={64} onChange={(event) => setConfig((current) => ({ ...current, name: event.target.value }))} /></label><div className="rounded-xl border p-4"><span className="text-xs font-medium text-muted-foreground">State</span><p className="mt-2 text-sm capitalize">{activity}</p></div></div>
          ...
          <div className="relative min-h-[390px] overflow-hidden border-t bg-muted/40 lg:border-l lg:border-t-0"><div className={`absolute inset-0 flex items-end justify-center overflow-hidden p-4 transition-transform ${activity === 'speaking' ? 'scale-[1.025]' : activity === 'listening' ? 'scale-[1.01]' : ''}`}><img src={config.imageDataUrl} alt={`${config.name || 'Desktop Buddy'} character`} className="max-h-[350px] max-w-[92%] select-none object-contain" style={{ transform: `translate(${config.offsetX}px, ${config.offsetY}px) scale(${config.scale / 100})` }} /></div><div className="absolute left-4 top-4 rounded-xl border bg-background/90 px-4 py-2 text-xs backdrop-blur"><span className="font-semibold">{config.name || 'Buddy'}</span> · {activity}</div></div>
        </div>
      </section>

      <section className="space-y-4 rounded-xl border bg-card p-4"><div className="flex flex-wrap items-end justify-between gap-4"><div><h2 className="font-semibold">KDE dragon starters</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">Starter artwork keeps source and license metadata inside the buddy pack.</p></div><a href="https://community.kde.org/Promo/Material/Mascots" target="_blank" rel="noreferrer" className="inline-flex cursor-pointer items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground">KDE mascot wiki <ExternalLink className="h-4 w-4" /></a></div><div className="grid gap-4 sm:grid-cols-3">{KDE_STARTERS.map((starter) => { const active = config.assetSourceUrl === starter.sourceUrl; return <button key={starter.id} type="button" onClick={() => chooseStarter(starter)} className={`flex h-full min-h-24 cursor-pointer flex-col overflow-hidden rounded-xl border text-left transition-colors hover:border-foreground/25 ${active ? 'ring-1 ring-primary/35' : ''}`}><div className="flex h-48 w-full items-center justify-center border-b border-border/60 bg-muted/20"><img src={starter.imageUrl} alt={starter.name} className="h-full w-full scale-90 object-contain" loading="lazy" /></div><div className="flex flex-1 flex-col p-4"><div className="text-sm font-semibold">{starter.name}</div><p className="mt-2 text-[11px] leading-4 text-muted-foreground">{starter.note}</p><p className="mt-4 text-[10px] leading-4 text-muted-foreground">{starter.license}</p></div></button> })}</div><div className="rounded-xl border bg-background/45 p-4 text-xs leading-5 text-muted-foreground"><strong className="text-foreground">Current asset:</strong> {config.assetLabel || 'Custom character'} · {config.assetLicense || 'No license metadata recorded'}{config.assetSourceUrl && <> · <a href={config.assetSourceUrl} target="_blank" rel="noreferrer" className="cursor-pointer underline underline-offset-2">source</a></>}</div></section>
