    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 md:p-6">
      ...
      <section className="space-y-4 rounded-xl border bg-card p-5 md:p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-3">{KDE_STARTERS.map((starter) => {
          ...
          return <button key={starter.id} type="button" onClick={() => chooseStarter(starter)} className={`flex h-full min-h-24  flex-col overflow-hidden rounded-xl border text-left transition-colors hover:border-foreground/25 ${active ? 'ring-1 ring-primary/35' : ''}`}>
            <div className="flex items-center justify-center h-44 w-full bg-muted/20 border-b border-border/60 sm:h-48">
              <img src={starter.imageUrl} alt={starter.name} className="h-full w-full object-contain scale-90" loading="lazy" />
            </div>
            <div className="flex flex-1 flex-col space-y-1 p-3">
              <div className="text-sm font-semibold">{starter.name}</div>
              <p className="text-[11px] leading-4 text-muted-foreground">{starter.note}</p>
              <p className="mt-auto pt-2 text-[10px] leading-4 text-muted-foreground">{starter.license}</p>
            </div>
          </button>
        })}</div>
