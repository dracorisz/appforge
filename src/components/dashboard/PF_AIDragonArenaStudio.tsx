  ...
  return (
    <div className="mx-auto max-w-6xl space-y-3 pb-10 text-foreground" style={themeStyle} data-story-theme={storyTheme}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-1">
        <Tabs tabs={[{ id: 'novel', label: 'Novel', icon: BookOpen }, { id: 'comics', label: 'Comics', icon: GalleryThumbnails }]} active={mode} onChange={(id) => setMode(id as BuilderMode)} ariaLabel="Story format" />
      ...
      <Card className="overflow-hidden p-0">
        ...
        {showAppearance && <section id="story-appearance" className="border-b border-border/70 bg-background/55 p-3" aria-label="Story appearance">
          ...
          <div className="mt-3 flex flex-wrap gap-2">{(Object.keys(STORY_THEMES) as StoryTheme[]).map((theme) => <button key={theme} type="button" aria-pressed={storyTheme === theme} onClick={() => selectStoryTheme(theme)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-medium focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring ${storyTheme === theme ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:bg-accent'}`}><span aria-hidden="true" className={`h-3 w-3 rounded-xl border border-current ${STORY_THEMES[theme].swatch}`} />{STORY_THEMES[theme].label}</button>)}</div>
        </section>}

        {showProviderSettings && <div className="border-b border-border/70 bg-background/55 p-3"><div className="mb-2"><div className="text-xs font-semibold">Personal Hugging Face tokens</div><div className="mt-1 text-[11px] leading-4 text-muted-foreground">Optional. Add up to three tokens with Inference Providers access. Story Studio rotates them before server-funded keys. Manage Gemini and OpenRouter keys in Settings → Integrations. Gemini is a text fallback; scene generation uses Hugging Face.</div></div><div className="grid gap-2 md:grid-cols-3">{hfTokens.map((value, index) => <Input type="password" autoComplete="off" aria-label={`Hugging Face token ${index + 1}`} key={index} value={value} onChange={(event) => setHfTokens((current) => current.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} placeholder={`HF token ${index + 1} · hf_…`} />)}</div><Link to="/settings?tab=integrations" className="mt-3 inline-flex rounded-xl border border-border px-3 py-2 text-xs font-medium hover:bg-accent focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">Manage AI provider keys</Link></div>}

        {panel && <div className="border-b border-border/70 bg-background/70 p-3">
          {panel === 'assets' && <div className="flex gap-2 overflow-x-auto pb-1">{assets.length === 0 ? <div className="text-xs text-muted-foreground">No generated scenes yet.</div> : assets.map((asset) => { const src = sceneAssetUrl(asset); return <div key={asset.id} className="w-28 shrink-0"><button type="button" onClick={() => src && setLightbox(src)} className="w-full text-left"><div className="h-16 overflow-hidden rounded-xl border border-border bg-muted">{src && <img src={src} alt="" className="h-full w-full object-cover" />}</div><div className="mt-1 truncate text-[10px] text-muted-foreground">{sceneModel(asset)}</div></button><button type="button" disabled={sharingAsset === asset.id} onClick={() => void togglePublic(asset)} className={`mt-1 inline-flex w-full items-center justify-center gap-1 rounded-xl border px-1.5 py-1 text-[10px] ${asset.is_public ? 'border-primary/30 bg-accent text-foreground' : 'border-border text-muted-foreground hover:text-foreground'}`}>{asset.is_public ? <><Globe2 className="h-3 w-3" /> Public</> : <><LockKeyhole className="h-3 w-3" /> Private</>}</button></div> })}</div>}
          {panel === 'sessions' && <div className="flex gap-2 overflow-x-auto pb-1">{sessions.length === 0 ? <div className="text-xs text-muted-foreground">No saved stories yet.</div> : sessions.map((session) => <button key={session.id} type="button" onClick={() => void loadSession(session)} className={`min-w-44 rounded-xl border px-3 py-2 text-left text-xs ${session.id === sessionId ? 'border-primary/50 bg-accent' : 'border-border hover:bg-accent/60'}`}><div className="truncate font-medium">{session.title || 'Untitled story'}</div><div className="mt-1 text-muted-foreground">{session.turn_count} turns</div></button>)}</div>}
          {panel === 'leaderboard' && <div className="grid gap-1 sm:grid-cols-2 lg:grid-cols-4">{leaderboardRows.slice(0, 8).map((row, index) => <div key={row.user_id} className="flex items-center justify-between rounded-xl border border-border/70 px-3 py-2 text-xs"><span>#{index + 1} {row.display_name || 'Writer'}</span><span className="text-muted-foreground">{row.points}</span></div>)}</div>}
        </div>}
            ...
            {history.map((item, index) => {
              ...
              return <div key={`${item.role}-${index}`} className={`flex ${item.role === 'player' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex max-w-[92%] items-start gap-2 ${item.role === 'player' ? 'flex-row-reverse' : ''}`}>
                  <div className={`rounded-xl px-3.5 py-2.5 text-sm leading-5 ${item.role === 'player' ? 'bg-primary text-primary-foreground' : 'border border-border/65 bg-card/90'}`}>
                    {paragraphize(item.text).map((paragraph, paragraphIndex) => <p key={paragraphIndex} className={paragraphIndex ? 'mt-2' : ''}>{paragraph}</p>)}
                  </div>
                  {attachedSrc && <button type="button" onClick={() => setLightbox(attachedSrc)} className={`group shrink-0 overflow-hidden rounded-xl border border-border/70 bg-muted ${mode === 'comics' ? 'h-24 w-32 sm:h-28 sm:w-40' : 'h-16 w-20 sm:h-20 sm:w-28'}`} title="Open scene"><img src={attachedSrc} alt={`Scene for turn ${sceneTurn(attachedScene!) || ''}`} className="h-full w-full object-cover transition-transform group-hover:scale-105" /></button>}
                </div>
              </div>
            })}
        ...
        <div className="space-y-2 border-t border-border/70 bg-card p-3 sm:p-4">
          ...
          {imageDailyUsed && !personalHfTokens.length && <div className="rounded-xl border border-border/70 bg-background/50 px-3 py-2 text-[11px] text-muted-foreground">Today’s shared image turn is used. Add a personal Hugging Face token to continue generating artwork.</div>}
          ...
          <div className="flex gap-2"><input value={action} disabled={loading} onChange={(event) => setAction(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void play() }} placeholder={mode === 'novel' ? 'Write your next move…' : 'Direct the next panel…'} className="h-10 min-w-0 flex-1 rounded-xl border border-input bg-background px-3 text-sm outline-none focus:ring-1 focus:ring-ring" /><Button disabled={loading || !action.trim()} onClick={() => void play()}><Send className="h-4 w-4" /> Act</Button></div>
        </div>
      </Card>

      <div className="grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-border/60 bg-card/50 px-3 py-2 text-[11px] text-muted-foreground"><span className="font-medium text-foreground">Novel:</span> writing-first story flow with Markdown export.</div><div className="rounded-xl border border-border/60 bg-card/50 px-3 py-2 text-[11px] text-muted-foreground"><span className="font-medium text-foreground">Comics:</span> larger beat-linked panels with standalone HTML export.</div><div className="rounded-xl border border-border/60 bg-card/50 px-3 py-2 text-[11px] text-muted-foreground"><span className="font-medium text-foreground">Appearance:</span> choose an app theme from the palette button, or inherit your workspace appearance.</div></div>

      {lightbox && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4" role="dialog" aria-modal="true" onClick={() => setLightbox(null)}><button type="button" onClick={() => setLightbox(null)} className="absolute right-4 top-4 rounded-xl bg-black/50 p-2 text-white"><X className="h-5 w-5" /></button><img src={lightbox} alt="Generated story scene" className="max-h-[88vh] max-w-[92vw] rounded-xl object-contain" onClick={(event) => event.stopPropagation()} /></div>}
    </div>
  )
}
