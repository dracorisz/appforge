    ...
    <div className="mx-auto max-w-6xl space-y-3 pb-10 text-foreground" style={themeStyle} data-story-theme={storyTheme}>
      ...
      <Card className="overflow-hidden p-0">
        ...
        {showAppearance && <section id="story-appearance" className="border-b border-border/70 bg-background/55 p-3" aria-label="Story appearance">
          ...
          <div className="mt-3 flex flex-wrap gap-2">{(Object.keys(STORY_THEMES) as StoryTheme[]).map((theme) => <button key={theme} type="button" aria-pressed={storyTheme === theme} onClick={() => selectStoryTheme(theme)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${storyTheme === theme ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:bg-accent'}`}><span aria-hidden="true" className={`h-3 w-3 rounded-xl border border-current ${STORY_THEMES[theme].swatch}`} />{STORY_THEMES[theme].label}</button>)}</div>
        </section>}

        {showProviderSettings && <div className="border-b border-border/70 bg-background/55 p-3"><div className="mb-2"><div className="text-xs font-semibold">Personal Hugging Face tokens</div><div className="mt-1 text-[11px] leading-4 text-muted-foreground">Optional. Add up to three tokens with Inference Providers access. Story Studio rotates them before server-funded keys. Manage Gemini and OpenRouter keys in Settings → Integrations. Gemini is a text fallback; scene generation uses Hugging Face.</div></div><div className="grid gap-2 md:grid-cols-3">{hfTokens.map((value, index) => <Input type="password" autoComplete="off" aria-label={`Hugging Face token ${index + 1}`} key={index} value={value} onChange={(event) => setHfTokens((current) => current.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} placeholder={`HF token ${index + 1} · hf_…`} />)}</div><Link to="/settings?tab=integrations" className="mt-3 inline-flex rounded-xl border border-border px-3 py-2 text-xs font-medium hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Manage AI provider keys</Link></div>}
