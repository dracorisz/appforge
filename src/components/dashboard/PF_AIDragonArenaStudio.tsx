            ...
            {history.map((item, index) => {
              ...
              return <div key={`${item.role}-${index}`} className={`flex ${item.role === 'player' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex max-w-[92%] items-start gap-2 ${item.role === 'player' ? 'flex-row-reverse' : ''}`}>
                  ...
                  {attachedSrc && <button type="button" onClick={() => setLightbox(attachedSrc)} className={`group shrink-0 overflow-hidden rounded-lg border border-border/70 bg-muted ${mode === 'comics' ? 'h-24 w-32 sm:h-28 sm:w-40' : 'h-16 w-20 sm:h-20 sm:w-28'}`} title="Open scene"><img src={attachedSrc} alt={`Scene for turn ${sceneTurn(attachedScene!) || ''}`} className="h-full w-full object-cover transition-transform group-hover:scale-105" /></button>}
                </div>
              </div>
            })}
