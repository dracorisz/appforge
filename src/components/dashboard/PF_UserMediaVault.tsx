...
const VaultThumb = ({ item }: { item: VaultMedia }) => {
  ...
  if (item.kind === 'video' && ytId) return <><img src={metadataUrl(item, 'thumbnail') || `https://i.ytimg.com/vi/${ytId}/hqdefault.jpg`} alt={item.title || 'YouTube video'} loading="lazy" className="h-full w-full object-cover" /><span className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border border-white/20 bg-black/55 text-white"><Play className="ml-2 h-5 w-5" /></span></>
  if (item.kind === 'video' && url) return <><video src={url} muted preload="metadata" className="h-full w-full object-cover" /><span className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border border-white/20 bg-black/55 text-white"><Play className="ml-2 h-5 w-5" /></span></>
