...
const VaultThumb = ({ item }: { item: VaultMedia }) => {
  ...
  if (item.kind === 'video' && ytId) return <><img src={metadataUrl(item, 'thumbnail') || `https://i.ytimg.com/vi/${ytId}/hqdefault.jpg`} alt={item.title || 'YouTube video'} loading="lazy" className="h-full w-full object-cover" /><span className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border border-white/20 bg-black/55 text-white"><Play className="ml-0.5 h-5 w-5" /></span></>
  if (item.kind === 'video' && url) return <><video src={url} muted preload="metadata" className="h-full w-full object-cover" /><span className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border border-white/20 bg-black/55 text-white"><Play className="ml-0.5 h-5 w-5" /></span></>
...
const VaultActions = ({ item, onPreview, onDelete }: { item: VaultMedia; onPreview: () => void; onDelete: () => void }) => {
  ...
  const cls = 'rounded-xl border border-border/60 p-1.5 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground'
...
export function PF_UserMediaVault() {
  ...
  return (
    <div className="w-full space-y-5 pb-10">
      ...
      <Card className="p-3"><div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Private upload quota used</span><span>{formatBytes(quota.used_bytes)} / {formatBytes(quota.quota_bytes)} ({usedPct}%)</span></div><div className="mt-1 h-2 overflow-hidden rounded-xl bg-muted"><div className="h-full rounded-xl bg-foreground transition-all" style={{ width: `${usedPct}%` }} /></div><p className="mt-2 text-xs text-muted-foreground">{formatBytes(quota.remaining_bytes)} remaining. Linked Story Studio assets and Getter Pro references are not double-counted against upload storage.</p></Card>
      {error && <Card className="border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</Card>}

      <div className="flex flex-wrap items-center gap-2"><Tabs className="min-w-0" tabs={[{ id: 'all', label: 'All kinds' }, ...(['image', 'video', 'document', 'audio', 'other'] as VaultMedia['kind'][]).map((kind) => ({ id: kind, label: kind.charAt(0).toUpperCase() + kind.slice(1) }))]} active={filterKind} onChange={(id) => setFilterKind(id as typeof filterKind)} ariaLabel="Media kinds" /><div className="flex-1" /><div className="flex items-center rounded-xl border border-border/60 p-0.5"><Button variant={viewMode === 'grid' ? 'secondary' : 'ghost'} size="sm" onClick={() => setViewMode('grid')} title="Grid view"><FileImage className="h-4 w-4" /></Button><Button variant={viewMode === 'showcase' ? 'secondary' : 'ghost'} size="sm" onClick={() => setViewMode('showcase')} title="Showcase view"><PanelsTopLeft className="h-4 w-4" /></Button><Button variant={viewMode === 'list' ? 'secondary' : 'ghost'} size="sm" onClick={() => setViewMode('list')} title="List view"><FileText className="h-4 w-4" /></Button></div></div>
