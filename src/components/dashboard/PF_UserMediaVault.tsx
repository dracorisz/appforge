...
export function PF_UserMediaVault() {
  ...
  return (
    <div className="w-full space-y-4 pb-8">
      ...
      {uploading && Object.keys(uploadProgress).length > 0 && <Card className="p-4"><div className="space-y-2">{Object.entries(uploadProgress).map(([name, pct]) => <div key={name} className="flex items-center gap-2 text-xs"><span className="w-40 truncate text-muted-foreground">{name}</span><div className="h-2 flex-1 rounded-xl bg-muted"><div className="h-full rounded-xl bg-foreground transition-all" style={{ width: `${pct}%` }} /></div><span>{pct}%</span></div>)}</div></Card>}
      <Card className="p-4"><div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Private upload quota used</span><span>{formatBytes(quota.used_bytes)} / {formatBytes(quota.quota_bytes)} ({usedPct}%)</span></div><div className="mt-2 h-2 overflow-hidden rounded-xl bg-muted"><div className="h-full rounded-xl bg-foreground transition-all" style={{ width: `${usedPct}%` }} /></div><p className="mt-2 text-xs text-muted-foreground">{formatBytes(quota.remaining_bytes)} remaining. Linked Story Studio assets and Getter Pro references are not double-counted against upload storage.</p></Card>
      ...
      <div className={viewMode === 'list' ? 'space-y-2' : viewMode === 'showcase' ? 'grid gap-4 md:grid-cols-2 xl:grid-cols-3' : 'grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}>
        {loading ? Array.from({ length: 6 }).map((_, index) => <Card key={index} className="overflow-hidden p-2 animate-pulse"><div className="aspect-video bg-muted" /><div className="h-16" /></Card>) : !sortedMedia.length ? <div className="col-span-full rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground"><Folder className="mx-auto h-10 w-10 opacity-50" /><p className="mt-2">No media in this folder/filter yet.</p></div> : sortedMedia.map((item) => {
          ...
          return <Card key={`${item.metadata?.source_table || item.source_app || 'vault'}-${item.id}`} className={list ? 'flex items-center gap-4 p-4' : `overflow-hidden p-2 ${showcase ? 'bg-card/70' : ''}`}>
            ...
            <div className={list ? 'min-w-0 flex-1' : 'p-4'}><div className="flex items-start justify-between gap-2"><div className={list ? 'flex min-w-0 items-center gap-2' : 'min-w-0'}>{list && <Icon className="h-5 w-5 shrink-0 text-muted-foreground" />}<div className="min-w-0"><p className="truncate text-sm font-medium">{item.title || item.file_name || 'Untitled'}</p><div className="mt-2 flex flex-wrap items-center gap-2">{kindBadge(item.kind)}<Badge color="slate">{vaultFolder(item)}</Badge><span className="text-xs text-muted-foreground">{sourceLabel(item)}</span></div></div></div><VaultActions item={item} onPreview={() => void openPreview(item)} onDelete={() => void handleDelete(item)} /></div><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><span className="text-xs text-muted-foreground">{new Date(item.created_at).toLocaleString()}</span>{!isPinnedAsset(item) && <select aria-label="Move file to folder" value={vaultFolder(item)} onChange={(event) => void moveItem(item, event.target.value)} className="h-8 max-w-44 rounded-xl border border-input bg-background px-2 text-xs text-foreground">{allFolderOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>}</div></div>
          </Card>
        })}
      </div>
