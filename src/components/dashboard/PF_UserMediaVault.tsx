...
const VaultThumb = ({ item }: { item: VaultMedia }) => {
  ...
  if (item.kind === 'video' && ytId) return <><img src={metadataUrl(item, 'thumbnail') || `https://i.ytimg.com/vi/${ytId}/hqdefault.jpg`} alt={item.title || 'YouTube video'} loading="lazy" className="h-full w-full object-cover" /><span className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border border-inverse/20 bg-overlay/55 text-inverse"><Play className="ml-2 h-5 w-5" /></span></>
  if (item.kind === 'video' && url) return <><video src={url} muted preload="metadata" className="h-full w-full object-cover" /><span className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border border-inverse/20 bg-overlay/55 text-inverse"><Play className="ml-2 h-5 w-5" /></span></>
...
export function PF_UserMediaVault() {
  ...
  return (
    <div className="w-full space-y-4 pb-8">
      ...
      <Card className="p-4"><div className="flex flex-col gap-4 xl:flex-row xl:items-end"><div className="flex min-w-0 flex-1 gap-2"><input value={newFolder} onChange={(event) => setNewFolder(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void createFolder() }} maxLength={60} placeholder="Create a folder…" className="h-10 min-w-0 flex-1 rounded-xl border border-input bg-background px-4 text-sm outline-none focus:ring-1 focus:ring-ring/30" /><Button variant="secondary" onClick={() => void createFolder()} disabled={!newFolder.trim()}><FolderPlus className="h-4 w-4" /> New folder</Button></div><div className="flex flex-wrap items-end gap-2"><label className="flex items-center gap-2 text-sm text-muted-foreground">Sort<select value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)} className="h-9 rounded-xl border border-input bg-background px-2 text-sm text-foreground"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="name-asc">Name A–Z</option><option value="name-desc">Name Z–A</option><option value="size-desc">Largest first</option><option value="type">Type</option></select></label><Button variant="secondary" onClick={() => void refresh()} disabled={loading}><RefreshCw className="h-4 w-4" /> Refresh</Button><input type="file" multiple accept="image/*,video/*,audio/*,application/pdf,.txt,.md,.json" className="hidden" onChange={handleUpload} disabled={uploading} id="vault-upload" /><label htmlFor="vault-upload" className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border/70 bg-background/45 px-4 py-2 text-sm font-medium hover:bg-accent"><Upload className="h-4 w-4" />{uploading ? 'Uploading…' : `Upload to ${uploadTarget === 'general' ? 'General' : uploadTarget}`}</label></div></div></Card>
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">{folders.map(({ id, label, icon: Icon }) => {
        ...
        return <div key={String(id)} className={`flex items-center gap-2 rounded-xl border px-2 py-2 transition-colors ${folder === id ? 'border-foreground/25 bg-accent' : 'border-border/70 bg-background/35 hover:bg-accent/60'}`}><button onClick={() => setFolder(id)} className="flex min-w-0 flex-1 items-center gap-2 px-2 py-2 text-left text-sm"><Icon className="h-4 w-4 shrink-0" /><span className="truncate">{label}</span></button>{userFolder && <><button type="button" onClick={() => void renameFolder(userFolder)} className="rounded-xl p-2 text-muted-foreground hover:bg-background hover:text-foreground" aria-label={`Rename ${label}`}><Pencil className="h-3.5 w-3.5" /></button><button type="button" onClick={() => void deleteFolder(userFolder)} className="rounded-xl p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label={`Delete ${label}`}><Trash2 className="h-3.5 w-3.5" /></button></>}</div>
      })}</div>

      {uploading && Object.keys(uploadProgress).length > 0 && <Card className="p-4"><div className="space-y-2">{Object.entries(uploadProgress).map(([name, pct]) => <div key={name} className="flex items-center gap-2 text-sm"><span className="w-40 truncate text-muted-foreground">{name}</span><div className="h-2 flex-1 rounded-xl bg-muted"><div className="h-full rounded-xl bg-foreground transition-all" style={{ width: `${pct}%` }} /></div><span>{pct}%</span></div>)}</div></Card>}
      <Card className="p-4"><div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Private upload quota used</span><span>{formatBytes(quota.used_bytes)} / {formatBytes(quota.quota_bytes)} ({usedPct}%)</span></div><div className="mt-2 h-2 overflow-hidden rounded-xl bg-muted"><div className="h-full rounded-xl bg-foreground transition-all" style={{ width: `${usedPct}%` }} /></div><p className="mt-2 text-sm text-muted-foreground">{formatBytes(quota.remaining_bytes)} remaining. Linked Story Studio assets and Getter Pro references are not double-counted against upload storage.</p></Card>
      {error && <Card className="border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">{error}</Card>}

      <div className="flex flex-wrap items-center gap-2"><Tabs className="min-w-0" tabs={[{ id: 'all', label: 'All kinds' }, ...(['image', 'video', 'document', 'audio', 'other'] as VaultMedia['kind'][]).map((kind) => ({ id: kind, label: kind.charAt(0).toUpperCase() + kind.slice(1) }))]} active={filterKind} onChange={(id) => setFilterKind(id as typeof filterKind)} ariaLabel="Media kinds" /><div className="flex-1" /><div className="flex items-center rounded-xl border border-border/60 p-2"><Button variant={viewMode === 'grid' ? 'secondary' : 'ghost'} size="sm" onClick={() => setViewMode('grid')} title="Grid view"><FileImage className="h-4 w-4" /></Button><Button variant={viewMode === 'showcase' ? 'secondary' : 'ghost'} size="sm" onClick={() => setViewMode('showcase')} title="Showcase view"><PanelsTopLeft className="h-4 w-4" /></Button><Button variant={viewMode === 'list' ? 'secondary' : 'ghost'} size="sm" onClick={() => setViewMode('list')} title="List view"><FileText className="h-4 w-4" /></Button></div></div>
      ...
      <div className={viewMode === 'list' ? 'space-y-2' : viewMode === 'showcase' ? 'grid gap-4 md:grid-cols-2 xl:grid-cols-3' : 'grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}>
        {loading ? Array.from({ length: 6 }).map((_, index) => <Card key={index} className="overflow-hidden p-2 animate-pulse"><div className="aspect-video bg-muted" /><div className="h-16" /></Card>) : !sortedMedia.length ? <div className="col-span-full rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground"><Folder className="mx-auto h-10 w-10 opacity-50" /><p className="mt-2">No media in this folder/filter yet.</p></div> : sortedMedia.map((item) => {
          const Icon = kindIcon(item.kind); const list = viewMode === 'list'; const showcase = viewMode === 'showcase'
          return <Card key={`${item.metadata?.source_table || item.source_app || 'vault'}-${item.id}`} className={list ? 'flex items-center gap-4 p-4' : `overflow-hidden p-2 ${showcase ? 'bg-card/70' : ''}`}>
            ...
            <div className={list ? 'min-w-0 flex-1' : 'p-4'}><div className="flex items-start justify-between gap-2"><div className={list ? 'flex min-w-0 items-center gap-2' : 'min-w-0'}>{list && <Icon className="h-5 w-5 shrink-0 text-muted-foreground" />}<div className="min-w-0"><p className="truncate text-sm font-medium">{item.title || item.file_name || 'Untitled'}</p><div className="mt-2 flex flex-wrap items-center gap-2">{kindBadge(item.kind)}<Badge color="slate">{vaultFolder(item)}</Badge><span className="text-sm text-muted-foreground">{sourceLabel(item)}</span></div></div></div><VaultActions item={item} onPreview={() => void openPreview(item)} onDelete={() => void handleDelete(item)} /></div><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><span className="text-sm text-muted-foreground">{new Date(item.created_at).toLocaleString()}</span>{!isPinnedAsset(item) && <select aria-label="Move file to folder" value={vaultFolder(item)} onChange={(event) => void moveItem(item, event.target.value)} className="h-8 max-w-44 rounded-xl border border-input bg-background px-2 text-sm text-foreground">{allFolderOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>}</div></div>
          </Card>
        })}
      </div>

      {preview && (preview.item.kind === 'image' || preview.item.kind === 'video') ? <><MediaShowbox open onClose={() => setPreview(null)} type={preview.item.kind} title={preview.item.title || preview.item.file_name || 'Media Vault item'} source={sourceLabel(preview.item)} originalUrl={metadataUrl(preview.item, 'original_url') || preview.item.source_ref || preview.item.external_url || preview.url} mediaUrl={preview.url} thumbnail={metadataUrl(preview.item, 'thumbnail') || undefined} note="Private Media Vault preview. YouTube references play in the privacy-enhanced embedded player." />{previewableMedia.length > 1 && <><button type="button" onClick={() => navigatePreview(-1)} className="fixed left-3 top-1/2 z-[70] grid h-11 w-11 -translate-y-1/2 place-items-center rounded-xl border border-inverse/20 bg-overlay/70 text-inverse hover:bg-overlay/90 sm:left-6" aria-label="Previous media"><ChevronLeft className="h-6 w-6" /></button><button type="button" onClick={() => navigatePreview(1)} className="fixed right-3 top-1/2 z-[70] grid h-11 w-11 -translate-y-1/2 place-items-center rounded-xl border border-inverse/20 bg-overlay/70 text-inverse hover:bg-overlay/90 sm:right-6" aria-label="Next media"><ChevronRight className="h-6 w-6" /></button></>}</> : preview ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/90 p-4" onClick={() => setPreview(null)}><div className="max-w-md rounded-xl border border-border/70 bg-background p-4 text-center text-muted-foreground">Preview is not available for this item type. Use the source/download action.</div><button onClick={() => setPreview(null)} className="absolute right-4 top-4 rounded-xl bg-overlay/50 p-2 text-inverse hover:bg-overlay/70"><X className="h-5 w-5" /></button></div> : null}
    </div>
  )
}
