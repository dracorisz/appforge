...
export function PF_ScrapperProNext() {
  ...
  const loadMoreYouTube = async () => {
    ...
    setLoadingMore(true)
    try { const data = await fetchYouTube(cleanQuery, nextPageToken); setResults((current) => dedupe([...current, ...(data.results || [])])); setNextPageToken(data.nextPageToken || null); setMessage(`Loaded more ${filter === 'all' ? 'results' : `${filter} results`}${data.nextPageToken ? '.' : ' · end of results.'}`) }
    catch (error) { setMessage(error instanceof Error ? error.message : `Could not load more ${filter === 'all' ? 'results' : `${filter} results`}.`) }
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-4 p-4 sm:p-4">
      <section className="surface-card rounded-xl border p-4 sm:p-4"><AppHeading />
        <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto_auto]"><label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void runSearch() }} placeholder="Search text, YouTube URL, channel ID, video ID, or @handle" className="h-11 w-full rounded-xl border bg-background pl-8 pr-4 text-sm" /></label><button type="button" onClick={() => void runSearch()} disabled={loading || !query.trim()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />} Search</button><button type="button" onClick={() => void runSearch()} disabled={loading || !results.length} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border px-4 text-sm font-semibold hover:bg-accent disabled:opacity-50"><RefreshCw className="h-4 w-4" /> Refresh</button></div>
        <div className="mt-4 flex flex-wrap gap-2">{SOURCES.map((source) => { const active = selectedSources.has(source.id); return <button key={source.id} type="button" onClick={() => toggleSource(source)} disabled={source.disabled} title={source.status} className={`rounded-xl border px-4 py-2 text-sm font-medium ${source.disabled ? 'cursor-not-allowed opacity-45' : active ? 'bg-accent text-foreground cursor-pointer' : 'text-muted-foreground hover:text-foreground'}`}>{source.name}{source.disabled ? ` · ${source.status}` : ''}</button> })}</div>
        {failures.length > 0 && <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-700 dark:text-amber-300">{failures.join(' · ')}</div>}<div aria-live="polite" className="mt-4 min-h-5 text-sm text-muted-foreground">{message}</div>
      </section>
      <section className="surface-card flex flex-col gap-4 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between"><Tabs className="min-w-0 flex-1" tabs={(['all', 'image', 'video', 'post', 'article'] as const).map((type) => ({ id: type, label: `${type === 'all' ? 'All' : type.charAt(0).toUpperCase() + type.slice(1)} ${type === 'all' ? results.length : counts[type]}` }))} active={filter} onChange={(id) => setFilter(id as typeof filter)} ariaLabel="Getter Pro result types" /><div className="flex flex-wrap gap-2"><button type="button" onClick={toggleAllVisible} disabled={!visibleResults.length} className="inline-flex min-h-9 items-center gap-2 rounded-xl border px-4 text-sm font-semibold disabled:opacity-40">{allVisibleSelected ? <CheckSquare className="h-3.5 w-3.5" /> : <Square className="h-3.5 w-3.5" />} {allVisibleSelected ? 'Clear visible' : 'Select visible'}</button><button type="button" onClick={() => void saveSelectedVault()} disabled={!selectedResults.length} className="inline-flex min-h-9 items-center gap-2 rounded-xl border px-4 text-sm font-semibold disabled:opacity-40"><Archive className="h-3.5 w-3.5" /> Media Vault</button></div></section>
      {visibleResults.length > 0 && <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{visibleResults.map((result) => { const selected = selectedIds.has(result.id); const savedVault = vaultSavedUrls.has(result.url); const saving = savingVault.has(result.url); const downloadable = Boolean(result.mediaUrl || result.thumbnail); return <article key={result.id} className={`surface-card flex h-full flex-col overflow-hidden rounded-xl border ${selected ? 'ring-1 ring-primary/40' : ''}`}><button type="button" onClick={() => toggleSelected(result.id)} className="relative block aspect-video w-full shrink-0 overflow-hidden bg-muted/40 text-left" aria-pressed={selected}>{result.thumbnail ? <img src={result.thumbnail} alt="" loading="lazy" className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-muted-foreground"><TypeIcon type={result.type} /></div>}<span className="absolute left-2 top-2 rounded-xl bg-background/90 p-2">{selected ? <CheckSquare className="h-4 w-4" /> : <Square className="h-4 w-4" />}</span>{result.source === 'YouTube' && <span className="absolute right-2 top-2 inline-flex items-center gap-2 rounded-xl bg-red-600 px-2 py-2 text-sm font-semibold text-white"><Youtube className="h-3 w-3" /> API</span>}</button><div className="flex flex-1 flex-col p-4"><div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground"><TypeIcon type={result.type} /> {result.source}{savedVault ? ' · vault' : ''}</div><h2 className="mt-2 line-clamp-2 text-sm font-semibold leading-5">{result.title}</h2><p className="mt-2 line-clamp-3 text-sm leading-5 text-muted-foreground">{result.snippet || 'No description.'}</p>{result.provenance && <p className="mt-2 line-clamp-2 text-sm leading-4 text-muted-foreground">{provenanceText(result)}</p>}<div className="mt-auto grid grid-cols-[1fr_auto_auto_auto] gap-2 pt-4"><a href={result.url} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center justify-center gap-2 rounded-xl border text-sm font-semibold hover:bg-accent">Open <ExternalLink className="h-3.5 w-3.5" /></a><button type="button" onClick={() => void saveOneVault(result)} disabled={saving} className="grid min-h-9 w-9 place-items-center rounded-xl border hover:bg-accent disabled:opacity-50" aria-label="Save to Media Vault" title="Save to Media Vault">{saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Archive className={`h-3.5 w-3.5 ${savedVault ? 'text-primary' : ''}`} />}</button><button type="button" onClick={() => void downloadResult(result)} disabled={!downloadable} className="grid min-h-9 w-9 place-items-center rounded-xl border hover:bg-accent disabled:cursor-not-allowed disabled:opacity-35" aria-label="Download result media" title="Download media"><Download className="h-3.5 w-3.5" /></button><button type="button" onClick={() => void copyUrl(result.url)} className="grid min-h-9 w-9 place-items-center rounded-xl border hover:bg-accent" aria-label="Copy source URL" title="Copy source URL"><Copy className="h-3.5 w-3.5" /></button></div></div></article> })}</section>}
      ...
      {nextPageToken && selectedSources.has('youtube') && <div className="flex justify-center"><button type="button" onClick={() => void loadMoreYouTube()} disabled={loadingMore} className="inline-flex min-h-11 items-center gap-2 rounded-xl border bg-background px-4 text-sm font-semibold hover:bg-accent disabled:opacity-50">{loadingMore ? <Loader2 className="h-4 w-4 animate-spin" /> : <Youtube className="h-4 w-4" />} Load more {filter === 'all' ? 'results' : `${filter} results`}</button></div>}
    </div>
  )
}
