...
import { Card, Button, Input, Switch, Textarea } from '@/components/ui'
import { ImagePlus, Search, Plus, Save, Trash2, Copy, X } from 'lucide-react'
...
export function AppAdminPage() {
  ...
  const saveEdit = async () => {
    ...
    try { updateApp(form as AppDefinition); await saveAppOverride(form as AppDefinition); setApps([...getManageableApps()]); cancelEdit() }
    catch (e) { setError(e instanceof Error ? e.message : 'Could not save app.') }
  }
  const removeApp = (id: string) => { if (!confirm('Delete this app?')) return; deleteApp(id); setApps([...getManageableApps()]) }
  const createApp = () => {
    ...
    try {
      ...
      setApps([...getManageableApps()]); setForm({}); setError('')
  ...
  const uploadCover = async (file: File) => {
    try {
      const { data } = await import('@/lib/supabase').then(({ supabase }) => supabase.auth.getUser())
  ...
  return (
    <div className="w-full space-y-5 pb-10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><h2 className="text-xl font-semibold tracking-tight text-foreground">Apps</h2><p className="mt-1 text-sm text-muted-foreground">Manage app names, covers and catalog visibility.</p></div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{filtered.map((app) => { const isEditing = editingId === app.id; return <Card key={app.id} className={`overflow-hidden p-0 ${isEditing ? 'ring-2 ring-foreground/25' : ''}`}>{!isEditing && app.coverImage && <div className="relative h-28 w-full overflow-hidden"><img src={app.coverImage} alt={app.name} className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-background/80 to-transparent" /></div>}<div className="space-y-3 p-4">{isEditing ? <div className="space-y-2"><Input label="ID" value={form.id || ''} disabled /><Input label="Name" value={form.name || ''} onChange={(e) => setForm({ ...form, name: e.target.value })} /><Input label="Route" value={form.route || ''} onChange={(e) => setForm({ ...form, route: e.target.value })} /><Input label="Category" value={form.category || ''} onChange={(e) => setForm({ ...form, category: e.target.value })} /><Input label="Icon" value={form.icon || ''} onChange={(e) => setForm({ ...form, icon: e.target.value })} /><Input label="Version" value={form.version || ''} onChange={(e) => setForm({ ...form, version: e.target.value })} /><Input label="Cover image URL" value={form.coverImage || ''} onChange={(e) => setForm({ ...form, coverImage: e.target.value })} /><label className="inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border px-3 text-xs font-medium hover:bg-accent"><ImagePlus className="h-4 w-4" /> Upload cover<input type="file" accept="image/*" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadCover(file); event.currentTarget.value = '' }} /></label><div className="flex items-center justify-between rounded-lg border p-3"><span className="text-xs font-medium">Visible across AppForge</span><Switch checked={form.visible !== false} onCheckedChange={(visible) => setForm({ ...form, visible })} label="Show app" /></div><Textarea label="Description" value={form.description || ''} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} />{error && <div className="text-sm text-destructive">{error}</div>}<div className="flex gap-2"><Button onClick={() => void saveEdit()} className="flex-1"><Save className="h-4 w-4" /> Save</Button><Button variant="secondary" onClick={cancelEdit}>Cancel</Button></div></div> : <><div className="flex items-start justify-between gap-2"><div><h3 className="font-semibold">{app.name}</h3><p className="text-xs text-muted-foreground">{app.category} · v{app.version} · {app.visible === false ? 'hidden' : app.status}</p></div></div><p className="line-clamp-2 text-xs text-muted-foreground">{app.description}</p><div className="flex gap-1"><Button variant="ghost" size="sm" onClick={() => startEdit(app)} className="flex-1">Edit</Button><Button variant="ghost" size="sm" onClick={() => removeApp(app.id)} className="text-red-500"><Trash2 className="h-3.5 w-3.5" /></Button></div></>}</div></Card> })}</div>
    </div>
  )
}
