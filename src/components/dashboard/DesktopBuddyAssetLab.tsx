...
export function DesktopBuddyAssetLab() {
  ...
  return (
    <section className="mx-auto mt-4 w-full max-w-6xl rounded-xl border bg-card p-4 md:p-4" aria-label="Desktop Buddy asset optimizer">
      ...
      <div aria-live="polite" className="mt-4 text-sm text-muted-foreground">{message}</div>
      {sourceName && <div className="mt-2 text-sm text-muted-foreground">Source: {sourceName} · {formatBytes(sourceBytes)}</div>}
            ...
            {variants.map((variant) => (
              <article key={variant.size} className="overflow-hidden rounded-xl border bg-background/45">
                ...
                <div className="space-y-4 p-4">
                  <div><div className="text-sm font-semibold">{variant.size} × {variant.size}</div><div className="mt-2 text-sm text-muted-foreground">PNG {formatBytes(variant.pngBytes)} · WebP {formatBytes(variant.webpBytes)}</div></div>
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => downloadUrl(variant.pngUrl, `${safeStem(sourceName)}-${variant.size}.png`)} className="inline-flex min-h-9 items-center justify-center gap-2 rounded-xl border text-sm font-semibold hover:bg-accent"><Download className="h-3.5 w-3.5" /> PNG</button>
                    <button type="button" disabled={!variant.webpUrl} onClick={() => variant.webpUrl && downloadUrl(variant.webpUrl, `${safeStem(sourceName)}-${variant.size}.webp`)} className="inline-flex min-h-9 items-center justify-center gap-2 rounded-xl border text-sm font-semibold hover:bg-accent disabled:opacity-40"><Download className="h-3.5 w-3.5" /> WebP</button>
                  </div>
                </div>
              </article>
            ))}
