...
export function AdminConsolePage() {
  ...
  if (!aal2) return <Card className="mx-auto max-w-lg p-5"><div className="flex items-start gap-3"><KeyRound className="mt-0.5 h-5 w-5 text-muted-foreground" /><div><h1 className="text-lg font-semibold">Verify this admin session</h1><p className="mt-1 text-sm leading-6 text-muted-foreground">Your authenticator is already enrolled. Enter the current code here to elevate this session to AAL2; you do not need to reconfigure Security.</p></div></div>{factorId ? <div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><Input label="Authenticator code" value={totpCode} inputMode="numeric" autoComplete="one-time-code" onChange={(event) => setTotpCode(event.target.value.replace(/\D/g, '').slice(0, 8))} onKeyDown={(event) => { if (event.key === 'Enter') void verifyAdminSession() }} placeholder="123456" /><Button onClick={() => void verifyAdminSession()} disabled={busy === 'totp' || totpCode.length < 6}>{busy === 'totp' ? <Loader2 className="animate-spin" /> : <ShieldCheck />} Verify & open Admin</Button></div> : <div className="mt-4 rounded-xl border border-border/70 p-3 text-sm text-muted-foreground">No verified TOTP factor is available. <Link to="/settings?tab=security" className="font-medium text-foreground underline">Set up an authenticator in Security</Link>.</div>}</Card>
