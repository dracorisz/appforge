...
export function AdminConsolePage() {
  ...
  const setContentSection = React.useCallback(
  ...
  const refreshAccess = React.useCallback(async () => {
    ...
    try {
      ...
      fail(accessError, "Could not verify admin access.");
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const refreshUsers = React.useCallback(async () => {
    ...
    try {
      ...
      fail(usersError, "Could not load users.");
    } finally {
      setBusy("");
    }
  }, []);
  ...
  const verifyAdminSession = async () => {
    ...
    try {
      ...
      fail(verifyError, "TOTP verification failed.");
    } finally {
      setBusy("");
    }
  };
  ...
  React.useEffect(() => {
    if (role === "admin" && aal2 && section === "users") void refreshUsers();
  }, [aal2, refreshUsers, role, section]);
  ...
  if (role !== "admin")
    return (
      <Card className="p-8 text-center">
        ...
        <h1 className="mt-4 text-lg font-semibold">Admin only</h1>
        <Link to="/settings" className="mt-4 inline-flex cursor-pointer text-sm underline">
          Back to Settings
        </Link>
      </Card>
    );
        ...
        {factorId ? (
          <div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
            <Input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter") void verifyAdminSession();
              }}
              placeholder="123456"
            />
      ...
      {section === "users" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="secondary" size="sm" onClick={() => void refreshUsers()} disabled={busy === "users"}>
              <RefreshCw className={busy === "users" ? "animate-spin" : ""} /> Refresh
            </Button>
          </div>
          {users.map((item) => (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <select
                    ...
                    onChange={async (event) => {
                      ...
                      try {
                        await adminSetRole(item.id, nextRole);
                        setUsers((rows) => rows.map((row) => (row.id === item.id ? { ...row, role: nextRole } : row)));
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await adminUpdateProfile(item.id, { display_name: item.display_name, username: item.username, is_public: !item.is_public });
                        setUsers((rows) => rows.map((row) => (row.id === item.id ? { ...row, is_public: !row.is_public } : row)));
                        toast.success("Visibility updated.");
                      } catch (profileError) {
                        fail(profileError, "Update failed.");
                      }
                    }}
                  >
                  ...
                  {item.id !== user.id && (
                    <Button
                      ...
                      size="sm"
                      onClick={async () => {
                        if (!confirm(`Delete ${item.email || "this user"}?`)) return;
                        try {
                          await adminDeleteUser(item.id);
                          setUsers((rows) => rows.filter((row) => row.id !== item.id));
                          toast.success("User deleted.");
                        } catch (deleteError) {
                          fail(deleteError, "Delete failed.");
                        }
                      }}
                    >
                      <Trash2 />
      ...
      {section === "content" && (
        <div className="space-y-4">
          <Tabs
            ...
            onChange={(id) => setContentSection(id as ContentSection)}
            ariaLabel="Content sections"
          />
          {contentSection === "blog" && (
            <div className="space-y-4">
              <AdminBlogFeaturedControl />
              <AdminContentManager embedded adminVerified contentType="blog_article" />
            </div>
          )}
