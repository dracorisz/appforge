  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
        <Card className="p-4">
          <Button onClick={newItem} variant="secondary" className="w-full justify-center gap-2"><Plus className="h-4 w-4" /> New page</Button>
          <div className="mt-4 space-y-2">
            {items.map((item) => (
              <Button key={item.id} onClick={() => selectItem(item)} variant={selectedId === item.id ? "secondary" : "ghost"} className="h-auto w-full justify-start p-4 text-left">
                <span className="min-w-0"><span className="block truncate text-sm font-medium">{item.title}</span><span className="mt-2 block truncate text-sm text-muted-foreground">{item.slug}</span></span>
              </Button>
            ))}
          </div>
        </Card>
        <Card className="p-4">
          <form onSubmit={save} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <label className="text-sm font-medium text-muted-foreground">Title<Input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} className="mt-2 w-full" /></label>
              <label className="text-sm font-medium text-muted-foreground">Slug<Input value={draft.slug} onChange={(event) => setDraft((current) => ({ ...current, slug: event.target.value }))} className="mt-2 w-full" placeholder="getting-started" /></label>
            </div>
            <label className="block text-sm font-medium text-muted-foreground">Summary<Textarea value={draft.summary || ""} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} rows={2} className="mt-2 w-full" /></label>
            <label className="block text-sm font-medium text-muted-foreground">Body (Markdown)<Textarea value={draft.body || ""} onChange={(event) => setDraft((current) => ({ ...current, body: event.target.value }))} rows={24} className="mt-2 w-full font-mono" /></label>
            <div className="grid gap-4 md:grid-cols-2">
              <label className="text-sm font-medium text-muted-foreground">Navigation order<Input type="number" value={draft.sort_order} onChange={(event) => setDraft((current) => ({ ...current, sort_order: Number(event.target.value) || 0 }))} className="mt-2 w-full" /></label>
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="submit" disabled={busy === "save"}><Save className="h-4 w-4" /> Save</Button>
              {selectedId && <Button type="button" variant="ghost" onClick={() => void remove()} disabled={busy === "delete"} className="text-destructive"><Trash2 className="h-4 w-4" /> Delete</Button>}
