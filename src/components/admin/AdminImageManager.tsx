    ...
    <section className="space-y-4" aria-labelledby="content-images-title">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(220px,.7fr)_minmax(0,1.3fr)]">
        <div className="space-y-2">
          {items.map((item) => <button key={item.id} type="button" onClick={() => select(item)} className={`w-full rounded-xl border p-3 text-left transition-[border-color,background-color,box-shadow] hover:shadow-sm ${selectedId === item.id ? 'border-foreground/25 bg-accent/70' : 'border-border/70 bg-card/60 hover:border-foreground/20'}`}><div className="flex items-center gap-3">{item.image_url ? <img src={item.image_url} alt="" className="h-12 w-16 rounded-lg border border-border/70 object-cover" /> : <div className="flex h-12 w-16 items-center justify-center rounded-lg border border-border/70 bg-muted"><ImagePlus className="h-4 w-4" /></div>}<div className="min-w-0"><div className="truncate text-sm font-medium">{item.title}</div><div className="truncate text-xs text-muted-foreground">{item.slug}</div></div></div></button>)}
        ...
        <Card className="p-4 sm:p-5">
          <form onSubmit={save} className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2"><label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-border/70 px-3 py-2 text-sm hover:bg-accent"><Upload className="h-4 w-4" /> Upload image<input type="file" accept="image/*" className="hidden" disabled={busy === 'upload'} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = '' }} /></label><Button type="submit" disabled={Boolean(busy)}>{busy === 'save' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save</Button>{selectedId && <Button type="button" variant="ghost" onClick={() => void remove()} disabled={Boolean(busy)}><Trash2 className="h-4 w-4" /> Delete</Button>}</div>
          </form>
        </Card>
      </div>
    </section>
