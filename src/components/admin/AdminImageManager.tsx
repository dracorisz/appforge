...
import { createFrontendContent, deleteFrontendContent, loadAllFrontendContent, updateFrontendContent, uploadFrontendContentMedia, type FrontendContentRecord } from "@/lib/frontendContent";
...
export function AdminImageManager() {
  ...
  React.useEffect(() => {
    void refresh().catch((error) => toast.error(error instanceof Error ? error.message : "Could not load images."));
  }, [refresh]);
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (!draft.title.trim() || !slug || !draft.image_url.trim()) {
      toast.error("Image title, slug and image URL are required.");
      return;
    }
    setBusy("save");
    try {
      const payload = {
        ...
        video_url: null,
        app_route: draft.app_route.trim() || "/huggingface",
  ...
  const remove = async () => {
    ...
    try {
      ...
      setDraft(blank);
      await refresh();
      toast.success("Image removed and gallery refreshed.");
    } catch (error) {
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(220px,.7fr)_minmax(0,1.3fr)]">
        <div className="space-y-2">
          {items.map((item) => (
            <Button
              ...
              type="button"
              onClick={() => select(item)}
              className={`h-auto min-h-0 max-h-none w-full cursor-pointer justify-start rounded-xl border p-4 text-left transition-colors ${selectedId === item.id ? "border-foreground/25 bg-accent/70" : "border-border bg-card/60 hover:border-foreground/20 hover:bg-accent/30"}`}
            >
              <div className="flex min-w-0 items-center gap-4">
                {item.image_url ? (
                  <img src={item.image_url} alt="" className="h-12 w-16 shrink-0 rounded-xl border border-border object-cover" />
                ) : (
                  <div className="flex h-12 w-16 shrink-0 items-center justify-center rounded-xl border border-border bg-muted">
        ...
        <Card className="p-4">
          <form onSubmit={save} className="space-y-4">
            ...
            {draft.image_url && <img src={draft.image_url} alt="Preview" className="max-h-72 w-full rounded-xl border border-border bg-muted object-contain" />}
            <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {selectedId && (
                <Button type="button" variant="ghost" onClick={() => void remove()} disabled={Boolean(busy)}>
