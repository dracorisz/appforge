...
export function AdminContentManager({ embedded = false, adminVerified = false, contentType }: Props) {
  ...
  const newItem = () => {
    if (filterType === 'video_teaser' && landingTeaser) { selectItem(landingTeaser); return }
    setSelectedId(null); setDraft(makeDraft(filterType)); setError('')
  }
  ...
  const persistDraft = async (publishOverride?: boolean) => {
    ...
    if (!slug || !draft.title.trim()) { setError('Title and slug are required.'); return null }
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try { const saved = await persistDraft(); if (saved) flash(draft.content_type === 'video_teaser' ? 'Landing walkthrough saved and published.' : 'Saved.') }
  ...
  const publishBlog = async () => {
    ...
    try { const saved = await persistDraft(true); if (saved) flash('Blog article published.') }
    catch (publishError) { setError(publishError instanceof Error ? publishError.message : 'Could not publish article.') }
    finally { setBusy('') }
  }
  ...
  const remove = async () => {
    ...
    setBusy('delete')
    try { await deleteFrontendContent(selectedId); await refreshItems(); setSelectedId(null); setDraft(makeDraft(filterType)); flash('Deleted.') }
  ...
  if (!user) return <div className="rounded-xl border border-border/70 p-4 text-center"><ShieldCheck className="mx-auto h-8 w-8" /><h2 className="mt-4 text-lg font-semibold">Admin sign-in required</h2><Link to="/login" className="mt-4 inline-flex text-sm underline">Sign in</Link></div>
  if (!isAdmin) return <div className="rounded-xl border border-border/70 p-4 text-center"><ShieldCheck className="mx-auto h-8 w-8" /><h2 className="mt-4 text-lg font-semibold">Admin only</h2></div>
  if (!aal2) return <div className="rounded-xl border border-border/70 p-4"><KeyRound className="h-7 w-7" /><h2 className="mt-4 text-lg font-semibold">Verify TOTP</h2>{factorId ? <form onSubmit={verifyTotp} className="mt-4 flex max-w-md gap-2"><input value={totpCode} onChange={(event) => setTotpCode(event.target.value.replace(/\D/g, '').slice(0, 8))} inputMode="numeric" className="h-10 min-w-0 flex-1 rounded-xl border border-input bg-background px-4" placeholder="Authenticator code" /><button disabled={busy === 'totp' || !totpCode} className="rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground">Verify</button></form> : <p className="mt-2 text-sm text-muted-foreground">Enroll TOTP in Settings → Security first.</p>}</div>

  return <div className="w-full space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-sm font-semibold">{filterType === 'blog_article' ? 'Blog content' : 'Landing content'}</h2><p className="mt-2 text-sm text-muted-foreground">{filterType === 'blog_article' ? 'Draft and publish public blog articles.' : 'Manage the public landing page walkthrough and presentation.'}</p></div><div className="inline-flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400"><CheckCircle2 className="h-3.5 w-3.5" /> Admin · TOTP</div></div>
    {!contentType && <div className="flex flex-wrap gap-2">{MANAGED_TYPES.map((type) => <button key={type} onClick={() => { setFilterType(type); setSelectedId(null); setDraft(type === 'video_teaser' && landingTeaser ? toDraft(landingTeaser) : makeDraft(type)); if (type === 'video_teaser' && landingTeaser) setSelectedId(landingTeaser.id) }} className={`rounded-xl px-4 py-2 text-sm font-medium ${filterType === type ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>{type === 'blog_article' ? 'Blog' : 'Landing'}</button>)}</div>}
    {message && <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/5 p-2 text-sm text-emerald-600 dark:text-emerald-400">{message}</div>}
    {error && <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-2 text-sm text-destructive">{error}</div>}

    {filterType === 'video_teaser' && <section className="rounded-xl border border-border/70 p-4" aria-label="Landing page presentation"><h3 className="text-sm font-semibold">Landing page</h3><label className="mt-4 flex items-center gap-4 text-sm"><input type="checkbox" checked={landingTeaser?.metadata.show_active_app_count !== false} disabled={Boolean(busy)} onChange={(event) => void saveLandingCount(event.target.checked)} />Show active app count and open-source row</label><p className="mt-2 text-sm text-muted-foreground">Applies to the public landing and sign-in pages. Changes save immediately.</p></section>}
    <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="rounded-xl border border-border/70 bg-background/40 p-4"><button onClick={newItem} className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-border/70 text-sm font-semibold hover:bg-accent"><Plus className="h-4 w-4" /> {filterType === 'video_teaser' && landingTeaser ? 'Open walkthrough' : 'New'}</button><div className="mt-4 space-y-2">{visible.map((item) => <button key={item.id} onClick={() => selectItem(item)} className={`w-full rounded-xl border p-4 text-left ${selectedId === item.id ? 'border-foreground/30 bg-accent/50' : 'border-border/60 hover:bg-accent/25'}`}><div className="truncate text-sm font-medium">{item.title}</div><div className="mt-2 truncate text-sm text-muted-foreground">{item.slug}</div></button>)}{!visible.length && <div className="p-4 text-center text-sm text-muted-foreground">No items.</div>}</div></aside>

      <section className="rounded-xl border border-border/70 bg-background/40 p-4 sm:p-4">
        {draft.content_type === 'blog_article' && <div className="mb-4 rounded-xl border border-border/70 p-4"><div className="flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4" /> Vertex writing</div><div className="mt-4 flex flex-col gap-2 sm:flex-row"><input value={vertexTopic} onChange={(event) => setVertexTopic(event.target.value)} className="h-10 min-w-0 flex-1 rounded-xl border border-input bg-background px-4 text-sm" placeholder="Topic" /><button type="button" onClick={() => void draftWithVertex(false)} disabled={busy === 'vertex'} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-border px-4 text-sm font-semibold hover:bg-accent">{busy === 'vertex' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} Generate draft</button><button type="button" onClick={() => void draftWithVertex(true)} disabled={busy === 'vertex'} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground"><CheckCircle2 className="h-4 w-4" /> Generate & publish</button></div></div>}
        <form onSubmit={save} className="space-y-4">
          <div className="flex items-center justify-between"><h3 className="text-lg font-semibold">{draft.content_type === 'video_teaser' ? 'Landing walkthrough' : selectedId ? 'Edit article' : 'New article'}</h3>{selectedId && draft.content_type === 'blog_article' && <button type="button" onClick={newItem} className="rounded-xl p-2 text-muted-foreground hover:bg-accent"><X className="h-4 w-4" /></button>}</div>
          <div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-medium text-muted-foreground">Title<input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground" /></label><label className="text-sm font-medium text-muted-foreground">Slug<input value={draft.slug} disabled={draft.content_type === 'video_teaser'} onChange={(event) => setDraft((current) => ({ ...current, slug: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground disabled:opacity-60" placeholder="article-slug" /></label></div>
          <label className="block text-sm font-medium text-muted-foreground"><span className="flex items-center justify-between"><span>Summary</span>{draft.content_type === 'blog_article' && <button type="button" onClick={() => setDraft((current) => ({ ...current, summary: generateSummary(current.body, current.title) }))} className="text-sm text-foreground hover:underline">Generate</button>}</span><textarea value={draft.summary || ''} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} rows={2} className="mt-2 w-full rounded-xl border border-input bg-background p-4 text-sm text-foreground" /></label>
          {draft.content_type === 'blog_article' && <label className="block text-sm font-medium text-muted-foreground">Body (Markdown)<textarea value={draft.body || ''} onChange={(event) => setDraft((current) => ({ ...current, body: event.target.value }))} rows={14} className="mt-2 w-full rounded-xl border border-input bg-background p-4 text-sm leading-6 text-foreground" /></label>}
          <div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-medium text-muted-foreground">Image URL<input value={draft.image_url || ''} onChange={(event) => setDraft((current) => ({ ...current, image_url: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground" /></label><label className="text-sm font-medium text-muted-foreground">Video URL<input value={draft.video_url || ''} onChange={(event) => setDraft((current) => ({ ...current, video_url: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground" /></label></div>
          <div className="flex flex-wrap items-center gap-2"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border/70 px-4 py-2 text-sm font-medium hover:bg-accent"><Upload className="h-4 w-4" /><input type="file" className="hidden" accept="image/*,video/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadMedia(file); event.currentTarget.value = '' }} />{busy === 'upload' ? 'Uploading…' : 'Upload media'}</label>{draft.content_type === 'blog_article' && <label className="inline-flex items-center gap-2 text-sm"><input type="checkbox" checked={draft.published} onChange={(event) => setDraft((current) => ({ ...current, published: event.target.checked }))} /> Published</label>}<button type="submit" disabled={busy === 'save'} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2 text-sm font-semibold hover:bg-accent"><Save className="h-4 w-4" /> {draft.content_type === 'video_teaser' ? 'Save landing video' : 'Save draft'}</button>{draft.content_type === 'blog_article' && <button type="button" onClick={() => void publishBlog()} disabled={busy === 'publish'} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground"><CheckCircle2 className="h-4 w-4" /> Publish</button>}{selectedId && draft.content_type === 'blog_article' && <button type="button" onClick={() => void remove()} className="inline-flex items-center gap-2 rounded-xl border border-destructive/30 px-4 py-2 text-sm font-semibold text-destructive hover:bg-destructive/5"><Trash2 className="h-4 w-4" /> Delete</button>}</div>
        </form>
      </section>
    </div>
  </div>
