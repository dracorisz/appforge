...
export function AdminContentManager({ embedded = false, adminVerified = false }: Props) {
  ...
  if (!user) return <div className="rounded-xl border border-border/70 p-6 text-center"><ShieldCheck className="mx-auto h-8 w-8" /><h2 className="mt-3 text-lg font-semibold">Admin sign-in required</h2><Link to="/login" className="mt-4 inline-flex text-sm underline">Sign in</Link></div>
  if (!isAdmin) return <div className="rounded-xl border border-border/70 p-6 text-center"><ShieldCheck className="mx-auto h-8 w-8" /><h2 className="mt-3 text-lg font-semibold">Admin only</h2></div>
  ...
  return <div className="w-full space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <section className="rounded-xl border border-border/70 bg-background/40 p-4 sm:p-5">
        {draft.content_type === 'blog_article' && <div className="mb-4 rounded-xl border border-border/70 p-3"><div className="flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4" /> Vertex writing</div><div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={vertexTopic} onChange={(event) => setVertexTopic(event.target.value)} className="h-10 min-w-0 flex-1 rounded-xl border border-input bg-background px-3 text-sm" placeholder="Topic" /><button type="button" onClick={() => void draftWithVertex(false)} disabled={busy === 'vertex'} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-border px-3 text-sm font-semibold hover:bg-accent">{busy === 'vertex' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} Generate draft</button><button type="button" onClick={() => void draftWithVertex(true)} disabled={busy === 'vertex'} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-primary px-3 text-sm font-semibold text-primary-foreground"><CheckCircle2 className="h-4 w-4" /> Generate & publish</button></div></div>}
        <form onSubmit={save} className="space-y-4">
          <div className="flex items-center justify-between"><h3 className="text-lg font-semibold">{draft.content_type === 'video_teaser' ? 'Landing walkthrough' : selectedId ? 'Edit article' : 'New article'}</h3>{selectedId && draft.content_type === 'blog_article' && <button type="button" onClick={newItem} className="rounded-xl p-1.5 text-muted-foreground hover:bg-accent"><X className="h-4 w-4" /></button>}</div>
          <div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-medium text-muted-foreground">Title<input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-input bg-background px-3 text-sm text-foreground" /></label><label className="text-xs font-medium text-muted-foreground">Slug<input value={draft.slug} disabled={draft.content_type === 'video_teaser'} onChange={(event) => setDraft((current) => ({ ...current, slug: event.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-input bg-background px-3 text-sm text-foreground disabled:opacity-60" placeholder="article-slug" /></label></div>
          <label className="block text-xs font-medium text-muted-foreground"><span className="flex items-center justify-between"><span>Summary</span>{draft.content_type === 'blog_article' && <button type="button" onClick={() => setDraft((current) => ({ ...current, summary: generateSummary(current.body, current.title) }))} className="text-[11px] text-foreground hover:underline">Generate</button>}</span><textarea value={draft.summary || ''} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} rows={2} className="mt-1 w-full rounded-xl border border-input bg-background p-3 text-sm text-foreground" /></label>
          {draft.content_type === 'blog_article' && <label className="block text-xs font-medium text-muted-foreground">Body (Markdown)<textarea value={draft.body || ''} onChange={(event) => setDraft((current) => ({ ...current, body: event.target.value }))} rows={14} className="mt-1 w-full rounded-xl border border-input bg-background p-3 text-sm leading-6 text-foreground" /></label>}
          <div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-medium text-muted-foreground">Image URL<input value={draft.image_url || ''} onChange={(event) => setDraft((current) => ({ ...current, image_url: event.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-input bg-background px-3 text-sm text-foreground" /></label><label className="text-xs font-medium text-muted-foreground">Video URL<input value={draft.video_url || ''} onChange={(event) => setDraft((current) => ({ ...current, video_url: event.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-input bg-background px-3 text-sm text-foreground" /></label></div>
          <div className="flex flex-wrap items-center gap-2"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border/70 px-3 py-2 text-xs font-medium hover:bg-accent"><Upload className="h-4 w-4" /><input type="file" className="hidden" accept="image/*,video/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadMedia(file); event.currentTarget.value = '' }} />{busy === 'upload' ? 'Uploading…' : 'Upload media'}</label>{draft.content_type === 'blog_article' && <label className="inline-flex items-center gap-2 text-xs"><input type="checkbox" checked={draft.published} onChange={(event) => setDraft((current) => ({ ...current, published: event.target.checked }))} /> Published</label>}<button type="submit" disabled={busy === 'save'} className="inline-flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-xs font-semibold hover:bg-accent"><Save className="h-4 w-4" /> {draft.content_type === 'video_teaser' ? 'Save landing video' : 'Save draft'}</button>{draft.content_type === 'blog_article' && <button type="button" onClick={() => void publishBlog()} disabled={busy === 'publish'} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground"><CheckCircle2 className="h-4 w-4" /> Publish</button>}{selectedId && draft.content_type === 'blog_article' && <button type="button" onClick={() => void remove()} className="inline-flex items-center gap-2 rounded-xl border border-destructive/30 px-3 py-2 text-xs font-semibold text-destructive hover:bg-destructive/5"><Trash2 className="h-4 w-4" /> Delete</button>}</div>
        </form>
      </section>
    </div>
  </div>
