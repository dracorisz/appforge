...
export function AdminContentManager({ embedded = false, adminVerified = false, contentType }: Props) {
  ...
  return <div className="w-full space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="rounded-xl border border-border/70 bg-background/40 p-4"><button onClick={newItem} className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-border/70 text-sm font-semibold hover:bg-accent"><Plus className="h-4 w-4" /> {filterType === 'video_teaser' && landingTeaser ? 'Open walkthrough' : 'New'}</button><div className="mt-4 space-y-2">{visible.map((item) => <button key={item.id} onClick={() => selectItem(item)} className={`w-full rounded-xl border p-4 text-left ${selectedId === item.id ? 'border-foreground/30 bg-accent/50' : 'border-border/60 hover:bg-accent/25'}`}><div className="truncate text-sm font-medium">{item.title}</div><div className="mt-2 truncate text-xs text-muted-foreground">{item.slug}</div></button>)}{!visible.length && <div className="p-4 text-center text-xs text-muted-foreground">No items.</div>}</div></aside>
      ...
      <section className="rounded-xl border border-border/70 bg-background/40 p-4 sm:p-4">
        ...
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2"><label className="text-xs font-medium text-muted-foreground">Title<input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground" /></label><label className="text-xs font-medium text-muted-foreground">Slug<input value={draft.slug} disabled={draft.content_type === 'video_teaser'} onChange={(event) => setDraft((current) => ({ ...current, slug: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground disabled:opacity-60" placeholder="article-slug" /></label></div>
          <label className="block text-xs font-medium text-muted-foreground"><span className="flex items-center justify-between"><span>Summary</span>{draft.content_type === 'blog_article' && <button type="button" onClick={() => setDraft((current) => ({ ...current, summary: generateSummary(current.body, current.title) }))} className="text-xs text-foreground hover:underline">Generate</button>}</span><textarea value={draft.summary || ''} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} rows={2} className="mt-2 w-full rounded-xl border border-input bg-background p-4 text-sm text-foreground" /></label>
          {draft.content_type === 'blog_article' && <label className="block text-xs font-medium text-muted-foreground">Body (Markdown)<textarea value={draft.body || ''} onChange={(event) => setDraft((current) => ({ ...current, body: event.target.value }))} rows={14} className="mt-2 w-full rounded-xl border border-input bg-background p-4 text-sm leading-6 text-foreground" /></label>}
          <div className="grid gap-4 md:grid-cols-2"><label className="text-xs font-medium text-muted-foreground">Image URL<input value={draft.image_url || ''} onChange={(event) => setDraft((current) => ({ ...current, image_url: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground" /></label><label className="text-xs font-medium text-muted-foreground">Video URL<input value={draft.video_url || ''} onChange={(event) => setDraft((current) => ({ ...current, video_url: event.target.value }))} className="mt-2 h-10 w-full rounded-xl border border-input bg-background px-4 text-sm text-foreground" /></label></div>
          <div className="flex flex-wrap items-center gap-2"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border/70 px-4 py-2 text-xs font-medium hover:bg-accent"><Upload className="h-4 w-4" /><input type="file" className="hidden" accept="image/*,video/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadMedia(file); event.currentTarget.value = '' }} />{busy === 'upload' ? 'Uploading…' : 'Upload media'}</label>{draft.content_type === 'blog_article' && <label className="inline-flex items-center gap-2 text-xs"><input type="checkbox" checked={draft.published} onChange={(event) => setDraft((current) => ({ ...current, published: event.target.checked }))} /> Published</label>}<button type="submit" disabled={busy === 'save'} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2 text-xs font-semibold hover:bg-accent"><Save className="h-4 w-4" /> {draft.content_type === 'video_teaser' ? 'Save landing video' : 'Save draft'}</button>{draft.content_type === 'blog_article' && <button type="button" onClick={() => void publishBlog()} disabled={busy === 'publish'} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground"><CheckCircle2 className="h-4 w-4" /> Publish</button>}{selectedId && draft.content_type === 'blog_article' && <button type="button" onClick={() => void remove()} className="inline-flex items-center gap-2 rounded-xl border border-destructive/30 px-4 py-2 text-xs font-semibold text-destructive hover:bg-destructive/5"><Trash2 className="h-4 w-4" /> Delete</button>}</div>
        </form>
      </section>
    </div>
  </div>
