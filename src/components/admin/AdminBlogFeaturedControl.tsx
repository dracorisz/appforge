...
export function AdminBlogFeaturedControl() {
  ...
  const refresh = React.useCallback(async () => {
    setLoading(true)
    try {
      const rows = (await loadAllFrontendContent()).filter((row) => row.content_type === 'blog_article')
      setArticles(rows)
  ...
  return <Card className="p-3">
    <div className="flex items-start gap-2"><Star className="mt-0.5 h-4 w-4 text-muted-foreground" /><div><h2 className="text-sm font-semibold">Featured blog article</h2><p className="mt-1 text-[11px] leading-4 text-muted-foreground">Choose the single article highlighted on the public Blog landing page.</p></div></div>
    <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
      <Select value={selected} onChange={(event) => setSelected(event.target.value)} disabled={loading || !articles.length} aria-label="Featured blog article">
        {!articles.length && <option value="">No blog articles</option>}
        {articles.map((article) => <option key={article.id} value={article.id}>{article.title}{article.published ? '' : ' (draft)'}</option>)}
      </Select>
