...
function App() {
  ...
  const addToRecent = (appId: string) => {
    setState((prev) => ({
      ...prev,
      recentApps: [appId, ...(prev.recentApps || []).filter((id) => id !== appId)].slice(0, 20),
    }))
  }
  ...
  const updateMiniApp = (updated: MiniApp) => {
    setState((prev) => ({
      ...prev,
      miniApps: prev.miniApps.map((app) => app.id === updated.id ? updated : app),
    }))
  }
