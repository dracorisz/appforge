...
const HF_FREE_IMAGE_MODEL = process.env.HF_IMAGE_MODEL || 'stabilityai/stable-diffusion-xl-base-1.0'

const generateImageFromHf = async (prompt, hfToken) => {
  const response = await fetch(`https://api-inference.huggingface.co/models/${encodeURIComponent(HF_FREE_IMAGE_MODEL)}`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${hfToken}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ inputs: `${prompt.trim().slice(0, 900)} Cinematic fantasy game scene. No words, logos, UI, watermark.` }),
  })
  if (!response.ok) {
...
export default async function handler(req, res) {
  ...
  const hfToken = userHfToken || nextHfToken()
  ...
  try {
    ...
    if (!imageUrl && hfToken) {
      try {
        const hfResult = await generateImageFromHf(prompt, hfToken)
        const saved = await saveImage(token, user.id, null, hfResult.buffer, hfResult.mimeType)
        return res.status(200).json({ imageUrl: saved.publicUrl, storagePath: saved.storagePath, model: HF_FREE_IMAGE_MODEL, sessionId, personalKeyUsed: usingPersonalKey, provider: 'hf' })
      } catch (hfError) {
        lastError = hfError
      }
    }
    ...
    if (!imageUrl) {
      ...
      const reason = lastError instanceof Error ? lastError.message : 'The scene generator could not answer. Your image turn was not consumed.'
      return res.status(502).json({ error: reason })
    }

    const saved = await saveImage(token, user.id, imageUrl)
    return res.status(200).json({ imageUrl: saved.publicUrl, storagePath: saved.storagePath, model, sessionId, personalKeyUsed: usingPersonalKey, provider: 'openrouter' })
  } catch (error) {
