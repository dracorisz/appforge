...
const PROMPT_SUFFIX = 'MANDATORY OUTPUT REQUIREMENT: generate exactly one original cute desktop assistant character as a PNG image with a genuinely transparent alpha background. The background outside the silhouette MUST be transparent, not white, colored, checkerboard, or scenery. The character itself MUST be fully colored and visually opaque: solid filled body, face, clothing and accessories; no transparent holes, ghosted patches, semi-transparent body parts, cutout textures, checkerboard painted inside the design, or background pattern visible through the character. Keep transparency only outside the silhouette and in natural empty gaps such as between separated limbs. Full body, centered, expressive and readable at small size, clean silhouette, generous transparent padding, no words, logos, captions, UI, watermark, border, floor, scenery, or text. Output must be suitable for direct use as a polished transparent PNG sprite.'
